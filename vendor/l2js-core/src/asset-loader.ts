...
> {
  private packages = new Map<
    string,
    Map<
      C.SupportedExtensions_T,
      TPackage | TCorePackage | TEnginePackage | TNativePackage
    >
  >();

  protected abstract createPackage(
    UPackage:
      | C.APackageConstructor
      | C.ACorePackageConstructor
      | C.AEnginePackageConstructor,
    downloadPath: string,
  ): TPackage;
  protected abstract createNativePackage(
  ...
  public getCorePackage() {
    return this.pkgCore;
  }
  public getEnginePackage() {
    return this.pkgEngine;
  }
  public getNativePackage() {
    return this.pkgNative;
  }

  protected init(
    assetList: L2JS.Core.IAssetListInfo,
    { UPackage, UCorePackage, UEnginePackage, UNativePackage }: InitParams_T,
  ) {
    this.packages.set(
      "native",
      new Map([["U", this.createNativePackage(UNativePackage)]]),
    );
    ...
    for (let [path, downloadPath] of Object.entries(assetList)) {
      ...
      switch (fullPkgName) {
        case "core.u":
          PackageConstructor = UCorePackage;
          break;
        case "engine.u":
          PackageConstructor = UEnginePackage;
          break;
        default:
          PackageConstructor = UPackage;
          break;
      }

      packages.set(
        pkgExt,
        this.createPackage(PackageConstructor, downloadPath),
      );
    }
  ...
  public getPackage<T extends string | "core" | "engine" | "native">(
    packagePath: T,
  ): ReturnType<T, TPackage, TCorePackage, TEnginePackage, TNativePackage>;
  public getPackage<T extends string | "core" | "engine" | "native">(
    pkgName: T,
    impType?: string,
  ): ReturnType<T, TPackage, TCorePackage, TEnginePackage, TNativePackage>;
...
const packageTypes = new Set<C.SupportedExtensions_T>(
  SUPPORTED_EXTENSIONS.slice().concat(["NATIVE"]),
);
const extToTypes = new Map<C.SupportedExtensions_T, Set<string>>(
  [...packageTypes].map((v) => {
...
function pathToPkgName(path: string): [string, C.SupportedExtensions_T] {
  ...
  if (!packageTypes.has(extUpper))
    throw new Error(
      `Unsupported package type '${ext}' for package '${_path.basename(path)}'`,
    );
...
>(
  allPackages: Map<
    ...
    Map<
      C.SupportedExtensions_T,
      TPackage | TNativePackage | TCorePackage | TEnginePackage
    >
  >,
  pkgName: T,
  impType: string,
...
type ReturnType<
  T extends string | "native" | "core" | "engine",
  TPackage,
  TCorePackage,
  TEnginePackage,
  TNativePackage,
...
type InitParams_T =
  | Record<string, any>
  | {
      // can contain anything but must contain at least these two packages
