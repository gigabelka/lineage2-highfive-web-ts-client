...
const buffInstances: Record<
  InstancedTypes,
  BufferValue<InstancedTypes>
> = numberTypes.reduce((acc, t) => {
...
abstract class UEncodedFile implements IEncodedFile {
  ...
  public ensureReadable() {
    if (!this.isReadable) throw new Error("Stream is not readable!");
  }
  ...
  public seek(offset: number, origin: C.Seek_T = "current") {
    ...
    switch (origin) {
      ...
      case "set":
        ...
        break;
      default:
        throw new Error(`Seek type not supported: ${origin}`);
    }
  }
  ...
  public read<T extends C.PrimitiveNumberTypes_T | "compat32">(
    target: T,
  ): number;
  public read<T extends C.BigNumberTypes_T>(target: T): bigint;
  public read<T extends C.ValueTypeNames_T>(
    target: BufferValue<T>,
  ): BufferValue<T>;
  ...
  public dump(
    ...
    const offsetHeader = printHeaders
      ? new Array(5 + lineCountHex).fill("-").join("")
      : null;
  ...
  protected _doDecode(): Promise<BufferValue> {
    ...
    return (this.handle.promiseDecoding = this.promiseDecoding =
        ...
        if (signature.value === 0x0069004c) {
          ...
          if (version.startsWith("1")) {
            if (this.version === "111") {
              ...
              this.moduloCryptKey =
                (this.read("uint8") as number) ^ modKeys.modulo121;
            }
            ...
            this.buffer = decoders.decryptModulo(
              new Uint8Array(this.buffer, HEADER_SIZE),
              this.moduloCryptKey,
            ) as ArrayBuffer;
            ...
            this.buffer = decoders.rsa.decryptEncdec(
              gmp,
              new Uint8Array(this.buffer, HEADER_SIZE),
            );
