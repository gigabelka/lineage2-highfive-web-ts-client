...
abstract class UObject implements C.ISerializable {
  ...
  public readonly propertyDict = UObject.ALLOW_EDITING
    ? new Map<string, any>()
    : new MapReflectable(this);
  ...
  public constructor(..._: any) {
    ...
    if (!this.isConstructed)
      throw new Error(
        `'${this.constructor.name}' must be created via package.`,
      );
  ...
  private static unserializedPropsCache = new Map<
    Function,
    C.UnserializedProperty_T[]
  >();
  ...
  public static collectUnserializedProperties(): C.UnserializedProperty_T[] {
    ...
    if (this === UObject) return this.getUnserializedProperties();
  ...
  protected makeLayout(): void {
    throw new Error(
      `Layout for '${this.constructor.name}' must be overloaded by the package, was this created via package?.`,
    );
  }
  ...
  public get objectFlags() {
    return this.exp ? this.exp.objectFlags : {};
  }

  protected static getConstructorName(): string {
    throw new Error(
      "Must be implemented by inheriting class because JS does not support inheritence chain.",
    );
  }
  ...
  protected getPropertyVarName(tag: PropertyTag) {
    return tag.name;
  }
  protected findValidProperty(varName: string) {
    return this.findPropReader(varName);
  }
  ...
  protected loadProperty(pkg: APackage, tag: PropertyTag) {
    ...
    if (!property)
      throw new Error(
        `Cannot map '${tag.getTypeName()}' property '${propName}' -> '${varName}' for '${this.constructor.friendlyName ?? this.constructor.name}'`,
      );

    if (property.type !== tag.type)
      throw new Error(
        `Property type mismatch got '${tag.getTypeName()}' expected '${property.getTypeName()}'`,
      );
    ...
    if (pkg.tell() > offEnd)
      throw new Error(
        `Reader exceeded by '${tag.name}' ${offEnd - pkg.tell()} bytes (${((offEnd - pkg.tell()) / 1024).toFixed(2)} kB) for package '${pkg.path}'`,
      );
  ...
  public load(pkg: APackage, info?: any) {
    if (info instanceof UExport) return this.loadWithExport(pkg, info);

    if (info instanceof PropertyTag) return this.loadWithPropertyTag(pkg, info);
  ...
  protected postLoad(pkg: APackage, _exp: UExport): void {
    ...
    if (UObject.UNREAD_AS_NATIVE && this.bytesUnread > 0)
      this.nativeBytes = pkg.read(
        BufferValue.allocBytes(this.readTail - this.readHead),
      );

    if (this.skipRemaining) this.readHead = this.readTail;
    if (this.bytesUnread > 0 && this.careUnread) {
      const constructorName = (this.constructor as any).isDynamicClass
        ? `${(this.constructor as any).friendlyName}[Dynamic]`
