...
abstract class APackage extends UEncodedFile {
  ...
  public async decode(): Promise<this> {
    ...
    if (signature.value !== 0x9e2a83c1)
      throw new Error(
        `Invalid signature: '0x${signature.value.toString(16).toUpperCase()}' expected '0x9E2A83C1'`,
      );
    ...
    readable.importGroups = readable.imports.reduce(
      (accum, imp, index) => {
    ...
    readable.exportGroups = readable.exports.reduce(
      (accum, exp, index) => {
  ...
  protected loadExportTable(header: UHeader, nameTable: UName[]) {
    ...
    for (let i = 0, ec = header.exportCount; i < ec; i++) {
      ...
      if (uexport.size > 0) uexport.offset = this.read("compat32");
  ...
  public getImportEntry(objref: number) {
    ...
    else if (objref > 0) throw Error("Expected an import table entry");
  ...
  public loadExportObject(index: number) {
    ...
    if (entry.idClass !== 0) {
      ...
      let objbase =
        entry.idSuper === 0
          ? null
          : (this.fetchObject(entry.idSuper) as UClass);
      ...
      if (!objbase && objname !== "Object") {
        ...
        if (!pkg.isDecoded())
          throw new Error(`Package must be decoded: 'Core'`);
  ...
  public fetchObject<T extends UObject = UObject>(objref: number): T {
    if (objref > 0) {
      ...
      if (!pkg.isDecoded())
        throw new Error(`Package must be decoded: '${packageName}'`);
  ...
  public fetchObjectByType<T extends UObject>(
    className: string,
    objectName: string,
    ...
    for (const exp of this.exports) {
      ...
      if (groupName !== "None") {
        if (exp.idPackage > 0) {
          ...
          // UE2 FName comparisons are case-insensitive. Import tables spell the
      ...
      if (isClass) {
        ...
        if (obj) {
          const inheritenceChain =
            obj instanceof UClass
              ? [obj.loadSelf().friendlyName]
              : obj.inheritenceChain;
  ...
  public newObject<T extends UObject = UObject>(
    objclass: UClass | ObjectConstructor,
  ): T {
  ...
  protected registerNativeClass(
    className: C.NativeTypes_T,
    baseClass: C.NativeTypes_T | "None" = "None",
  ): void {
    ...
    exp.idClass = 0;
    exp.idSuper =
      baseClass === "None" ? 0 : this.findObjectRef("Class", baseClass);
    exp.idPackage = 0;
...
abstract class ANativePackage extends APackage {
  ...
  public constructor(loader: C.AAssetLoader) {
    super(loader, "__native__.u");
  }

  protected readArrayBuffer(): Promise<ArrayBuffer> {
    throw new Error("Method not used by native package.");
  }
  ...
  public getStructConstructor<T extends typeof UObject = typeof UObject>(
    ...
    return UObject as any;
  }
  protected getNonNativeConstructor<T extends typeof UObject = typeof UObject>(
    constructorName: C.NativeTypes_T,
  ): new () => T {
    return UObject as any;
  }

  public getConstructor<T extends typeof UObject = typeof UObject>(
    constructorName: C.NativeTypes_T,
  ): new () => T {
    let Constructor: any;
    ...
    switch (constructorName) {
      ...
      case "FloatProperty":
        Constructor = UnProperties.UFloatProperty;
        break;
      case "ByteProperty":
        Constructor = UnProperties.UByteProperty;
        break;
      case "StrProperty":
        Constructor = UnProperties.UStrProperty;
        break;
      case "IntProperty":
        Constructor = UnProperties.UIntProperty;
        break;
      case "BoolProperty":
        Constructor = UnProperties.UBoolProperty;
        break;
      case "NameProperty":
        Constructor = UnProperties.UNameProperty;
        break;
      case "ClassProperty":
        Constructor = UnProperties.UClassProperty;
        break;
      case "ArrayProperty":
        Constructor = UnProperties.UArrayProperty;
        break;
      case "StructProperty":
        Constructor = UnProperties.UStructProperty;
        break;
      case "ObjectProperty":
        Constructor = UnProperties.UObjectProperty;
        break;
      case "DelegateProperty":
        Constructor = UnProperties.UDelegateProperty;
        break;

      default:
        Constructor = this.getNonNativeConstructor(constructorName);
        break;
    }
...
function addPackageDependendency(
  nameTable: UName[],
  nameHash: Map<string, number>,
  imports: UImport[],
  classPackage: string,
...
function addClassDependency(
  ...
  const nativePackage = imports.find(
    (imp) => imp.className === "Package" && imp.classPackage === classPackage,
  );
