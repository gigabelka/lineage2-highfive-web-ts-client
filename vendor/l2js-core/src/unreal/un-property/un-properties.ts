...
abstract class UProperty<T1 = any, T2 = T1> extends UField {
  ...
  public getTypeName() {
    return getPropertyTypeName(this.type);
  }
  ...
  public getDefaultValue(): T2 | T2[] {
    console.assert(
      ...
      `Invalid array dimensions: ${this.arrayDimensions}`,
    );

    if (this.arrayDimensions === 1) return this.makeDefaultValue();
...
abstract class UBaseExportProperty<
  T1 extends UObject,
  T2 = T1,
  T3 = T2,
> extends UProperty<T2, T3> {
...
class UObjectProperty<T extends UObject = UObject> extends UBaseExportProperty<
  UObject,
  BufferValue<"compat32">,
  T
> {
  public readonly type = UNP_PropertyTypes.UNP_ObjectProperty;

  protected makeReader(): BufferValue<"compat32"> {
    return new BufferValue(BufferValue.compat32);
  }
  public readValue(pkg: APackage): T {
    return pkg.fetchObject(pkg.read(this.reader).value);
  }
  protected makeDefaultValue(): T {
    return null;
  }

  public toString() {
    const friendlyName =
      this.valueId === 0 ? null : this._value?.friendlyName || "<eval>";

    return super.toString("ObjectProperty", friendlyName);
  }
}

class UClassProperty extends UBaseExportProperty<
  UClass,
  BufferValue<"compat32">,
  UClass
> {
  ...
  public readValue(pkg: APackage): UClass {
    return pkg.fetchObject(pkg.read(this.reader).value);
  }
  protected makeDefaultValue(): UClass {
    return null;
  }
...
class UStructProperty<T extends UObject = UObject> extends UBaseExportProperty<
  C.UStruct,
  T,
  T
> {
  ...
  public readValue(pkg: APackage, tag: PropertyTag): T {
    ...
    const isNative =
      ["Vector", "Rotator", "Color"].includes(tag?.structName || null) ||
      verArchive < 0x76;
...
abstract class UNumericProperty<T extends C.NumberTypes_T | C.StringTypes_T>
  extends UProperty<BufferValue<T>, ReturnType<T>>
  implements IBufferValueProperty<T>
{
  declare ["constructor"]: typeof UNumericProperty & {
    dtype: C.ValidTypes_T<T>;
  };

  public toString() {
    return super.toString(this.constructor.name, undefined);
  }

  public readValue(pkg: APackage) {
    return pkg.read(this.reader).value;
  }
...
class UFloatProperty extends UNumericProperty<"float"> {
  ...
  protected makeDefaultValue(): number {
    return 0;
  }
}
...
class UStrProperty extends UProperty<BufferValue<"char">, string> {
  ...
  protected makeReader(): BufferValue<"char"> {
    return new BufferValue(BufferValue.char);
  }
  protected makeDefaultValue(): string {
    return "";
  }
...
class UDelegateProperty extends UProperty<any, any> {
  protected makeDefaultValue(): any {
    return null;
  }
...
class UBoolProperty extends UProperty<boolean, boolean> {
  ...
  protected makeDefaultValue(): boolean {
    return false;
  }
  protected makeReader(): boolean {
    return null;
  }
  public readValue(pkg: APackage, tag: PropertyTag): boolean {
    return tag.boolValue;
  }

  public toString() {
    return super.toString("BoolProperty", undefined);
  }
}
...
class UNameProperty extends UProperty<BufferValue<"compat32">, string> {
  ...
  public readValue(pkg: APackage): string {
    return pkg.nameTable[pkg.read(this.reader).value].name;
  }
  protected makeReader(): BufferValue<"compat32"> {
    return new BufferValue(BufferValue.compat32);
  }
  ...
  public toString() {
    return super.toString("NameProperty", undefined);
  }
}
...
> {
  ...
  public readValue(pkg: APackage): number {
    return pkg.read(this.reader).value;
  }
  protected makeReader(): BufferValue<"uint8"> {
    return new BufferValue(BufferValue.uint8);
  }
  protected makeDefaultValue(): number {
    return 0;
  }
  ...
  public toString() {
    if (!this.valueId) return super.toString("ByteProperty", undefined);
...
class UArrayProperty extends UBaseExportProperty<
  UProperty<ArrayType, ArrayType>,
  ArrayType,
  ArrayType
> {
  public readonly type = UNP_PropertyTypes.UNP_ArrayProperty;

  protected makeReader(): ArrayType {
    return null;
  }
  protected makeDefaultValue(): ArrayType {
    return null;
  }
  ...
  public readValue(pkg: APackage, tag: PropertyTag): ArrayType {
    ...
    if (type instanceof UStructProperty || type instanceof UClassProperty) {
      // Vector/Rotator/Color elements (and every struct in pre-0x76 archives)
      // are serialized raw, with no per-element property tags — load them
      // natively instead of as tagged property lists.
      const elemStruct = type.value?.loadSelf();
      const nativeElements =
        pkg.header.getArchiveFileVersion() < 0x76 ||
        ["Vector", "Rotator", "Color"].includes(
          elemStruct?.friendlyName || null,
        );

      return new FArray(
        type.value.buildClass(pkg.loader.getNativePackage()),
      ).load(pkg, tag, { nativeElements });
      ...
      return new FArray(type.buildClass(pkg.loader.getNativePackage())).load(
        pkg,
        tag,
      );
    } else {
...
type ReturnType<T extends C.ValueTypeNames_T> = T extends C.NumberTypes_T
  ? T extends C.BigNumberTypes_T
    ? bigint
    : number
  : T extends C.StringTypes_T
    ? string
    : DataView;

type IBufferValueProperty<T extends C.ValueTypeNames_T = C.ValueTypeNames_T> =
  C.IBufferValueProperty<T>;
