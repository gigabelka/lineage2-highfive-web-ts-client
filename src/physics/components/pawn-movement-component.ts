...
import { PAWN_TELEPORTED_EVENT, SCRIPT_NATIVE_EVENT } from "@client/game/component-events";
...
function findVolumeTransition(_start: Vector3, _end: Vector3, _bsp: unknown, _startsInside: boolean): number {
...
export class PawnMovementComponent extends PhysicsComponent<BaseActor> {
    ...
    protected get transformComponent(): ITransformComponent | null { return this.findComponent<any>("transform"); }
    ...
    public getBaseActor(): ICollidable | null { return this.transformComponent?.getBase() ?? null; }
    public getBasedActors(): ReadonlySet<ICollidable> { return this.transformComponent?.getBasedActors() ?? EMPTY_BASED_ACTORS; }
    ...
    protected getWaterVolumeAt(position: Vector3): IWaterVolumeDecodeInfo | null {
        ...
        for (const sector of this.renderManager.getLoadedSectors()) {
            ...
            const getter = (sector as any).getWaterVolumeAt as ((position: Vector3) => IWaterVolumeDecodeInfo | null) | undefined;
