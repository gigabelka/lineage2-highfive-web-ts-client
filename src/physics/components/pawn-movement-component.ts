...
import { Box3, Euler, LoopOnce, Object3D, Quaternion, Vector3 } from "three";
import {
  COMPONENT_EVENT_NOT_HANDLED,
  ComponentEventResult_T,
} from "@client/game/components";
import {
  PAWN_TELEPORTED_EVENT,
  SCRIPT_NATIVE_EVENT,
} from "@client/game/component-events";
...
import type {
  ActorCollisionProfile_T,
  CollisionPrimitive_T,
  ICollidable,
} from "@client/objects/objects";
import type {
  CheckResult_T,
  CollisionQuery_T,
...
import type {
  ...
  ScriptValue_T,
  Vector3Arr,
} from "@client/ue-script/script-values";

export { PAWN_TELEPORTED_EVENT };

/**
 * Minimal shape of a UE2 water `PhysicsVolume`. The decode-side type (`un-water-volume.ts`) and the
 * volume BSP live in Phase 7/Phase 2 respectively; Phase 1 never produces one, so the swimming path
 * is reachable but inert. Do not widen this without porting the decode side.
 */
type IWaterVolumeDecodeInfo = {
  priority: number;
  gravity: [number, number, number];
  zoneVelocity: [number, number, number];
  bsp: unknown;
};

/**
 * The portion of `TransformComponent` the movement physics touches (base riding: standing on
 * movers, inheriting their translation/rotation). TransformComponent lands in Phase 4; every call
 * site below is optional-chained and is a no-op until then.
 */
type ITransformComponent = {
  getBase(): ICollidable | null;
  setBase(actor: ICollidable | null): void;
  getBasePosition(): Vector3;
  getBaseQuaternion(): Quaternion;
  updateBaseTransform(position: Vector3, quaternion: Quaternion): void;
  updateBaseRelativePosition(): void;
  getBasedActors(): ReadonlySet<ICollidable>;
  addBasedActor(actor: ICollidable): void;
  removeBasedActor(actor: ICollidable): void;
};

const EMPTY_BASED_ACTORS: ReadonlySet<ICollidable> = new Set<ICollidable>();

// Phase 2 replaces this stub with the real import from `../volume-bsp`:
//     import { findVolumeTransition } from "../volume-bsp";
// Unreachable in Phase 1 - nothing hands out a water volume yet.
function findVolumeTransition(
  _start: Vector3,
  _end: Vector3,
  _bsp: unknown,
  _startsInside: boolean,
): number {
  throw new Error("volume BSP not yet implemented (Phase 2)");
}
...
export class PawnMovementComponent extends PhysicsComponent<BaseActor> {
  ...
  protected readonly analyticalPrimitive: CollisionPrimitive_T<"cylinder"> = {
    kind: "cylinder",
    center: this.analyticalCenter,
    radius: this.collisionRadius,
    halfHeight: this.collisionHeight,
  ...
  protected readonly collisionQuery: CollisionQuery_T = {
    location: null,
    delta: null,
    extent: null,
  ...
  protected get position(): Vector3 {
    return this.getParent().position;
  }
  protected get rotation(): Euler {
    return this.getParent().rotation;
  }
  // TransformComponent lands in Phase 4; until then this resolves to null and every base-riding
  // call below is a no-op.
  protected get transformComponent(): ITransformComponent | null {
    return this.findComponent<any>("transform");
  }

  public getPhysicsTickRate(): number {
    return this.tickRate;
  }
  public setPhysicsTickRate(tickRate: number): void {
  ...
  public onEvent(
    type: string,
    data: unknown,
  ): ComponentEventResult_T<ScriptValue_T> {
    ...
    if (typeof context.setUnrealScriptProperty === "function")
      context.setUnrealScriptProperty("Physics", call.args[0]);
    else if (context.scriptProperties instanceof Map)
      context.scriptProperties.set("Physics", call.args[0]);
    else throw new Error(`'${context.scriptClassId}' has no physics mode.`);

    // Handled, no value - the donor falls off the end here; main's tsconfig wants it explicit.
    return undefined;
  }
  ...
  public getBaseActor(): ICollidable | null {
    return this.transformComponent?.getBase() ?? null;
  }
  public getBasedActors(): ReadonlySet<ICollidable> {
    return this.transformComponent?.getBasedActors() ?? EMPTY_BASED_ACTORS;
  }
  public addBasedActor(actor: ICollidable) {
    this.transformComponent?.addBasedActor(actor);
  }
  public removeBasedActor(actor: ICollidable) {
    this.transformComponent?.removeBasedActor(actor);
  }
  public isInteractive(): boolean {
    return this.renderManager.isSectorCollisionReady(this.position);
  }
  ...
  public getCollisionPrimitive(): CollisionPrimitive_T {
    ...
    if (this.analyticalOrigin.equals(this.position))
    ...
    this.analyticalBounds.min.set(
      this.analyticalCenter.x - this.collisionRadius,
      this.analyticalCenter.y - this.collisionRadius,
      this.analyticalCenter.z - this.collisionHeight,
    );
    this.analyticalBounds.max.set(
      this.analyticalCenter.x + this.collisionRadius,
      this.analyticalCenter.y + this.collisionRadius,
      this.analyticalCenter.z + this.collisionHeight,
    );
  ...
  public createCollider(physicsWorld: RAPIER.World): RAPIER.Collider {
    ...
    const rigidbodyDesc =
      RAPIER.RigidBodyDesc.kinematicPositionBased().setTranslation(
        this.position.x,
        this.position.y,
        this.position.z + this.collisionHeight,
      );
  ...
  protected updatePhysics(currentTime: number, deltaTime: number) {
    ...
    while (remainingTime > 0 && iteration++ < 8) {
      const tick =
        this.hasStartedPhysics && this.physicsMode === "walking"
          ? remainingTime
          : remainingTime <= 0.05
            ? remainingTime
            : Math.min(0.05, remainingTime * 0.5);
  ...
  protected updateEnterRise(deltaTime: number): void {
    ...
    if (
      (this.enterRiseVelocity >= 0 && nextZ >= targetZ) ||
      (this.enterRiseVelocity < 0 && nextZ <= targetZ)
    ) {
  ...
  protected updateIgnoredActors(position: Vector3) {
    ...
    for (const actor of this.ignoredActors)
      if (!this.isOverlapping(actor, position))
        this.ignoredActors.delete(actor);
  }

  protected isOverlapping(actor: ICollidable, position: Vector3): boolean {
    const primitive = actor.getCollisionPrimitive
      ? actor.getCollisionPrimitive()
      : null;
    ...
    return (
      ...
      Math.abs(dz) < primitive.halfHeight + this.collisionHeight
    );
  }
  ...
  public ignoreOverlappingActors(actors: Iterable<ICollidable>) {
    for (const actor of actors)
      if (
        actor !== this.getParent() &&
        this.isOverlapping(actor, this.position)
      )
        this.ignoredActors.add(actor);
  }
  ...
  protected tickPhysics(deltaTime: number) {
      ...
      if (waterVolume) {
        ...
        if (floorHit) {
          ...
          if (tmpNormal.z >= MIN_FLOOR_Z) {
            position
              .addScaledVector(floorMovement, floorHit.time)
              .addScaledVector(tmpUp, FLOOR_DISTANCE);
    ...
    const isThreeDimensional =
      ...
      (this.physicsMode === "swimming" && desired.swimToDepth);
    ...
    const willReachDestination =
      ...
      Math.max(0, distance - desired.offset) <= maxSpeed * deltaTime;

    if (this.actorState.locomotion) {
      this.acceleration
        .set(distanceX, distanceY, distanceZ)
        .normalize()
        .multiplyScalar(ACCEL_RATE);

      if (desired.faceTarget) {
        desired.faceTarget.getWorldPosition(tmpMovement);
        this.setDesiredHeading(
          tmpMovement.x - position.x,
          tmpMovement.y - position.y,
        );
      } else if (desired.faceMovement)
        this.setDesiredHeading(distanceX, distanceY);
    } else {
      this.acceleration.set(0, 0, 0);
    }

    if (this.physicsMode === "walking" && waterVolume)
      this.physicsMode = "swimming";
    ...
    switch (this.physicsMode) {
      case "none":
        ...
        break;
      case "walking":
        this.physWalking(position, deltaTime);
        break;
      case "falling":
        this.physFalling(position, deltaTime);
        break;
      case "swimming":
        this.physSwimming(position, deltaTime, waterVolume);
        break;
      case "flying":
        this.physFlying(position, deltaTime);
        break;
      default:
        throw new Error(`Unknown player physics mode '${this.physicsMode}'.`);
    }
  ...
  protected calcVelocity(
    ...
    const dz =
      this.physicsMode === "flying" ||
      (this.physicsMode === "swimming" && this.actorState.desired.swimToDepth)
        ? this.actorState.desired.position.z - position.z
        : 0;
    const distance =
      Math.sqrt(dx * dx + dy * dy + dz * dz) - this.actorState.desired.offset;
  ...
  protected updateBaseMovement(position: Vector3) {
    ...
    // TransformComponent lands in Phase 4, this call is a no-op until then.
    if (!transform) return;
    ...
    if (
      tmpBasePosition.equals(transform.getBasePosition()) &&
      tmpBaseQuaternion.equals(transform.getBaseQuaternion())
    )
      return;
    ...
    tmpBaseOffset
      .copy(position)
      .sub(transform.getBasePosition())
      .applyQuaternion(tmpBaseInverseQuaternion)
      .applyQuaternion(tmpBaseQuaternion)
      .add(tmpBasePosition);
    ...
    tmpBaseDeltaQuaternion
      .copy(tmpBaseQuaternion)
      .multiply(tmpBaseInverseQuaternion);

    const yaw = Math.atan2(
      2 *
        (tmpBaseDeltaQuaternion.w * tmpBaseDeltaQuaternion.z +
          tmpBaseDeltaQuaternion.x * tmpBaseDeltaQuaternion.y),
      1 -
        2 *
          (tmpBaseDeltaQuaternion.y * tmpBaseDeltaQuaternion.y +
            tmpBaseDeltaQuaternion.z * tmpBaseDeltaQuaternion.z),
    );

    this.rotationYaw =
      (this.rotationYaw + Math.round((yaw * 32768) / Math.PI)) & 65535;
  ...
  // TransformComponent lands in Phase 4, this call is a no-op until then.
  protected updateBaseRelativePosition(): void {
    this.transformComponent?.updateBaseRelativePosition();
  }

  protected physWalking(
    position: Vector3,
    deltaTime: number,
    iterations: number = 0,
  ) {
    ...
    while (remainingTime > 0 && iterations++ < 8) {
      const timeTick =
        (this.getParent() as any).isPlayer && remainingTime > 0.05
          ? Math.min(0.05, remainingTime * 0.5)
          : remainingTime;
      ...
      if (floorHit) {
        ...
        if (tmpNormal.z >= MIN_FLOOR_Z) {
          ...
          if (floorDistance > MAX_FLOOR_DISTANCE)
            this.moveSwept(
              position,
              tmpDown.set(0, 0, -(floorDistance - FLOOR_DISTANCE)),
            );
          else if (floorDistance > 0 && floorDistance < MIN_FLOOR_DISTANCE)
            this.moveSwept(
              position,
              tmpStepUp.set(0, 0, FLOOR_DISTANCE - floorDistance),
            );
        ...
        if (deltaX * tmpNormal.x + deltaY * tmpNormal.y < 0) {
          tmpStepUp
            .copy(tmpUp)
            .multiplyScalar(MAX_STEP_HEIGHT)
            .addScaledVector(tmpNormal, -MAX_STEP_HEIGHT * tmpNormal.z)
            .multiplyScalar(-1);
      ...
      if (desiredDistance > 0)
        remainingTime +=
          timeTick * (1 - Math.min(1, actualDistance / desiredDistance));
  ...
  protected traceExtent(
    ...
    query.location = tmpBodyPosition
      .copy(start)
      .addScaledVector(tmpUp, this.collisionHeight);
  ...
  protected checkForLedges(
    ...
    if (
      !this.traceExtent(
        position,
        tmpLedgeDelta.set(0, 0, -LEDGE_PROBE),
        radius,
        height,
      )
    )
    ...
    if (
      this.traceExtent(
        position,
        tmpLedgeDelta.copy(tmpLedgeEnd).sub(position),
        0,
        0,
      )
    )
    ...
    const dropDistance =
      Math.max(MAX_STEP_HEIGHT, distance + radius) + height + LEDGE_PROBE;
    const dropHit = this.traceExtent(
      tmpLedgeEnd,
      tmpLedgeDelta.set(0, 0, -dropDistance),
      0,
      0,
    );
    ...
    if (dropHit && dropHit.normal.z >= MIN_FLOOR_Z) {
      const slope = Math.min(
        MAX_STEP_HEIGHT,
        ((distance + radius) *
          Math.sqrt(1 - dropHit.normal.z * dropHit.normal.z)) /
          dropHit.normal.z,
      );
    ...
    const floorHit = this.traceExtent(
      tmpLedgeEnd,
      tmpLedgeDelta.set(0, 0, -LEDGE_DROP),
      radius,
      height,
    );
  ...
  protected findLedgeDetour(
    ...
    const floorHit = this.traceExtent(
      tmpLedgeDelta,
      tmpLedgeDrop.set(0, 0, -LEDGE_DROP),
      radius,
      height,
    );
  ...
  protected stepUp(
    ...
    if (
      Math.abs(tmpStepNormal.z) < MAX_STEP_SIDE_Z ||
      tmpStepNormal.z >= MIN_FLOOR_Z
    ) {
      ...
      tmpStepRemaining.set(
        movement.x,
        movement.y,
        movement.z + movement.length() * tmpStepNormal.z,
      );
    ...
    if (
      Math.abs(tmpNormal.z) < MAX_STEP_SIDE_Z &&
      movement.lengthSq() * current.time > STEP_RECURSE_DIST_SQ
    ) {
      this.moveActor(position, tmpDown.set(0, 0, -MAX_STEP_HEIGHT));
    ...
    tmpStepRemaining
      .copy(movement)
      .addScaledVector(tmpStepNormal, -movement.dot(tmpStepNormal))
      .multiplyScalar(1 - current.time);
  ...
  protected physFalling(
    ...
    this.velocity.z = Math.max(
      -DEFAULT_VOLUME_TERMINAL_VELOCITY,
      this.velocity.z + DEFAULT_VOLUME_GRAVITY_Z * deltaTime,
    );
    tmpMovement
      .addVectors(tmpOldVelocity, this.velocity)
    ...
    if (tmpNormal.z >= MIN_FLOOR_Z) {
      ...
      if (remainingTime >= 0.0003)
        this.physWalking(position, remainingTime, iterations);
      return;
    }
    ...
    tmpRemaining
      .copy(tmpMovement)
      .multiplyScalar(1 - hit.time)
      .addScaledVector(tmpNormal, -tmpRemaining.dot(tmpNormal));
  ...
  protected physSwimming(
    position: Vector3,
    deltaTime: number,
    volume: IWaterVolumeDecodeInfo | null,
    iterations: number = 0,
  ) {
    if (deltaTime < 0.0003 || iterations > 7) return;
    ...
    if (!hit) {
      ...
      remainingTime =
        deltaTime *
        Math.min(1, Math.abs(position.z - outZ) / tmpMovement.length());
      tmpMovement.copy(position).sub(tmpSwimStart);
      ...
      if (this.getWaterVolume(position)) {
        if (remainingTime < deltaTime)
          this.velocity
            .copy(tmpMovement)
            .multiplyScalar(1 / (deltaTime - remainingTime));
    ...
    this.velocity
      .copy(tmpMovement)
      .multiplyScalar(1 / Math.max(0.0003, deltaTime - remainingTime));
  ...
  protected physFlying(position: Vector3, deltaTime: number) {
    ...
    if (volume) {
      ...
      if (
        !(this.getParent() as any).isPlayer &&
        tmpVelocityDelta.lengthSq() <= 90000
      )
        tmpVelocityDelta.set(0, 0, 0);
    }
  ...
  protected resolveFlyingHit(
    ...
    if (
      Math.abs(tmpStepNormal.z) < 0.2 &&
      gravityVelocityDot > -0.2 &&
      gravityVelocityDot < 0.5
    ) {
      tmpRemaining.copy(movement).multiplyScalar(1 - hit.time);
      this.stepUpFlying(
        position,
  ...
  protected stepUpFlying(
    ...
    this.moveSwept(
      position,
      tmpStepUp.copy(gravityDirection).multiplyScalar(-MAX_STEP_HEIGHT),
    );
    ...
    if (hit) {
      ...
      if (tmpStepNormal.lengthSq() > 0) {
        ...
        tmpStepRemaining
          .copy(movement)
          .multiplyScalar(1 - hit.time)
          .addScaledVector(tmpStepNormal, -tmpStepRemaining.dot(tmpStepNormal));
    ...
    this.moveSwept(
      position,
      tmpDown.copy(gravityDirection).multiplyScalar(MAX_STEP_HEIGHT),
    );
  }
  ...
  protected moveWithWallResponse(position: Vector3, movement: Vector3) {
    ...
    tmpRemaining
      .copy(movement)
      .multiplyScalar(1 - hit.time)
      .addScaledVector(tmpStepNormal, -tmpRemaining.dot(tmpStepNormal));
  ...
  protected getWaterVolume(position: Vector3): IWaterVolumeDecodeInfo | null {
    tmpWaterPosition
      .copy(position)
      .addScaledVector(tmpUp, this.collisionHeight + WATERLINE_DEPTH);
  ...
  protected getWaterVolumeAt(position: Vector3): IWaterVolumeDecodeInfo | null {
    ...
    for (const sector of this.renderManager.getLoadedSectors()) {
      // Water volumes are decoded in Phase 7; SectorObject has no getWaterVolumeAt yet.
      const getter = (sector as any).getWaterVolumeAt as
        | ((position: Vector3) => IWaterVolumeDecodeInfo | null)
        | undefined;

      if (!getter) return null;

      const volume = getter.call(sector, position);

      if (volume && (!selected || volume.priority >= selected.priority))
        selected = volume;
    }
  ...
  protected findWaterTransition(
    position: Vector3,
    movement: Vector3,
    startsInWater: boolean,
  ): number {
    tmpWaterEnd.copy(position).add(movement);
    const volume = startsInWater
      ? this.getWaterVolume(position)
      : this.getWaterVolume(tmpWaterEnd);
  ...
  public moveActor(position: Vector3, movement: Vector3): CheckResult_T | null {
    const bodyPosition = tmpBodyPosition
      .copy(position)
      .addScaledVector(tmpUp, this.collisionHeight);
    const query = this.collisionQuery;
    ...
    query.delta = movement;
    query.extent = tmpStepPosition.set(
      this.collisionRadius,
      this.collisionRadius,
      this.collisionHeight,
    );
  ...
  protected setDesiredHeading(x: number, y: number) {
    ...
    this.desiredRotationYaw =
      Math.round((Math.atan2(y, x) * 32768) / Math.PI) & 65535;
  ...
  protected physicsRotation(deltaTime: number) {
    ...
    const deltaRate = (this.getParent() as any).isPlayer
      ? Math.round(PLAYER_YAW_RATE * deltaTime)
      : Math.trunc(YAW_RATE * deltaTime);

    this.rotationYaw = fixedTurn(
      this.rotationYaw,
      this.desiredRotationYaw,
      deltaRate,
    );
    ...
    if (this.rotationYaw === this.desiredRotationYaw)
  ...
  protected checkAnimationState() {
    ...
    const state: PawnMovementState_T = !this.hasStartedPhysics
      ? "idle"
      : this.physicsMode === "falling"
        ? "falling"
        : this.physicsMode === "swimming"
  ...
  public goTo(position: Vector3): Vector3 | null {
    ...
    const target =
      this.physicsMode === "flying" || this.physicsMode === "swimming"
        ? position
  ...
  public goToActor(actor: Object3D, offset: number = 0) {
    ...
    this.actorState.desired.swimToDepth = !!this.getWaterVolumeAt(
      this.actorState.desired.position,
    );
  ...
  public isIdle(): boolean {
    return this.actorState.state === "idle";
  }
  public isDying(): boolean {
    return this.actorState.state === "dying";
  }
  public isRunning(): boolean {
    return this.actorState.state === "running";
  }
  public isLocomoting(): boolean {
    return this.actorState.locomotion;
  }
  public isWalkingMovement(): boolean {
    return this.isWalking;
  }
  public isSwimmingMovement(): boolean {
    return this.physicsMode === "swimming";
  }
  ...
  public isUnderwaterMovement(): boolean {
    tmpWaterPosition
      .copy(this.position)
      .addScaledVector(tmpUp, this.collisionHeight * 2);
  ...
  public setCollisionSize(collisionRadius: number, collisionHeight: number) {
    if (!Number.isFinite(collisionRadius) || collisionRadius <= 0)
      throw new Error(`Invalid pawn CollisionRadius '${collisionRadius}'.`);
    if (!Number.isFinite(collisionHeight) || collisionHeight <= 0)
      throw new Error(`Invalid pawn CollisionHeight '${collisionHeight}'.`);
    ...
    if (this.collider)
      this.collider.setShape(
        new RAPIER.Cylinder(collisionHeight, collisionRadius),
      );
    if (this.rigidbody)
      this.rigidbody.setTranslation(
        tmpBodyPosition.set(
          this.position.x,
          this.position.y,
          this.position.z + collisionHeight,
        ),
        true,
      );
  }
    ...
    if (this.rigidbody)
      this.rigidbody.setTranslation(
        tmpBodyPosition.set(
          position.x,
          position.y,
          position.z + this.collisionHeight,
        ),
        true,
      );
  }
  ...
  public getPhysicsMode(): PhysicsMode_T {
  ...
  public setVelocity(value: Vector3Arr): void {
    this.velocity.fromArray(value);
  }
  public setAcceleration(value: Vector3Arr): void {
    this.acceleration.fromArray(value);
  }
  public setPhysicsMode(value: number): void {
    const modes: PhysicsMode_T[] = [
      "none",
      "walking",
      "falling",
      "swimming",
    ...
    if (!mode)
      throw new Error(`Unsupported UnrealScript physics mode '${value}'.`);
...
function twoWallAdjust(
  desiredDir: Vector3,
  delta: Vector3,
  hitNormal: Vector3,
  oldHitNormal: Vector3,
  hitTime: number,
) {
  if (oldHitNormal.dot(hitNormal) <= 0) {
    ...
    delta
      .addScaledVector(hitNormal, -delta.dot(hitNormal))
      .multiplyScalar(1 - hitTime);
