...
// Phase 2: the analytical ("ue") backend's per-primitive math. `pointPrimitive`/`queryPrimitive`
// return a pooled `PrimitiveHit_T | null` (the Phase 1 stubs declared `boolean` / `{time, normal}`);
// both call sites below only test truthiness and read `.time`/`.normal`, so the real signatures
// drop in unchanged.
...
export class CollisionWorld {
    ...
    // Phase 1 defaults to "rapier"; the donor project defaults to "ue", which returns once
    // collision-primitive.ts lands in Phase 2.
    public constructor(world: RAPIER.World, backend: CollisionBackend_T = "rapier") {
        this.world = world;
    ...
    protected rapierRayCheck(query: CollisionQuery_T, direction: Vector3, maxDistance: number): RayCheckResult_T | null {
        ...
        return this.makeRayResult(hit.timeOfImpact, tmpBodyPosition.copy(direction).multiplyScalar(hit.timeOfImpact).add(query.location), tmpNormal.copy(hit.normal as Vector3), hit.collider, this.colliderOwners.get(hit.collider.handle) || null);
    }
    ...
    protected rapierLineCheck(query: CollisionQuery_T, testDelta: Vector3, testDistance: number, requestedDistance: number, zeroExtent: boolean): CheckResult_T | null {
        ...
        const hit = this.world.castShape(query.location, identityRotation, testDelta, this.getShape(query.extent, zeroExtent), 0, 1, true, undefined, undefined, query.sourceCollider, query.sourceBody, this.rapierFilter);
        ...
        const travelled = Math.max(0, hit.time_of_impact * testDistance - getTraceBackoff(primitive, testDistance, zeroExtent));
