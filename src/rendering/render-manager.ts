...
import L2Environment, {
  ...
  interpolateFogInfoCloudColor,
  interpolateFogInfoHazeColors,
} from "@client/rendering/l2-env";
...
function findVisibleMaterialBinding(
  bindings: SectorMaterialBinding_T[],
  cacheVisibleMaterials: WeakMap<THREE.Mesh, Set<number>>,
): number {
  for (let i = bindings.length - 1; i >= 0; i--) {
    ...
    if (!visibleMaterials) {
      visibleMaterials = new Set(
        binding.object.geometry.groups.map((group) => group.materialIndex),
      );
...
function shouldUpdateOffscreenEmitter(emitter: any, currentTime: number) {
  if (!emitter.isOffscreenThrottled) {
    ...
    emitter.nextOffscreenUpdate =
      currentTime +
      OFFSCREEN_EMITTER_INTERVAL_MS +
      (getEmitterPhase(emitter) % OFFSCREEN_EMITTER_INTERVAL_MS);
    return true;
  }
  ...
  if (
    inactiveTimeout > 0 &&
    currentTime - emitter.offscreenSince > inactiveTimeout * 1000
  )
    return false;
  ...
  const missedIntervals =
    Math.floor(
      (currentTime - emitter.nextOffscreenUpdate) /
        OFFSCREEN_EMITTER_INTERVAL_MS,
    ) + 1;
  emitter.nextOffscreenUpdate +=
    missedIntervals * OFFSCREEN_EMITTER_INTERVAL_MS;

  return true;
}

function shouldUpdateVisibleEmitter(
  emitter: any,
  detailFrame: number,
  dropDetail: boolean,
...
class RenderManager implements IPhysicsHost {
  ...
  protected readonly lastMoverTriggerPosition = new Vector3(
    Infinity,
    Infinity,
    Infinity,
  );
  ...
  public constructor(
    ...
    this.displayGammaPass = new DisplayGammaPass(
      256,
      256,
      this.renderer.capabilities.isWebGL2 ? 4 : 0,
    );

    guiFolders.quality
      .add(this.envConfig, "fogPreset", {
        "1 (2k-8k)": "1",
        "2 (3k-10k)": "2",
        "3 (4k-12k)": "3",
        "4 (5k-14k)": "4",
    ...
    audioFolder
      .add(this.audioManager, "musicVolume", 0, 1, 0.01)
      .name("Music Volume");
    audioFolder
      .add(this.audioManager, "ambientVolume", 0, 1, 0.01)
      .name("Ambient Volume");
    audioFolder.open();
    ...
    this.controls.orbit = new OrbitControls(
      this.camera,
      this.renderer.domElement,
    );
    this.controls.fps = new ZUpPointerLockControls(
      this.camera,
      this.renderer.domElement,
    );
    ...
    this.controls.orbit.target.set(
      13298.353862721668,
      114463.56670278899,
      -3547.92988464792,
    );
    ...
    this.controls.fps.addEventListener(
      "lock",
      this.onPointerControlsLocked.bind(this),
    );
    this.controls.fps.addEventListener(
      "unlock",
      this.onPointerControlsUnlocked.bind(this),
    );
  ...
  public setEnv(env: EnvInfo): this {
    ...
    const timeState = {
      ...
      get timeScale() {
        return environment.getTimeScale();
      },
      set timeScale(v) {
    ...
    guiFolders.world.add(timeState, "time", 0, 24, 0.01).name("Time").listen();
    guiFolders.world
      .add(timeState, "timeScale", 0, 100, 0.01)
      .name("Time Scale");

    const envSignsSkyState = {
      get signsSky() {
        return environment.getActiveEnv();
      },
      set signsSky(v) {
    ...
    guiFolders.world
      .add(envSignsSkyState, "signsSky", { Normal: 0, Dusk: 1, Dawn: 2 })
      .name("Signs Sky");
  ...
  public onPointerControlsLocked() {
    ...
    Object.keys(this.dirKeys).forEach(
      (k: "up" | "left" | "right" | "down" | "shift") =>
        (this.dirKeys[k] = false),
    );
  }
  ...
  public onPointerControlsUnlocked() {
    ...
    this.controls.fps
      .getDirection(this.controls.orbit.target)
      .multiplyScalar(100)
  ...
  public toScreenSpaceCoords(point: Vector2) {
    ...
    return new Vector2((point.x / width) * 2 - 1, 1 - (point.y / height) * 2);
  }
  ...
  public onHandleKeyDown(event: KeyboardEvent) {
    ...
    if (event.key === "F3" || event.code === "F3") {
      ...
      if (currentSector && this.visualizer.isEnabled()) {
        const cameraPos =
          this.bspHelperActive && this.bspHelperCamera
            ? this.bspHelperCamera.position
            : this.camera.position;
        const cameraFrustum =
          this.bspHelperActive && this.bspHelperCamera
            ? new Frustum().setFromProjectionMatrix(
                new Matrix4().multiplyMatrices(
                  this.bspHelperCamera.projectionMatrix,
                  this.bspHelperCamera.matrixWorldInverse,
                ),
              )
            : this.frustum;
        ...
        if (this.visualizer.getMode() === VisualizerMode.Portals) {
          ...
          this.visualizer.updateFogs(
            currentSectorMap,
            this.activeFogId || undefined,
          );
        }
    ...
    if (event.key === "F5" || event.code === "F5") {
      ...
      if (
        currentSector &&
        this.visualizer.getMode() === VisualizerMode.Leaves &&
        this.visualizer.isEnabled()
      ) {
        const cameraPos =
          this.bspHelperActive && this.bspHelperCamera
            ? this.bspHelperCamera.position
    ...
    switch (event.key.toLowerCase()) {
      ...
      case "-":
        ...
        break;
      case "w":
        if (!this.isOrbitControls) this.dirKeys.up = true;
        break;
      case "a":
        if (!this.isOrbitControls) this.dirKeys.left = true;
        break;
      case "d":
        if (!this.isOrbitControls) this.dirKeys.right = true;
        break;
      case "s":
        if (!this.isOrbitControls) this.dirKeys.down = true;
        break;
      case "shift":
        if (!this.isOrbitControls) {
  ...
  public onHandleKeyUp(event: KeyboardEvent) {
    switch (event.key.toLowerCase()) {
      case "c":
        ...
        break;
      case "w":
        if (!this.isOrbitControls) this.dirKeys.up = false;
        break;
      case "a":
        if (!this.isOrbitControls) this.dirKeys.left = false;
        break;
      case "d":
        if (!this.isOrbitControls) this.dirKeys.right = false;
        break;
      case "s":
        if (!this.isOrbitControls) this.dirKeys.down = false;
        break;
      case "shift":
        if (!this.isOrbitControls) this.dirKeys.shift = false;
        break;
    }
  }
  ...
  public getSectorByCoords(
    ...
    if (!this.sectors.has(sectorX)) return null;

    const xsect = this.sectors.get(sectorX);

    if (!xsect.has(sectorY)) return null;
  ...
  protected scheduleMovableObject(
    mover: MovableObject,
    nextUpdate: number,
  ): void {
    ...
    this.sectors.forEach((row) =>
      row.forEach((sector) => {
        for (const pawn of sector.pawns.children) {
          ...
          pawn.visible =
            leafIndex !== null && containingSector.visibleLeaves.has(leafIndex);
        }
      }),
    );
  ...
  protected _updateObjects(currentTime: number, deltaTime: number) {
    ...
    const ambientSun =
      this.environment.getAmbientPlaneStaticMeshSunLightHalved(tmpColorByte);
    const sunColor =
      this.environment.getBaseColorPlaneStaticMeshSunLightScaled(
        tmpColorByte_2,
      );
    ...
    // camera-facing billboard basis, computed once and shared as a uniform (was per-particle in sprite-emitter.ts's onBeforeRender)
    {
      ...
      const projFront = tmpBillboardFront
        .set(0, 0, 1)
        .applyQuaternion(camera.quaternion)
        .normalize();
      const projRight = tmpBillboardRight
        .crossVectors(projFront, projUp)
        .normalize();
    ...
    const bspCullingCamera =
      this.bspHelperCamera && this.bspHelperActive
        ? this.bspHelperCamera
        : this.camera;
    ...
    this.frustum.setFromProjectionMatrix(
      new Matrix4().multiplyMatrices(
        bspCullingCamera.projectionMatrix,
        bspCullingCamera.matrixWorldInverse,
      ),
    );
    ...
    // THREE.js frustum plane indices: 0=right, 1=left, 2=bottom, 3=top, 4=far, 5=near
    {
      ...
      this.frustum.planes[4].setFromNormalAndCoplanarPoint(
        tmpCamDir.negate(),
        tmpFarPoint,
      );
    }
    ...
    const emitterCullDist =
      this.assetManager.userConfig.clippingRange.actor * CLIPPING_RANGE_SCALE;
    ...
    this.scene.traverse((object: THREE.Object3D) => {
      if ((object as any).isSectorObject) {
        ...
        if (
          isCameraInSector ||
          !wasVisible ||
          !(sector as any).visibilityCacheInitialized
        ) {
          sector.updateVisibility(
            this.environment,
            bspCullingPosition,
            this.frustum,
            this.frustumCullingEnabled,
    ...
    if (this.neighborVisibilitySectors.length > 0) {
      const index =
        this.neighborVisibilityCursor++ % this.neighborVisibilitySectors.length;
    ...
    this.scene.traverseVisible((child) => {
      if ((child as any).isUpdatable) {
        ...
        if (
          sector &&
          sector !== activeSector &&
          !(child as any).needsInitialLighting
        ) {
          ...
          if ((child as any).particlePool) {
            ...
            const isMaintenanceDue = shouldUpdateOffscreenEmitter(
              child,
              currentTime,
            );
            if (
              offscreenEmitterUpdates < offscreenEmitterUpdateLimit &&
              isMaintenanceDue
            ) {
              offscreenEmitterUpdates++;
              (child as any).updateMatrixWorld =
        ...
        if ((child as any).particlePool) {
          ...
          const isVisible =
            !!sector &&
            emitterUuid !== undefined &&
            sector.visibleEmitterUuids.has(emitterUuid);
        ...
        if ((child as any).particlePool) {
          ...
          if (pendingSounds.length) {
            for (const snd of pendingSounds)
              this.audioManager.playOneShotSound(
                snd.soundDataUri,
                snd.position,
                snd.volume,
                snd.pitch,
      ...
      if ((child as any).isSkinnedMesh && !(child as any).hasStartedAnimation) {
        const meshAnimations = (child as any).meshAnimations as Record<
          string,
          AnimationClip
        >;
        const clip =
          meshAnimations?.["Wait"] ?? Object.values(meshAnimations ?? {})[0];
  ...
  protected _updateEnvironment() {
    ...
    const presetIndex = parseInt(
      String(this.envConfig.fogPreset).split(" ")[0],
    );
    ...
    if (sector) {
      ...
      if (zone && zone.fog) {
        if (zone.isSunAffected) {
          ...
          targetFogColor.set(
            zone.fog.color.r * 255,
            zone.fog.color.g * 255,
            zone.fog.color.b * 255,
          );
        }
      }
      ...
      activeInfos.forEach(({ fogInfo, weight, hArr }) => {
        let range = (fogInfo as any)[`fogRange${presetIndex}`] as {
          A: number;
          B: number;
        };
        ...
        const skyColor = interpolateFogInfoSkyColor(
          timeOfDay,
          fogInfo.colors,
          tmpColorByte_3,
        );
        const hazeColor = interpolateFogInfoHazeColor(
          timeOfDay,
          fogInfo.colors,
          tmpColorByte_4,
        );
      ...
      if (totalHArrWeight > 0) {
        ...
        const haW =
          totalHArrWeight >= 0.95 ? 1.0 : Math.min(totalHArrWeight, 1.0);
      ...
      if (totalHazeWeight > 0) {
        const hW =
          totalHazeWeight >= 0.95 ? 1.0 : Math.min(totalHazeWeight, 1.0);
        tmpColorByte_4.set(
    ...
    const clearColorThree = new Color().setRGB(
      targetClearColor.r / 255,
      targetClearColor.g / 255,
      targetClearColor.b / 255,
    );
    this.renderer.setClearColor(clearColorThree);

    // Update Scene Fog (Always use targetFogColor for 3D fogging)
    const fogColorThree = new Color().setRGB(
      targetFogColor.r / 255,
      targetFogColor.g / 255,
      targetFogColor.b / 255,
    );
  ...
  protected _preRender(currentTime: number, deltaTime: number) {
    ...
    this.lastProjectionScreenMatrix.multiplyMatrices(
    ...
    if (!this.isOrbitControls) {
        ...
        (this.dirKeys.shift
          ? Math.min(
              ...
              Math.max(
                Math.pow(
                  2,
                  Math.log10((Date.now() - this.shiftTimeDown) * 0.25),
                ),
                2,
              ),
            )
      ...
      dirRight
        .set(1, 0, 0)
        .applyQuaternion(this.camera.quaternion)
        .multiplyScalar(-sidewaysVelocity);
    ...
    const musicInfo = activeSector
      ? activeSector.getMusicIdAt(this.camera.position)
      : { musicId: -1, isLooped: false, isForced: false };
    ...
    // Ambient sound spatial update, after UALAudioSubsystem::Update in the retail alaudio.dll
    {
      ...
      const candidates: {
        uuid: string;
        snd: GD.IAmbientSoundObjectDecodeInfo;
        priority: number;
      }[] = [];
      ...
      for (const [, sectorYMap] of this.sectors) {
        for (const [, sector] of sectorYMap) {
          ...
          for (const snd of sector.ambientSounds) {
            ...
            const priority =
              snd.volume * Math.min(Math.max(1 - distSq / maxDistSq, 0.01), 1);
            ...
            if (
              !snd.looping &&
              !this.audioManager.rollAmbientTrigger(
                snd.uuid,
                snd.soundDataUri,
                snd.randomChance,
                currentTime,
      ...
      if (
        !this._lastListenerPos.equals(camPos) ||
        !this._lastListenerQuat.equals(this.camera.quaternion)
      ) {
  ...
  protected _doRender(_currentTime: number, _deltaTime: number) {
    ...
    if (this.displayGammaEnabled)
      this.renderer.setRenderTarget(this.displayGammaPass.getTarget());
    ...
    if (this.visualizer.isEnabled()) {
      ...
      if (this.activeFogId) {
        ...
        if (fogInfo) {
          ...
          activeFogColors = {
            fog: interpolateFogInfoColor(
              timeOfDay,
              fogInfo.colors,
              new ColorByte(),
            ),
            sky: interpolateFogInfoSkyColor(
              timeOfDay,
              fogInfo.colors,
              new ColorByte(),
            ),
            cloud: interpolateFogInfoCloudColor(
              timeOfDay,
              fogInfo.colors,
              new ColorByte(),
              0,
            ),
            haze: interpolateFogInfoHazeColor(
              timeOfDay,
              fogInfo.colors,
              new ColorByte(),
            ),
      ...
      if (this.visualizer.getMode() === VisualizerMode.Fogs) {
        ...
        this.visualizer.updateAudioHUD(
          musicState,
          ambientSounds,
          _currentTime,
          this.camera.position,
  ...
  protected processSectorWarmups() {
    ...
    for (let i = 0; i < this.pendingSectorWarmups.length; i++) {
      if (
        findVisibleMaterialBinding(
          this.pendingSectorWarmups[i].materialQueue,
          cacheVisibleMaterials,
        ) < 0
      )
        continue;

      const distance = this.pendingSectorWarmups[
        i
      ].sector.worldBounds.distanceToPoint(this.camera.position);
    ...
    while (
      ...
      restoredMaterials < RenderManager.MATERIAL_RESTORES_PER_FRAME
    ) {
      const bindingIndex = visibleOnly
        ? findVisibleMaterialBinding(job.materialQueue, cacheVisibleMaterials)
        : job.materialQueue.length - 1;
      ...
      while (binding.textureQueue.length > 0) {
        ...
        if (binding.textureQueue.length > 0 && performance.now() >= deadline)
          return;
      }
      ...
      if (binding.materialIndex < 0) binding.object.material = binding.material;
        ...
        (binding.object.material as THREE.Material[])[binding.materialIndex] =
          binding.material;
    ...
    if (job.releaseEmitters) setEmitterWarmupGate(job.sector, true);

    (job.sector as any).visibilityCacheInitialized = false;
  }

  protected queueSectorWarmup(
    sector: SectorObject,
    root: THREE.Object3D,
    releaseEmitters: boolean,
  ) {
  ...
  protected processShaderDiagnostics() {
    if (this.parallelShaderCompileExt === undefined)
      this.parallelShaderCompileExt =
        this.renderer.extensions.get("KHR_parallel_shader_compile") ?? null;
    ...
    for (let i = this.pendingShaderChecks.length - 1; i >= 0; i--) {
      ...
      if (!gl.getProgramParameter(program.program, ext.COMPLETION_STATUS_KHR))
        continue; // still compiling, retry next frame
  ...
  public removeSector(sector: SectorObject) {
    if (sector.index) this.sectors.get(sector.index.x)?.delete(sector.index.y);
  ...
  public toggleBSPHelperCamera() {
    if (!this.bspHelperCamera) {
      ...
      this.bspHelperCamera = new PerspectiveCamera(
        this.camera.fov,
        this.camera.aspect,
        0.1,
        DEFAULT_FAR,
...
function addResizeListeners(manager: RenderManager) {
  global.addEventListener(
    "resize",
    (manager as any).onHandleResize.bind(manager),
  );
...
function disposeSectorResources(sector: SectorObject) {
  ...
  for (const celestial of sector.celestials) {
    ...
    const materials = Array.isArray(celestial.material)
      ? celestial.material
      : [celestial.material];
    for (const material of materials) {
      if (!material) continue;
...
function collectTexturesDeep(
  ...
  for (const nested of Object.values(value))
    collectTexturesDeep(nested, textures, seen);
}
...
function collectMaterialTextures(
  ...
  if ((material as any).uniforms)
    collectTexturesDeep((material as any).uniforms, textures, seen);
  if ((material as any).sprites)
    collectTexturesDeep((material as any).sprites, textures, seen);
}

function stageSectorWarmup(
  sector: SectorObject,
  root: THREE.Object3D,
  releaseEmitters: boolean,
): SectorWarmup_T {
  ...
  root.traverse((child) => {
    ...
    const fallback = materials.map((material, materialIndex) => {
      ...
      if (!fallbackMaterial) {
        fallbackMaterial = new MeshBasicMaterial({
          color: 0x777777,
          side: material.side,
          transparent: material.transparent,
          opacity: material.transparent ? 0.5 : 1,
    ...
    if (Array.isArray(mesh.material)) {
      for (let i = 0; i < materials.length; i++)
        if (fallback[i] !== materials[i])
          materialQueue.push({
            ...
            materialIndex: i,
            textureQueue: materialTextures[i],
          });
      ...
      materialQueue.push({
        ...
        materialIndex: -1,
        textureQueue: materialTextures[0],
      });
    }
  ...
  return {
    sector,
    materialQueue,
    warmedTextures: new Set(),
    lightingQueue,
    fallbackMaterials: new Set(fallbackCache.values()),
    releaseEmitters,
  };
}
