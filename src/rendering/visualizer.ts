...
class Visualizer {
  ...
  private readonly hudColors: Map<
    string,
    { swatch: HTMLElement; rgbDisplay: HTMLElement; alpha: HTMLElement }
  > = new Map();
  private readonly audioLines: Map<string, HTMLElement> = new Map();
  private readonly emitterLines: Map<string, HTMLElement> = new Map();
  private readonly emitterGroups: Map<
    string,
    { wrapper: HTMLElement; header: HTMLElement; body: HTMLElement }
  > = new Map(); // one wrapper per parent "Emitter" actor
  ...
  public setEmitterVisibilityHandlers(
    onToggle: (uuid: string, visible: boolean) => void,
  ...
  private createHudRow(
    ...
    this.hudColors.set(`${prefix}:${name}`, {
      swatch,
      rgbDisplay,
      alpha,
    } as any);
  ...
  private initHUD(): HTMLElement {
    ...
    ["Fog", "Sky", "Cloud", "Haze"].forEach((name) =>
      this.createHudRow(fogPanel, name, "fog"),
    );
    hud.appendChild(fogPanel);

    const globalPanel = createPanel("GLOBAL ENVIRONMENT");
    ["Fog", "Sky", "Cloud 1", "Cloud 2", "Cloud 3", "Sun", "Haze"].forEach(
      (name) => this.createHudRow(globalPanel, name, "global"),
    );
  ...
  private initEmittersHUD(): HTMLElement {
    ...
    this.emitterMasterCheckbox.onchange = () =>
      this.onEmitterToggleAll?.(this.emitterMasterCheckbox!.checked);
  ...
  public updateEmitters(emitters: EmitterDebugInfo[]): void {
    ...
    if (this.emitterListElement) {
      ...
      for (const groupId of orderedGroupIds) {
        ...
        const expectedNext = prevGroupWrapper
          ? prevGroupWrapper.nextElementSibling
          : this.emitterListElement.firstElementChild;
        ...
        members.forEach((e) => {
          ...
          if (!line) {
            ...
            checkbox.onchange = () =>
              this.onEmitterToggle?.(e.uuid, checkbox!.checked);
          ...
          const expectedNextLine = prevLine
            ? prevLine.nextElementSibling
            : group!.body.firstElementChild;
    ...
    for (const e of emitters) {
      ...
      if (!sprite) {
        sprite = this.makeTextSprite(
          this.shortEmitterName(e.name),
          e.isVisible ? "#0f0" : "#f80",
        );
  ...
  public updateAudioHUD(
    ...
    if (this.ambientListElement) {
      ...
      sorted.forEach((s) => {
        ...
        const baseVolText = Math.round(baseVol * 100)
          .toString()
          .padStart(3, " ");
        const actualVolText = Math.round(actualVol * 100)
          .toString()
          .padStart(3, " ");
  ...
  public updateHUD(
    activeFogColors?: FogSourceColors,
    globalEnvColors?: GlobalEnvColors,
    zoneFogData?: ZoneFogData,
  ): void {
    ...
    if (zoneFogData) {
      ...
      if (startEntry) {
        startEntry.rgbDisplay.innerText = Math.round(zoneFogData.start)
          .toString()
          .padStart(11, " ");
      ...
      if (endEntry) {
        endEntry.rgbDisplay.innerText = Math.round(zoneFogData.end)
          .toString()
          .padStart(11, " ");
      ...
      if (zoneEntry) {
        zoneEntry.rgbDisplay.innerText = zoneFogData.isFogZone
          ? "        YES"
          : "         NO";
        zoneEntry.alpha.innerText = "";
        zoneEntry.swatch.style.backgroundColor = zoneFogData.isFogZone
          ? "#0f0"
          : "#f00";
      }
    } else {
  ...
  private static readonly VISUALIZATION_MODES = Object.values(
    VisualizerMode,
  ).filter((mode) => mode !== VisualizerMode.None);
  ...
  private updateVisibility(): void {
    ...
    const isEmittersMode =
      this.enabled && this.mode === VisualizerMode.Emitters;
  ...
  public updatePortals(
    ...
    for (const [, sectorYMap] of sectors) {
      for (const [, sector] of sectorYMap) {
        ...
        for (
          let nodeIndex = 0;
          nodeIndex < sector.bspNodes.length;
          nodeIndex++
        ) {
          const node = sector.bspNodes[nodeIndex];
          const hasPortalFlag =
            node.surfFlags !== undefined && (node.surfFlags & PF_Portal) !== 0;

          if (
            hasPortalFlag &&
            node.zones[0] >= 0 &&
            node.zones[1] >= 0 &&
            node.zones[0] !== node.zones[1]
          ) {
            let isVisible = true;
            if (activeZoneMask !== null) {
              ...
              isVisible =
                !!(activeZoneMask & zone0Mask) ||
                !!(activeZoneMask & zone1Mask);
            }

            const portalColor = isVisible
              ? visiblePortalColor
              : hiddenPortalColor;
            ...
            const boxMaterial = Array.isArray(boxHelper.material)
              ? boxHelper.material[0]
              : boxHelper.material;
            if (boxMaterial) {
            ...
            if (arrow.line) {
              const lineMaterial = Array.isArray(arrow.line.material)
                ? arrow.line.material[0]
                : arrow.line.material;
              if (lineMaterial) {
            ...
            if (arrow.cone) {
              const coneMaterial = Array.isArray(arrow.cone.material)
                ? arrow.cone.material[0]
                : arrow.cone.material;
              if (coneMaterial) {
  ...
  private clearPortalVisualizations(): void {
    this.portalVisualizations.forEach((viz) => {
      ...
      if (viz instanceof Mesh) {
        ...
        const material = Array.isArray(viz.material)
          ? viz.material[0]
          : viz.material;
  ...
  public updateZones(
    ...
    for (const [, sectorYMap] of sectors) {
      for (const [, sector] of sectorYMap) {
        ...
        for (
          let leafIndex = 0;
          leafIndex < sector.bspLeaves.length;
          leafIndex++
        ) {
          ...
          if (leaf && leaf.zone >= 0 && leaf.zone < 64) {
            for (
              ...
              const node = sector.bspNodes[nodeIndex];
              if (
                node.leaves[0] === leafIndex ||
                node.leaves[1] === leafIndex
              ) {
                ...
                if (!bounds) {
                  const sphere =
                    node.exclusiveSphereBound.radius > 0
                      ? node.exclusiveSphereBound
                      : node.inclusiveSphereBound;
                  if (
                    sphere.radius > 0 &&
                    !isNaN(sphere.radius) &&
                    sphere.center
                ...
                if (bounds) {
                  ...
                  zoneNodeCounts.set(
                    leaf.zone,
                    (zoneNodeCounts.get(leaf.zone) || 0) + 1,
                  );
                }
                break;
              }
        ...
        for (let zoneIndex = 0; zoneIndex < 64; zoneIndex++) {
          ...
          for (
            ...
            if (zoneData.connectivity & zoneBit) {
              ...
              const connectivityColor = isSourceActive
                ? activeConnectivityColor
                : inactiveConnectivityColor;
  ...
  private calculateNodeDepths(bspNodes: any[]): Map<number, number> {
    ...
    const queue: { nodeIndex: number; depth: number }[] = [
      { nodeIndex: 0, depth: 0 },
    ];
    depths.set(0, 0);
  ...
  private darkenColorByDepth(
    ...
    const darknessFactor =
      1.0 - (1.0 - maxDarkness) * (depth / Math.max(maxDepth, 1));
  ...
  public updateLeaves(
    ...
    for (const [, sectorYMap] of sectors) {
      for (const [, sector] of sectorYMap) {
        ...
        const maxDepth =
          nodeDepths.size > 0
            ? Math.max(...Array.from(nodeDepths.values()))
            : 0;

        const initialZoneMask = sector.getActiveZoneMask(cameraPosition);
        const frustum = cameraFrustum || new Frustum();

        const { visibleLeaves, zonesAddedThroughPortals } = sector.traverseBSP(
          cameraPosition,
          initialZoneMask,
          frustum,
        ...
        const effectiveLeafDetail =
          this.leafDetail === LeafVisualizerDetail.Auto
            ? visibleLeaves.size >= this.leafAutoAggregateThreshold
        ...
        const leafToNodeMap = new Map<
          number,
          { nodeIndex: number; depth: number }
        >();
        ...
        const leafBoxes = new Map<
          number,
          { box: Box3; zone: number; depth: number; isPortalLeaf: boolean }
        >();
        ...
        for (const [leafIndex, { nodeIndex, depth }] of leafToNodeMap) {
          ...
          if (!box) {
            ...
            if (
              exclusiveSphere.radius > 0 &&
              !isNaN(exclusiveSphere.radius) &&
              exclusiveSphere.center &&
              !isNaN(exclusiveSphere.center.x) &&
              !isNaN(exclusiveSphere.center.y) &&
              !isNaN(exclusiveSphere.center.z)
            ) {
              box = new Box3();
              ...
              if (
                inclusiveSphere.radius > 0 &&
                !isNaN(inclusiveSphere.radius) &&
                inclusiveSphere.center &&
                !isNaN(inclusiveSphere.center.x) &&
                !isNaN(inclusiveSphere.center.y) &&
                !isNaN(inclusiveSphere.center.z)
              ) {
                box = new Box3();
        ...
        if (effectiveLeafDetail === LeafVisualizerDetail.PerLeaf) {
          for (const [leafIndex, info] of leafBoxes) {
            const baseColor = info.isPortalLeaf
              ? portalLeafColor
              : directLeafColor;
            const leafColor = this.darkenColorByDepth(
              baseColor,
              info.depth,
              maxDepth,
            );
          ...
          const zoneUnion = new Map<
            number,
            {
              box: Box3;
              minDepth: number;
              anyPortalLeaf: boolean;
              leafCount: number;
            }
          >();
          ...
          for (const [, info] of leafBoxes) {
            ...
            if (!existing) {
              ...
              existing.anyPortalLeaf =
                existing.anyPortalLeaf || info.isPortalLeaf;
              existing.leafCount++;
            }
          }

          for (const [zone, agg] of zoneUnion) {
            const baseColor = agg.anyPortalLeaf
              ? portalLeafColor
              : directLeafColor;
            const zoneColor = this.darkenColorByDepth(
              baseColor,
              agg.minDepth,
              maxDepth,
            );
