...
const replaceBytes = new Uint8Array(
  "OggS".split("").map((x) => x.charCodeAt(0)),
);
const MAX_AUDIOCHANNELS = 32,
  ROLLOFF = 0.5; // hardcoded from l2.ini
...
class AudioManager {
  ...
  public async playMusic(
    ...
    if (
      ...
      (this.currentSource || this.nextMusicTrackTime !== undefined)
    ) {
    ...
    const wasPlaying =
      !!this.currentSource ||
      (this.fadeEndTime !== undefined && time < this.fadeEndTime);
  ...
  protected async stopMusicInternal(
    _incrementPlayId: boolean,
    currentTime: number,
  ) {
  ...
  public async update(currentTime: number) {
    ...
    if (
      this.nextMusicTrackTime !== undefined &&
      currentTime >= this.nextMusicTrackTime
    ) {
      ...
      if (
        ...
        (playId === undefined || this.currentPlayId === playId)
      ) {
  ...
  protected async fetchRandomBuffer(
    index: number,
  ...
  protected readonly pendingAmbientBuffers = new Map<
    string,
    Promise<AudioBuffer | null>
  >();
  ...
  public rollAmbientTrigger(
    ...
    if (
      Math.floor((currentTime - anchor) / slotMs) <=
      Math.floor((this.prevTime - anchor) / slotMs)
    )
      return false;
  ...
  public playAmbientSound(
    ...
    if (
      this.activeAmbientSounds.size >= MAX_AUDIOCHANNELS &&
      !this.stealAmbientChannel(priority)
    )
      return false;
  ...
  protected async loadAmbientBuffer(
    dataUri: string,
  ): Promise<AudioBuffer | null> {
    const cached = this.ambientBufferCache.get(dataUri);
    if (cached) return cached;

    if (this.pendingAmbientBuffers.has(dataUri))
      return this.pendingAmbientBuffers.get(dataUri)!;
  ...
  protected startAmbientSource(
    id: string,
    buffer: AudioBuffer,
    entry: AmbientChannel_T,
  ) {
    ...
    if (!info.looping)
      source.onended = () => {
        if (
          this.activeAmbientSounds.get(id) !== entry ||
          entry.source !== source
        )
          return;
  ...
  public getMusicState() {
    return {
      ...
      isFading:
        this.fadeEndTime !== undefined && this.lastTime < this.fadeEndTime,
