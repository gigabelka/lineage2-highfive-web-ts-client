...
async function report(data: SectorReport): Promise<void> {
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      ...
      console.warn(
        `[sector-test] report rejected (${response.status}), retrying`,
      );
    } catch (e) {
...
class SectorRenderTester {
  ...
  protected measureLuminance(sector: SectorObject): {
    lum: number;
    coverage: number;
  } {
    ...
    return {
      lum: count ? Math.round(sum / count) : -1,
      coverage: Math.round((count / (width * height)) * 100),
    };
  }
  ...
  protected forceRender(sector: SectorObject): void {
    ...
    sector.traverse((object) => {
      ...
      if (
        !renderable.isMesh &&
        !(renderable as any).isPoints &&
        !(renderable as any).isSprite &&
        !(renderable as any).isLine
      )
        return;
      ...
      for (const material of materials) {
        if (
          !material ||
          !(material as any).isMaterial ||
          materialStates.has(material)
        )
          continue;
    ...
    try {
      ...
      for (const [material, visible] of materialStates)
  ...
  public run(sector: SectorObject): {
    lumBefore: number;
    lumAfter: number;
    coverage: number;
    colBefore: number;
    ...
    try {
      ...
      const bounds = sector.worldBounds.isEmpty()
        ? new Box3().setFromObject(sector)
        : sector.worldBounds;
      ...
      try {
        ...
        for (const [object, material] of stashedMaterials)
      ...
      particleBatcher.root.traverse((child) =>
    ...
    sector.traverse((child) => {
      ...
      for (const m of Array.isArray(material) ? material : [material]) {
        for (const uniform of Object.values(
          ...
          (uniform as any)?.value?.isTexture &&
            (uniform as any).value.dispose();
...
async function runSectorTest(): Promise<void> {
  await RAPIER.init(); // rapier3d-compat loads its wasm lazily - must resolve before RenderManager builds a RAPIER.World
  ...
  if (start === 0)
    await report({
      note: `sweep started: ${sectors.length} sectors, emitters=${loadEmitters}, cache=${cacheEnabled}, render=${renderEnabled}, forceRender=${forceRenderObjects}`,
    });
  ...
  if (renderEnabled) {
    try {
      ...
      await report({
        note: `render tester unavailable (${(e as Error)?.message}), falling back to decode-only`,
      });
      renderTester = null;
    }
  }
  ...
  const hasS3TC = !!renderTester?.renderer.extensions.get(
  ...
  await report({
    note: `textures: mode=${textureMode}, s3tc=${hasS3TC} -> ${preferCompressed ? "compressed" : "rgba"}`,
  });
  ...
  for (let i = start; i < sectors.length; i++) {
    ...
    try {
      const library = await withTimeout(
        client.decodeSector(sector, loadSettings),
        SECTOR_TIMEOUT_MS,
        `decode '${sector}'`,
      );
      ...
      const bsp: [number, number, number] = [
        library.bspNodes.length,
        library.bspSections.length,
        library.bspZones.length,
      ];
      ...
      if (client.isDead || /timed out/.test(error?.message ?? "")) {
        await report({
          note: `worker lost at '${sector}', restarting sweep at index ${i + 1}`,
        });
        continueFrom(i + 1);
        return;
      }
