...
class Terrain extends Mesh implements ICollidable {
  ...
  constructor(
    ...
    fieldInfo?: TerrainFieldInfo_T,
    lightingInfo?: TerrainLightingInfo,
  ) {
  ...
  public setTerrainField({
    ...
    this.colliderDesc = ColliderDesc.heightfield(vx, vz, heightfield, {
      x: this.boundsSize.x,
      y: 1,
      z: this.boundsSize.z,
    });
  ...
  public update(sector: SectorObject, env: L2Environment) {
    ...
    if (this.useShadowLerp && this.lightingInfo.shadowMapTimes.length > 0) {
      [shadowIndex, shadowNextIndex, alpha] =
        this.getShadowMapIndicesAndLerp(timeOfDay);
    } else {
    ...
    let staticCacheDirty =
      !this.staticLightingCache || shadowIndex !== this.lastShadowIndex;

    if (this.useShadowLerp && this.staticLightingCache) {
      const colorsChanged =
        cbAmbient.r !== this.lastAmbientR ||
        Math.abs(cbAmbient.g - this.lastAmbientG) > 0 ||
        Math.abs(cbAmbient.b - this.lastAmbientB) > 0 ||
        cbLight.r !== this.lastSunR ||
        Math.abs(cbLight.g - this.lastSunG) > 0 ||
        Math.abs(cbLight.b - this.lastSunB) > 0;
      const alphaChanged =
        Math.abs(alpha - this.lastAlpha) >= 0.02 ||
        shadowNextIndex !== this.lastShadowNextIndex;
    ...
    const lights = this.lightingInfo.lights.map((l) => ({
      ...l,
    ...
    if (staticCacheDirty) {
      if (
        ...
        this.staticLightingCache.length !== vertexCount * 3
      ) {
      ...
      // 2. Add Static Lights
      const staticLights = lights.filter(
        (l) =>
          l.instance &&
          !l.instance.isDynamic &&
          (!l.instance.isTimeBased || l.instance.lightMethod === "Sunlight"),
      );
    ...
    const dynamicLights = lights.filter(
      (l) =>
        l.instance &&
        (l.instance.isDynamic ||
          (l.instance.isTimeBased && l.instance.lightMethod !== "Sunlight")),
  ...
  protected computeLighting(
    lights: { flags: Uint8Array; instance?: DynamicLight }[],
  ...
  protected getShadowMapIndicesAndLerp(
    timeOfDay: number,
  ...
  public stitchWestToEast(neighbor: Terrain) {
    ...
    const nAttr = (neighbor.batchGeometry || neighbor.geometry).getAttribute(
      "position",
    );
