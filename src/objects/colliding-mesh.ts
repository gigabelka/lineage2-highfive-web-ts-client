...
// UModel hull flags carry the "use the back side of this node's plane" bit in 0x40000000
const HULL_FLIP = 0x40000000;
// Below this many collider indices a linear triangle scan beats building/keeping a grid.
const TRIANGLE_INDEX_MIN_INDICES = 384;

export type CollidingMeshProps_T = {
    ...
    collision?: ActorCollisionProfile_T,
    staticMeshCollision?: GD.IStaticMeshCollisionDecodeInfo,
    collisionIndex?: CollisionTriangleIndex_T
};
...
class CollidingMesh extends LitActorMesh implements ICollidable {
    ...
    public constructor(props: CollidingMeshProps_T) {
        super(props);
    ...
    public makeCollider(indices: Uint32Array, vertices: Float32Array) {
        ...
        if (this.collisionProfile?.useCylinderCollision) {
            ...
            this.analyticalPrimitive = {
                ...
                index: this.collisionIndex || (this.colliderIndices.length < TRIANGLE_INDEX_MIN_INDICES ? null : buildTriangleIndex(this.colliderVertices, this.colliderIndices)),
    ...
    public createCollider(physicsWorld: World): Collider {
        ...
        // The trimesh desc is built from unscaled local vertices; bake a non-unit world scale into
        // a private copy (and flip winding on a mirroring scale) so Rapier matches the render mesh.
...
/**
 * Rebuilds a UStaticMesh's `collisionModel` (a UModel) into plane-set convex hulls.
 * Each hull flag indexes the model's node plane table; 0x40000000 means "negated plane".
 */
    ...
    for (const collision of model.hulls) {
        ...
        hulls.push({
            planes: collision.flags.map(flag => {
                ...
                return [plane[0] * scale, plane[1] * scale, plane[2] * scale, plane[3] * scale, nodeIndex] as [number, number, number, number, number];
            }),
