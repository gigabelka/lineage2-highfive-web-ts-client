...
const tmpVelocityLossRange: Range3_T = {
  min: new Vector3(),
  max: new Vector3(),
};
...
abstract class BaseEmitter extends Object3D {
  ...
  protected initialSettings: {
    particlesPerSecond: number;
    position: { min: THREE.Vector3; max: THREE.Vector3 };
    offset: THREE.Vector3;
    scale: { min: THREE.Vector3; max: THREE.Vector3 };
    velocity: { min: THREE.Vector3; max: THREE.Vector3 };
    angularVelocity: { min: THREE.Vector3; max: THREE.Vector3 };
  };
  ...
  protected revolutionScale: {
    relativeTime: number;
    relativeRevolution: THREE.Vector3;
  }[];
  ...
  constructor(config: GD.EmitterConfig_T) {
    ...
    this.fadingSettings = {
      fadeIn: config.fadeIn
        ? {
            time: config.fadeIn.time * 1000,
            color: new Vector4().fromArray(config.fadeIn.color || [1, 1, 1, 1]),
          }
        : null,
      fadeOut: config.fadeOut
        ? {
            time: config.fadeOut.time * 1000,
            color: new Vector4().fromArray(
              config.fadeOut.color || [1, 1, 1, 1],
            ),
          }
        : null,
    };
    ...
    this.generalSettings = {
      colorMultiplierRange: {
        min: new Vector3().fromArray(
          config.colorMultiplierRange?.min ?? [1, 1, 1],
        ),
        max: new Vector3().fromArray(
          config.colorMultiplierRange?.max ?? [1, 1, 1],
        ),
      },
      ...
      lifetime: {
        min: config.lifetime[0] * 1000,
        max: config.lifetime[1] * 1000,
      },
    ...
    this.sizeScale = (config.changesOverLifetime.scale?.values ?? []).map(
      ([relTime, relSize]) => ({ relTime, relSize }),
    );
    this.velocityScale = (
      (config.changesOverLifetime as any).velocity?.values ?? []
    ).map(([relTime, v]: [number, [number, number, number]]) => ({
      relativeTime: relTime,
      relativeVelocity: new Vector3().fromArray(v),
    }));
    this.colorScale = (
      (config.changesOverLifetime as any).color?.values ?? []
    ).map(([relTime, c]: [number, number[]]) => ({
      relativeTime: relTime,
      color: new Vector4().fromArray(c.map((v) => v / 255)),
    }));
    ...
    this.initialSettings = {
      ...
      angularVelocity: {
        min: new Vector3().fromArray(
          config.initial.angularVelocity?.min || [1, 1, 1],
        ),
        max: new Vector3().fromArray(
          config.initial.angularVelocity?.max || [1, 1, 1],
        ),
      },
    };
    ...
    const vec3 = (v: any) =>
      Array.isArray(v) ? new Vector3().fromArray(v) : v;
    const vec4 = (v: any) =>
      Array.isArray(v) ? new Vector4().fromArray(v) : v;
    const range = (v: any) => (Array.isArray(v) ? { min: v[0], max: v[1] } : v);
    const rangeVec3 = (v: any) =>
      v && Array.isArray(v.min)
        ? {
            min: new Vector3().fromArray(v.min),
            max: new Vector3().fromArray(v.max),
          }
        : v;
    ...
    this.scaleSizeByVelocityMultiplier =
      vec3(this.scaleSizeByVelocityMultiplier) ?? new Vector3(1, 1, 1);
    this.spawningSoundIndex = range(this.spawningSoundIndex) ?? {
      min: 0,
      max: 0,
    };
    this.spawningSoundProbability = range(this.spawningSoundProbability) ?? {
      min: 1,
      max: 1,
    };
    ...
    this.acceleration = new Vector3().fromArray(
      config.acceleration ?? [0, 0, 0],
    );
    ...
    this.startMassRange = {
      min: config.startMassRange?.[0] ?? 0,
      max: config.startMassRange?.[1] ?? 0,
    };
    this.initialTimeRange = {
      min: config.initialTimeRange?.[0] ?? 0,
      max: config.initialTimeRange?.[1] ?? 0,
    };
    this.lifetimeRange = {
      min: config.lifetime?.[0] ?? 0,
      max: config.lifetime?.[1] ?? 0,
    };
    ...
    this.initialTimeRange = config.initialTimeRange
      ? { min: config.initialTimeRange[0], max: config.initialTimeRange[1] }
      : { min: 0, max: 0 };
    ...
    this.velocityLossRange = config.velocityLossRange
      ...
      : { min: new Vector3(), max: new Vector3() };
    this.sphereRadiusRange = config.sphereRadiusRange
      ? { min: config.sphereRadiusRange[0], max: config.sphereRadiusRange[1] }
      : { min: 0, max: 0 };
    this.startSpinRange = {
      min: new Vector3().fromArray(
        config.initial.angularVelocity?.min || [0, 0, 0],
      ),
      max: new Vector3().fromArray(
        config.initial.angularVelocity?.max || [0, 0, 0],
      ),
    };
    ...
    const poolSize = this.forcedMaxParticles
      ? this.maxParticles
      : this.maxParticles * 2;
  ...
  protected finishConstruction(config: GD.EmitterConfig_T): void {
    ...
    if (this.instancedMesh) {
      ...
      for (let i = 0; i < poolSize; i++) {
        const particle = (this.particlePool[i] = Particle.init(
          this,
  ...
  public warmUp(relativeTime: number, ticksPerSecond: number) {
    ...
    const primeTime =
      ((this.lifetimeRange.min + this.lifetimeRange.max) / 2) * relativeTime;
  ...
  protected spawnParticle(
    ...
    flags: number = 0,
    spawnFlags: number = 0,
    localLocationOffset = ZERO_VECTOR3,
  ) {
    //
    ...
    randVector(
      particle.velocity,
      this.initialSettings.velocity.min,
      this.initialSettings.velocity.max,
    );
    ...
    if (applyAll || this.startLocationShape === "box")
      (randVector(
        this.tmpVec,
        this.initialSettings.position.min,
        this.initialSettings.position.max,
      ),
    ...
    if (applyAll || this.startLocationShape === "sphere")
      particle.position.add(
        this.tmpVec
          .randomDirection()
          .multiplyScalar(
            randRange(this.sphereRadiusRange.min, this.sphereRadiusRange.max),
          ),
      );
    if (applyAll || this.startLocationShape === "polar") {
      ...
      randVector(
        polar,
        this.startLocationPolarRange.min,
        this.startLocationPolarRange.max,
      );
    ...
    if (this.meshSpawning !== "none" && this.meshSpawningStaticMesh) {
      ...
      if (maxIndex > 0) {
        let vertexIndex =
          this.meshSpawning === "linear"
            ? this.currentMeshSpawningIndex++
            : Math.trunc(Math.random() * maxIndex);
        ...
        if (this.isSpawningTowardsNormal) {
          ...
          if (
            normal.dot(this.realMeshNormal) <
            1 -
              2 *
                randRange(
                  this.meshNormalThreshold.min,
                  this.meshNormalThreshold.max,
                )
          ) {
        ...
        particle.position.add(
          this.uniformMeshScale
            ? location.multiplyScalar(locationScale.x)
            : location.multiply(locationScale),
        );
        ...
        if (this.isVelocityFromMesh) {
          ...
          particle.velocity.add(
            this.uniformVelocityScale
              ? meshVelocity.multiplyScalar(velocityScale.x)
              : meshVelocity.multiply(velocityScale),
          );
        }
    ...
    if (
      this.useSkeletalLocationAs &&
      this.useSkeletalLocationAs !== "none" &&
      this.meshVertsAndNormals &&
      this.meshVertsAndNormals.length > 0 &&
      this.relativeBoneIndexRange &&
      this.skeletalScale
    ) {
      ...
      const rangeMin = Array.isArray(this.relativeBoneIndexRange)
        ? this.relativeBoneIndexRange[0]
        : 0;
      const rangeMax = Array.isArray(this.relativeBoneIndexRange)
        ? this.relativeBoneIndexRange[1]
        : 1;
      ...
      if (vertexIndex < this.meshVertsAndNormals.length) {
        const scaleVec = Array.isArray(this.skeletalScale)
          ...
          : this.skeletalScale;
        particle.oldMeshLocation.copy(
          this.meshVertsAndNormals[vertexIndex].clone().multiply(scaleVec),
        );
    ...
    switch (this.rotationSource) {
      case "actor":
        ...
        tmpParticleQuaternion
          .copy(owner.quaternion)
    ...
    randVector(
      particle.revolutionCenter,
      this.revolutionCenterOffsetRange.min,
      this.revolutionCenterOffsetRange.max,
    );
    randVector(
      particle.revolutionsPerSecond,
      this.revolutionsPerSecondRange.min,
      this.revolutionsPerSecondRange.max,
    );
    ...
    randVector(
      particle.colorMultiplier,
      this.colorMultiplierRange.min,
      this.colorMultiplierRange.max,
    );
    particle.maxLifetime = randRange(
      this.lifetimeRange.min,
      this.lifetimeRange.max,
    );
    particle.time =
      spawnTime +
      randRange(this.initialTimeRange.min, this.initialTimeRange.max);
    particle.hitCount = 0;
    particle.flags = EParticleFlags_T.PTF_Active | flags;
    particle.mass = randRange(this.startMassRange.min, this.startMassRange.max);
    randVector(
      particle.startSize,
      this.initialSettings.scale.min,
      this.initialSettings.scale.max,
    );
    ...
    particle.velocity.add(
      currentAccel.multiplyScalar(spawnRangeScale * spawnTime),
    );
    ...
    if (this.addVelocityFromOwner && this.coordinateSystem !== "relative")
      __break__() &&
        particle.velocity.add(
          randVector(
            this.tmpVec,
            this.addVelocityMultiplierRange.min,
    ...
    if (this.coordinateSystem === "independent" && spawnTime > 0) {
      const ownerVelocity = tmpPhysicsVector
        .copy(ownerLocation())
        .sub(this.oldOwnerLocation)
        .divideScalar(clamp(this.lastDeltaTime, 0.001, 1.0));
      particle.position.sub(ownerVelocity.multiplyScalar(spawnTime));
    }
    particle.position.add(
      tmpPhysicsVector.copy(particle.velocity).multiplyScalar(spawnTime),
    );

    // Scale size by velocity.
    if (
      this.scaleSizeXByVelocity ||
    ...
    if (particle.mass) particle.mass = 1 / particle.mass;

    randVector(
      particle.startSpin,
      this.startSpinRange.min,
      this.startSpinRange.max,
    );
    randVector(
      particle.spinsPerSecond,
      this.spinsPerSecondRange.min,
      this.spinsPerSecondRange.max,
    );
    ...
    if (this.isUsingRandomSubdiv) {
      if (this.subdivEnd)
        particle.subdivision = Math.trunc(
          (this.subdivEnd - this.subdivStart) * Math.random() +
            this.subdivStart,
        );
      else
        particle.subdivision = Math.trunc(
          Math.random() * this.texSubdivU * this.texSubdivV,
        );
    } else particle.subdivision = -1;
    ...
    if (this.spawningSound !== "none" && this.sounds.length) {
      ...
      soundIndex %= Math.trunc(
        this.spawningSoundIndex.max - this.spawningSoundIndex.min
          ? this.spawningSoundIndex.max - this.spawningSoundIndex.min + 1
          : this.sounds.length,
      );
      ...
      if (
        Math.random() <=
        randRange(sound.probability[0], sound.probability[1]) *
          randRange(
            this.spawningSoundProbability.min,
            this.spawningSoundProbability.max,
          )
      ) {
  ...
  protected spawnParticles(
    ...
    const shouldInterpolate =
      this.oldOwnerLocation.distanceToSquared(owner.position) > 1;

    for (let i = 0; i < spawnCount; i++) {
      this.spawnParticle(
        this.particleIndex,
        startTime - i * increment,
        EParticleFlags_T.PTF_InitialSpawn,
      );

      // Laurent -- location interpolation
      if (shouldInterpolate && this.coordinateSystem === "independent") {
        percent = 1 - (i + 1) / spawnCount;
        this.particles[this.particleIndex].position.add(
          tmpPhysicsVector
            .copy(this.oldOwnerLocation)
            .sub(owner.position)
            .multiplyScalar(percent),
        );
      }

      this.activeParticles = Math.max(
  ...
  protected updateParticles(deltaTime: number) {
    ...
    if (
      ...
      if (
        !this.skeletalMeshActor.bDeleteMe &&
        this.skeletalMeshActor.Mesh &&
        this.skeletalMeshActor.Mesh.IsA("USkeletalMesh")
      ) {
        const SkeletalMeshInstance =
          this.skeletalMeshActor.Mesh.MeshGetInstance(this.skeletalMeshActor);
        if (SkeletalMeshInstance)
          numBones = SkeletalMeshInstance.GetMeshJointsAndNormals(
            this.skeletalMeshActor,
            this.meshVertsAndNormals,
          );
      }
    ...
    if (this.activeParticles < this.maxParticles) {
      if (this.isAutomaticInitialSpawning) {
        rate =
          this.maxParticles /
          ((this.lifetimeRange.min + this.lifetimeRange.max) / 2);
      } else {
    ...
    // Spawning on trigger.
    if (this.currentSpawnOnTrigger) rate += this.spawnOnTriggerPPS;
    ...
    if (!this.killPending) {
      ...
      for (let i = 0; i < Amount; i++) {
        if (this.particleIndex !== -1) {
          ...
          this.particleIndex =
            (this.particleIndex + 1) % this.maxActiveParticles;
        }
      }
    ...
    const ownerOffset = tmpOwnerOffset
      .copy(owner.position)
      .sub(this.oldOwnerLocation);
    const accelerationScale = this.parent?.scale.x
    ...
    for (
      ...
      if (tickParticle) {
        ...
        particle.position.add(
          tmpPhysicsVector
            ...
            .multiply(particle.velocityMultiplier)
            .multiplyScalar(deltaTime),
        );

        if (numBones && this.useSkeletalLocationAs === "location") {
          const newMeshLocation = tmpPhysicsVector
            .copy(this.meshVertsAndNormals[particle.boneIndex * 2])
            .multiply(this.skeletalScale);
          particle.position.add(
            tmpPhysicsVector2
              .copy(newMeshLocation)
              .sub(particle.oldMeshLocation),
          );
        ...
        if (this.isUsingRevolution) {
          ...
          loc.applyAxisAngle(
            AXIS_X,
            particle.revolutionsPerSecond.x *
              particle.revolutionsMultiplier.x *
              angleScale,
          );
          loc.applyAxisAngle(
            AXIS_Y,
            particle.revolutionsPerSecond.y *
              particle.revolutionsMultiplier.y *
              angleScale,
          );
          loc.applyAxisAngle(
            AXIS_Z,
            particle.revolutionsPerSecond.z *
              particle.revolutionsMultiplier.z *
              angleScale,
          );
        ...
        if (this.isScaleSizeRegular)
          ...
          for (let n = 0; n < this.sizeScale.length; n++) {
            if (this.sizeScale[n].relTime >= sizeRelativeTime) {
              ...
              let a;
              if (r2) a = (sizeRelativeTime - r1) / (r2 - r1);
              else a = 1;
      ...
      particle.scale
        ...
        .multiplyScalar(timeFactor * this.drawScale);
        ...
        if (particle.maxLifetime) {
          ...
          for (let n = 0; n < this.velocityScale.length; n++) {
            if (this.velocityScale[n].relativeTime >= velocityRelativeTime) {
              ...
              let a;
              if (r2) a = (velocityRelativeTime - r1) / (r2 - r1);
              else a = 1;
      ...
      if (
        ...
        if (this.scaleSizeXByVelocity)
          particle.scale.x *=
            velocitySize * this.scaleSizeByVelocityMultiplier.x;
        if (this.scaleSizeYByVelocity)
          particle.scale.y *=
            velocitySize * this.scaleSizeByVelocityMultiplier.y;
        if (this.scaleSizeZByVelocity)
          particle.scale.z *=
            velocitySize * this.scaleSizeByVelocityMultiplier.z;
      }
      ...
      if (this.isUsingRevolutionScale) {
        ...
        if (particle.maxLifetime) {
          ...
          for (let n = 0; n < this.revolutionScale.length; n++) {
            if (
              this.revolutionScale[n].relativeTime >= revolutionRelativeTime
            ) {
              let v1,
              ...
              let a;
              if (r2) a = (revolutionRelativeTime - r1) / (r2 - r1);
              else a = 1;
      ...
      if (this.isUsingColorScale && particle.maxLifetime) {
        ...
        for (let n = 0; n < this.colorScale.length; n++) {
          if (this.colorScale[n].relativeTime >= colorRelativeTime) {
            ...
            let a;
            if (r2) a = (colorRelativeTime - r1) / (r2 - r1);
            else a = 1;
      ...
      if (!this.isRespawningDeadParticles) {
        if (
          ...
          this.initialParticlesPerSecond === 0
        ) {
      ...
      if (
        (this.isFadingOut &&
          time > this.fadeOutStartTime &&
          particle.maxLifetime != this.fadeOutStartTime) ||
        ...
        if (this.drawStyle === "modulate") {
          color.set(0.5, 0.5, 0.5, 1 - fadeFactor * maxFade.w);
      ...
      if (
        this.opacity < 1 &&
        this.drawStyle !== "normal"
      ) //don't do Opacity for Regular blend mode
      {
        if (
          this.drawStyle === "alpha" ||
      ...
      const particleExtent =
        Math.max(particle.scale.x, particle.scale.y, particle.scale.z) *
        this.particleGeometryRadius;
      this.boundingBox.expandByPoint(
        tmpBoxExpand.copy(particle.position).addScalar(particleExtent),
      );
      this.boundingBox.expandByPoint(
        tmpBoxExpand.copy(particle.position).addScalar(-particleExtent),
      );
      if (particleExtent > maxParticleExtent)
        maxParticleExtent = particleExtent;

      // Clamping velocity.
      if (this.maxAbsVelocity.x)
        particle.velocity.x = clamp(
          particle.velocity.x,
          -this.maxAbsVelocity.x,
          this.maxAbsVelocity.x,
        );
      if (this.maxAbsVelocity.y)
        particle.velocity.y = clamp(
          particle.velocity.y,
          -this.maxAbsVelocity.y,
          this.maxAbsVelocity.y,
        );
      if (this.maxAbsVelocity.z)
        particle.velocity.z = clamp(
          particle.velocity.z,
          -this.maxAbsVelocity.z,
          this.maxAbsVelocity.z,
        );
      ...
      particle.velocity.sub(
        tmpPhysicsVector
          ...
          .multiply(
            randVector(
              this.tmpVec,
              this.realVelocityLossRange.min,
              this.realVelocityLossRange.max,
            ),
          )
          .multiplyScalar(deltaTime),
      );
    ...
    if (
      ...
      if (this.scaleSizeXByVelocity)
        maxScaleSizeByVelocityMultiplier = Math.max(
          maxScaleSizeByVelocityMultiplier,
          this.scaleSizeByVelocityMultiplier.x,
        );
      if (this.scaleSizeYByVelocity)
        maxScaleSizeByVelocityMultiplier = Math.max(
          maxScaleSizeByVelocityMultiplier,
          this.scaleSizeByVelocityMultiplier.y,
        );
      if (this.scaleSizeZByVelocity)
        maxScaleSizeByVelocityMultiplier = Math.max(
          maxScaleSizeByVelocityMultiplier,
          this.scaleSizeByVelocityMultiplier.z,
        );
    }

    // Subclasses use this to expand bounding box accordingly.
    this.maxSizeScale =
      maxScale * maxVelocityScale * maxScaleSizeByVelocityMultiplier;
    ...
    if (this.instancedMesh)
      this.worldParticleExtent =
        maxParticleExtent * this.maxSizeScale * this.scale.x;

    // Finalize state.
    if (
      (deadParticles >= this.maxActiveParticles ||
        this.activeParticles - deadParticles <= 0) &&
      this.particlesPerSecond === 0 &&
      !this.isRespawningDeadParticles
    )
      this.allParticlesDead = true;
    else this.allParticlesDead = false;

    this.activeCount = this.activeParticles - deadParticles;
    if (this.parent) this.oldOwnerLocation.copy(owner.position);
  ...
  public update(currentTime: number) {
    ...
    if (!this.warmedUp && this.parent && this.warmupGate) {
      ...
      if (this.warmupTime > 0) {
        ...
        if (this.maxParticles > 0) {
          const numToSpawn =
            this.forcedMaxParticles || this.isAutomaticInitialSpawning
              ? this.maxParticles
              : Math.min(
                  this.maxParticles,
                  Math.floor(
                    Math.max(
                      (this.lifetimeRange.min + this.lifetimeRange.max) / 2,
                      0,
                    ) * this.initialSettings.particlesPerSecond,
                  ),
                );

          for (let i = 0; i < numToSpawn; i++) {
            const randomAge = lerp(
              this.initialTimeRange.min,
              this.initialTimeRange.max,
              Math.random(),
            );
            this.spawnParticle(
              i,
    ...
    if (
      this.instancedMesh?.isWorldBatchCandidate &&
      this.activeCount > 0 &&
      isOrderIndependentAdditive(this.instancedMesh.material)
    ) {
      // Batched emitters write directly from simulation state.
      this.instancedMesh.clearInstances();
      if (this.instancedMesh.material.isUpdatable)
        this.instancedMesh.material.update(currentTime);
      this.currentTime = currentTime;
      return;
    }
    ...
    this.particlePool.forEach((p, i) => {
      ...
      p.updateMatrixWorld = p.visible
        ? Object3D.prototype.updateMatrixWorld
        : frozenUpdateMatrixWorld;

      if (this.instancedMesh) {
        if (!p.visible) {
          this.instancedMesh.setInactive(i);
          return;
        }
      } else if (!p.visible) {
        return;
      ...
      if (this.isSpinning || this.spinParticles) {
        ...
        const rotPitch =
          (settings.startSpin.x + settings.time * settings.spinsPerSecond.x) *
          ((Math.PI * 2) / 65536);
        const rotYaw =
          (settings.startSpin.y + settings.time * settings.spinsPerSecond.y) *
          ((Math.PI * 2) / 65536);
        const rotRoll =
          (settings.startSpin.z + settings.time * settings.spinsPerSecond.z) *
          ((Math.PI * 2) / 65536);
      ...
      if (this.instancedMesh) {
        ...
        this.instancedMesh.setInstance(
          i,
          settings.position,
          settings.scale.x * scaleFactor,
          settings.scale.y * scaleFactor,
          spin,
          settings.color,
          tmpSubdivUV[0],
          tmpSubdivUV[1],
      ...
      const mats =
        visualizer.material instanceof Array
          ? visualizer.material
          : [visualizer.material];
      for (const mat of mats) {
        if ((mat as any).isMeshEmitterMaterial) {
          ...
          emat.uniforms.diffuse.value.setRGB(
            settings.color.x,
            settings.color.y,
            settings.color.z,
          );
          ...
          smat.uniforms.diffuse.value.setRGB(
            settings.color.x,
            settings.color.y,
            settings.color.z,
          );
          ...
          pmat.uniforms.diffuse.value.setRGB(
            settings.color.x,
            settings.color.y,
            settings.color.z,
          );
          pmat.uniforms.opacity.value = settings.color.w;
          if (pmat.uniforms.uvOffsetScale)
            pmat.uniforms.uvOffsetScale.value.set(
              tmpSubdivUV[0],
              tmpSubdivUV[1],
              tmpSubdivUV[2],
              tmpSubdivUV[3],
  ...
  protected resolveSubdivision(settings: {
    subdivision: number;
    time: number;
    maxLifetime: number;
  }): number {
    if (settings.subdivision !== -1) return settings.subdivision;
    if (!this.texSubdivU || !this.texSubdivV || !settings.maxLifetime)
      return -1;
  ...
  protected createInstancedMesh(_capacity: number): InstancedSpriteMesh | null {
    return null;
  }
}
...
class Particle extends Object3D {
  ...
  public get velocity(): Readonly<THREE.Vector3> {
    return this._velocity;
  }

  protected _oldLocation = new Vector3();
  public get oldLocation(): Readonly<THREE.Vector3> {
  ...
  private constructor(
    particleSystem: BaseEmitter,
    visualizer: THREE.Mesh<THREE.BufferGeometry, ParticleMaterial> | null,
  ) {
    super();
  ...
  public update(currentTime: number) {
    ...
    const mats =
      this.visualizer.material instanceof Array
        ? this.visualizer.material
        : [this.visualizer.material];
    ...
    for (const mat of mats) {
      if (this.fadeIn && this.fadeIn.time > timeAlive) {
        ...
        mat.color.setRGB(
          this.initial.color.x * r,
          this.initial.color.y * g,
          this.initial.color.z * b,
        );
        ...
        const [r, g, b, a] = this.fadeOut.color
          .toArray()
          .map((v) => 1 - v * fade);
        ...
        mat.color.setRGB(
          this.initial.color.x,
          this.initial.color.y,
          this.initial.color.z,
        );
    ...
    if (this.changesOverLifetime.scale) {
      ...
      const idxFinish = Math.min(idxStart + 1, values.length - 1);
...
function buildChangeRanges(ranges: {
