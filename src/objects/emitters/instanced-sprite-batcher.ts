...
const cacheMaterialKey = new WeakMap<
  ShaderMaterial,
  { textureUuid: string; key: string }
>();

function isOrderIndependentAdditive(material: any): material is ShaderMaterial {
  return (
    material?.isInstancedParticleMaterial === true &&
...
class SpriteParticleBatch {
  ...
  public update(emitters: any[]) {
    ...
    const rights = this.fixedNormal
      ? (this.rightAttr.array as Float32Array)
      : null;
    const ups = this.fixedNormal ? (this.upAttr.array as Float32Array) : null;
    const scales = this.fixedNormal
      ? null
      : (this.scaleAttr.array as Float32Array);
    const spins = this.fixedNormal
      ? null
      : (this.spinAttr.array as Float32Array);
      ...
      for (let i = 0; i < particleLimit; i++) {
        ...
        if (this.fixedNormal) {
          ...
          right
            .copy(baseRight)
            .multiplyScalar(cos)
            .add(spinTemp.copy(baseUp).multiplyScalar(sin))
            .multiplyScalar(scaleX);
          up.copy(baseUp)
            .multiplyScalar(cos)
            .sub(spinTemp.copy(baseRight).multiplyScalar(sin))
            .multiplyScalar(scaleY);
        }

        const subdivUV = emitter.computeSubdivUV(
          emitter.resolveSubdivision(settings),
          emitter.worldBatchSubdivUV,
        );
...
class InstancedSpriteBatcher {
  ...
  public update(emitters: any[], camera: Camera) {
    ...
    for (const emitter of emitters) {
      ...
      if (
        !mesh?.visible ||
        !mesh.isWorldBatchCandidate ||
        !emitter.activeCount ||
        !isOrderIndependentAdditive(material)
      )
        continue;

      if (!emitter.worldBatchSubdivUV)
      ...
      const depthBucket = Math.max(
        0,
        Math.floor(-tmpViewPosition.z / DEPTH_BUCKET_SIZE),
      );
