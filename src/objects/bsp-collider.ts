...
type Vector3Arr = GD.Vector3Arr;
type IBSPNodeDecodeInfo_T = GD.IBSPNodeDecodeInfo_T;
type IBSPNodeCollisionInfo_T = GD.IBSPNodeCollisionInfo_T;
...
export class BSPCollider extends GameObject implements ICollidable {
    ...
    public constructor(nodes: IBSPNodeDecodeInfo_T[]) {
        ...
        // Every node carried a collision flag but no usable bounds - nothing to register, and
        // createCollider would throw. Stay in the scene graph, just opt out of collision.
        if (this.colliderDescs.length === 0) (this as any).isCollidable = false;
    ...
    public createCollider(physicsWorld: RAPIER.World): RAPIER.Collider {
        ...
        for (const info of this.colliderDescs) {
            try {
                ...
                throw new Error(`Failed to create BSP collision hull ${info.nodeIndex}: ${e}`, { cause: e });
            }
        }
