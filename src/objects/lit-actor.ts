...
function getAffectedVertices(
  flags: Uint8Array,
  vertexCount: number,
  rangeStart: number = 0,
  rangeEnd: number = vertexCount,
): Uint32Array {
...
class LitActorMesh extends Mesh {
  ...
  public constructor(props: {
    ...
    if (
      this.lightInfo ||
      (this.ambient && !this.ambient.isUnlit) ||
      this.isSunAffected
    ) {
      ...
      this.geometry.setAttribute(
        "sunAffected",
        new BufferAttribute(sunAffected, 1, true),
      );
    }
  }
  ...
  protected getPerVertexGlow(
    ...
    if (
      ...
      this.perVertexGlow?.length !== vertexCount
    ) {
      ...
      for (let vi = 0; vi < vertexCount; vi++) {
        if (
          currentActor &&
          vi >= currentActor.startVertex + currentActor.count
        ) {
          currentActorIndex++;
  ...
  protected getVertexToElement(
    perActorAmbient: { startVertex: number; count: number }[],
    vertexCount: number,
  ): Uint32Array {
  ...
  protected computeLighting(
    _sector: SectorObject,
    lights: AugmentedLight_T[],
    target: Uint8ClampedArray,
    multiplier: number,
    elemVisibility?: Uint8Array,
    vertexToElement?: Uint32Array,
  ) {
    ...
    const perActorAmbient:
      | { startVertex: number; count: number; scaledGlow: number }[]
      | undefined = this.perActorAmbient;
    const glowPerVertex = perActorAmbient
      ? this.getPerVertexGlow(perActorAmbient, vertexArrayLen)
      : null;
    ...
    for (const {
      ...
      const indices = getAffectedVertices(
        flags,
        vertexArrayLen,
        vertexRangeStart,
        vertexRangeEnd,
      ...
      for (let k = 0, len = indices.length; k < len; k++) {
        ...
        const intensity =
          ...
          scaleGlow *
          0.5 *
          this.sampleIntensity(light, samplingPoint, samplingNormal);
  ...
  public get needsInitialLighting(): boolean {
    return (
      !this.staticLightingCache &&
      (!!this.lightInfo || !!this.ambient || this.isSunAffected)
    );
  }
  ...
  protected lightSets?: {
    ...
    staticScene: AugmentedLight_T[];
    staticEnv: AugmentedLight_T[];
    dynamicScene: AugmentedLight_T[];
    dynamicEnv: AugmentedLight_T[];
  };

  protected resolveLightSets(sector: SectorObject) {
    if (this.lightSets) return this.lightSets;

    const scene: AugmentedLight_T[] =
      this.lightInfo?.scene.map((l) => ({
        ...l,
        instance: sector.lights[l.light],
      })) || [];
    const environment: AugmentedLight_T[] =
      this.lightInfo?.environment.map((l) => ({
        ...l,
        instance: sector.lights[l.light],
      })) || [];

    const isStatic = (l: AugmentedLight_T) =>
      l.instance && !l.instance.isDynamic && !l.instance.isTimeBased;
    const isDynamic = (l: AugmentedLight_T) =>
      l.instance && (l.instance.isDynamic || l.instance.isTimeBased);
  ...
  public update(sector: SectorObject, env: L2Environment) {
    ...
    let staticCacheDirty =
      !this.staticLightingCache ||
      this.staticLightingCache.length !== colorArray.length;
    ...
    const {
      ...
      dynamicScene,
      dynamicEnv,
    } = this.resolveLightSets(sector);

    if (staticEnv.length >= 2) {
      const [currEnvIndex, nextEnvIndex, lerp] =
        env.selectEnvironmentLightIndices(staticEnv.length);

      if (
        currEnvIndex !== this.lastStaticEnvIndex ||
        nextEnvIndex !== this.lastStaticEnvNextIndex ||
        Math.abs(lerp - this.lastStaticEnvLerp) >= 0.02
      ) {
    ...
    if (
      !staticCacheDirty &&
      !anyDynamicLightNeedsUpdate &&
      !this.needsRelightPass
    )
      return;

    // dynamic pass below filters by elemVisibility - a batch's .visible flag covers every merged actor
    const perActorAmbientForFilter = this.perActorAmbient as
      | { startVertex: number; count: number }[]
      | undefined;
    const canFilterByVisibility =
      !!this.elemVisibility && !!perActorAmbientForFilter;
    const vertexToElement = canFilterByVisibility
      ? this.getVertexToElement(
          perActorAmbientForFilter!,
          colorArray.length / 3,
        )
      : undefined;
    const relight =
      canFilterByVisibility && !staticCacheDirty && !anyDynamicLightNeedsUpdate
        ? this.elemRelight
        : undefined;
    const filterVisibility =
      relight ?? (canFilterByVisibility ? this.elemVisibility : undefined);
    ...
    if (staticCacheDirty) {
      ...
      if (this.perActorAmbient) {
        ...
        for (const actor of perActorAmbient) {
          if (actor.ambient && actor.ambient.isUnlit) {
            ...
            for (
              let i = actor.startVertex * 3,
                end = (actor.startVertex + actor.count) * 3;
              i < end;
              i += 3
            ) {
            ...
            tmpColorByte.set(
              actor.ambient.vector[0],
              actor.ambient.vector[1],
              actor.ambient.vector[2],
            );
      ...
      if (staticEnv.length >= 2) {
        ...
        if (lerp > 0.0)
          this.computeLighting(
            sector,
            [staticEnv[nextEnvIndex]],
            this.staticLightingCache,
            lerp,
          );
      } else if (staticEnv.length === 1)
        this.computeLighting(sector, staticEnv, this.staticLightingCache, 1.0);
    }
    ...
    if (relight) {
      for (let ei = 0; ei < relight.length; ei++) {
        ...
        colorArray.set(
          this.staticLightingCache!.subarray(
            startVertex * 3,
            (startVertex + count) * 3,
          ),
          startVertex * 3,
    ...
    if (dynamicEnv.length >= 2) {
      const [currEnvIndex, nextEnvIndex, lerp] =
        env.selectEnvironmentLightIndices(dynamicEnv.length);
      if (lerp < 1.0)
        this.computeLighting(
          sector,
          [dynamicEnv[currEnvIndex]],
          colorArray,
          1.0 - lerp,
          filterVisibility,
          vertexToElement,
  ...
  protected sampleIntensity(
    light: DynamicLight,
    samplingPoint: Vector3,
    samplingNormal: Vector3,
  ): number {
