...
function getSunModifierInfo(
  timeOfDay: number,
  baseYawDegrees: number = 180,
...
class DynamicLight extends Object3D {
  ...
  public update(envManager: L2Environment, levelBrightness: number) {
    ...
    const timeVal =
      (timeSeconds * 35 * 65536) / Math.max(Math.floor(this.period), 1) +
      (this.phase << 8);
    ...
    this.color.multiplyScalar(
      Math.min(1, Math.max(0, intensity * levelBrightness)),
    );
    ...
    if (this.lightEffect === LE_SUNLIGHT) {
      ...
      this.lightEffect === LE_SPOTLIGHT ||
      this.lightEffect === LE_STATIC_SPOT
    ) {
    ...
    if (this.isDynamic) {
      ...
      if (this.isTimeBased || envChanged) {
        if (
          ...
          this.lastComputedDirection === null ||
          envChanged
        ) {
          this.needsUpdate = true;
          ...
          const directionChanged =
            this.lightDirection.distanceToSquared(this.lastComputedDirection) >
            DIRECTION_CHANGE_EPSILON_SQ;
  ...
  public sampleIntensity(
    samplePosition: Vector3,
    sampleNormal: Vector3,
  ): number {
    ...
    if (this.lightEffect === LE_SUNLIGHT) {
      ...
      if (distance < radius)
        return (
          Math.max(
            0,
            1 - (lightVector.x ** 2 + lightVector.y ** 2) / radius ** 2,
          ) * 2
        );
      else return 0;
      ...
      const distance = Math.sqrt(distanceSquared);
      const baseAttenuation = calculateAttenuation(
        distance,
        radius,
        dx,
        dy,
      ...
      if (baseAttenuation > 0) {
        ...
        if (vDotV > 0.0 && vDotV ** 2 > sineSq * distanceSquared)
          return (
            Math.pow((vDotV * rSine) / distance - sineRSine, 2) *
            baseAttenuation
          );
      }
