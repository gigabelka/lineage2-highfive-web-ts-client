...
function getTransparentLookup(
  object: any,
  transparentMaterials: Set<number>,
): Uint8Array {
  let lookup = object.transparentLookup as Uint8Array;
  if (lookup) return lookup;

  const allTransparent =
    (object.transparentMaterialIndexes as Set<number>) ?? transparentMaterials;

  lookup = object.transparentLookup = new Uint8Array(
...
function rebuildBatchGroups(
  object: any,
  geometry: BufferGeometry,
  visibleGroups: BatchGroup_T[],
  transparentMaterials: Set<number>,
) {
  ...
  visibleGroups.sort((a, b) => {
    ...
    if (a.transparent)
      return b.distance - a.distance || a.materialIndex - b.materialIndex;
  ...
  const source = (object as any).batchIndices as
    | Uint8Array
    | Uint16Array
    | Uint32Array
    | null;
  ...
  if (!index || !source) {
    ...
    for (const group of visibleGroups) {
      if (
        run &&
        run.materialIndex === group.materialIndex &&
        run.start + run.count === group.start
      ) {
...
class SectorObject extends Object3D {
  ...
  public celestials: {
    ...
    data: any;
  }[] = []; // Decoded celestial data with textures
  ...
  protected terrainRenderables?: {
    batches: THREE.Mesh[];
    standalone: any[];
    decorations: Map<string, TerrainDecoration[]>;
  };
  ...
  protected topLevelBSPResult?: {
    visibleNodes: Set<number>;
    visibleLeaves: Set<number>;
    finalZoneMask: bigint;
    zonesAddedThroughPortals: Set<number>;
  };
  ...
  public setBSPInfo(
    bspZones: GD.IBSPZoneDecodeInfo_T[],
    bspNodes: GD.IBSPNodeDecodeInfo_T[],
    bspLeaves: GD.IBSPLeafDecodeInfo_T[],
  ) {
    ...
    this.bspZones.forEach((zone, index) => {
      if (index === 0 || zone.zoneInfo?.isSunAffected)
        outdoorMask |= 1n << BigInt(index);
    });
  ...
  public getSectionZoneAmbient(sectionIndex: number): ColorByte | null {
    if (this.cacheSectionZoneAmbient.has(sectionIndex))
      return this.cacheSectionZoneAmbient.get(sectionIndex);
  ...
  public findPositionZone(position: THREE.Vector3) {
    ...
    const findZonePosition = tmpVec4.set(
      position.x,
      position.y,
      position.z,
      -1,
  ...
  public setSun(sunMaterial: any) {
    this.sunTexture = sunMaterial;
  }

  public getMusicIdAt(cameraPosition: THREE.Vector3): {
    musicId: number | null;
    isLooped: boolean;
    isForced: boolean;
  } {
  ...
  public getActiveZoneMask(cameraPosition: THREE.Vector3): bigint {
    ...
    // Get camera zone
    const cameraZone =
      leafIndex !== null && leafIndex >= 0 && leafIndex < this.bspLeaves.length
    ...
    if (
      leafIndex !== null &&
      leafIndex >= 0 &&
      leafIndex < this.bspLeaves.length
    ) {
  ...
  public traverseBSP(
    ...
    if (this.bspNodes.length === 0 || !this.nodeZoneMasks) {
      ...
      return {
        visibleNodes,
        visibleLeaves,
        finalZoneMask: activeZoneMask,
        zonesAddedThroughPortals: new Set<number>(),
      };
    }
    ...
    if (topLevelOnly) {
      ...
      for (let nodeIndex = 0; nodeIndex < this.bspNodes.length; nodeIndex++) {
        const sectionIndex = this.nodeToSection
          ? this.nodeToSection[nodeIndex]
          : -1;
        if (sectionIndex >= 0) {
          const sectionInfo = this.bspSections
            ? (this.bspSections[sectionIndex] as any)
            : null;
      ...
      return (this.topLevelBSPResult = {
        visibleNodes,
        visibleLeaves,
        finalZoneMask: this.outdoorZoneMask,
        zonesAddedThroughPortals: new Set<number>(),
      });
    }
    ...
    if (leafOnlyMode) {
      if (this.bspLeaves && this.bspLeaves.length > 1) {
        ...
        if (leaf1 && leaf1.zone >= 0) {
          ...
          for (
            let nodeIndex = 0;
            nodeIndex < this.bspNodes.length;
            nodeIndex++
          ) {
            const node = this.bspNodes[nodeIndex];
            if (
              (node.leaves[0] === 1 || node.leaves[1] === 1) &&
              this.nodeToSection &&
              this.nodeToSection[nodeIndex] !== undefined &&
              this.nodeToSection[nodeIndex] >= 0
            ) {
              visibleNodes.add(nodeIndex);
            }
          }
    ...
    while (nodeStack.length > 0) {
      ...
      if (pass === "front") {
        ...
        if (
          hasViewZone &&
          this.bspGroup &&
          node.iRenderBound !== undefined &&
          node.iRenderBound >= 0 &&
          library?.bspRenderBounds
        ) {
              ...
              if (currentZoneMask & zoneBit) {
                ...
                if (nodeZoneMask && nodeZoneMask & zoneBit) {
                  ...
                  if (
                    ...
                    cameraFrustum.intersectsBox(box)
                  ) {
                    boxVisible = true;
                    break;
                  }
        ...
        if (farChild >= 0) {
          ...
          if (
            farLeafIndex >= 0 &&
            farLeafIndex < (this.bspLeaves?.length || 0)
          ) {
        ...
        if (nearChild >= 0) {
          ...
          if (
            nearLeafIndex >= 0 &&
            nearLeafIndex < (this.bspLeaves?.length || 0)
          ) {
        ...
        while (current >= 0) {
          ...
          const hasPortalFlag =
            currentNode.surfFlags !== undefined &&
            (currentNode.surfFlags & PF_Portal) !== 0;
          ...
          if (
            ...
            const portalDistance = cameraPosition.distanceTo(
              portalSphere.center,
            );
            const isPortalInRange =
              portalDistance <= PORTAL_MAX_DISTANCE + portalSphere.radius;

            const portalVisible =
              isCurrentZoneActive &&
              isPortalInRange &&
              (!frustumCullingEnabled ||
                cameraFrustum.intersectsSphere(portalSphere));
            ...
            if (leafOnlyMode) {
              ...
              if (!alreadyAdded) {
                ...
                const sourceDepth =
                  zoneDepthMap.get(currentZone) ?? recursionDepth;
                ...
                if (
                  this.bspZones &&
                  currentZone >= 0 &&
                  currentZone < this.bspZones.length
                ) {
          ...
          if (frustumCullingEnabled) {
            isInFrustum = cameraFrustum.intersectsSphere(
              currentNode.exclusiveSphereBound,
            );
          }
          ...
          if (
            isInFrustum &&
            this.nodeToSection &&
            this.nodeToSection[current] !== undefined &&
            this.nodeToSection[current] >= 0
          ) {
    ...
    return {
      visibleNodes,
      visibleLeaves: filteredVisibleLeaves,
      finalZoneMask: currentZoneMask,
      zonesAddedThroughPortals,
    };
  }
   ...
   */
  public updateVisibleBSPSections(
    cameraPosition: THREE.Vector3,
    cameraFrustum: THREE.Frustum,
    frustumCullingEnabled: boolean = true,
  ) {
    ...
    const activeZoneMask = leafOnlyMode
      ? this.outdoorZoneMask
      : this.getActiveZoneMask(cameraPosition); // outdoor zones if camera is outside the bsp
    ...
    const zoneChanged =
      currentZone !== null &&
      currentZone >= 0 &&
      (this._lastLoggedZone === null || this._lastLoggedZone !== currentZone);
    const leafChanged =
      leafIndex !== null &&
      leafIndex >= 0 &&
      (this._lastLoggedLeaf === null || this._lastLoggedLeaf !== leafIndex);
    ...
    // Traverse BSP to find visible nodes (with optional frustum culling)
    const { visibleNodes, finalZoneMask } = this.traverseBSP(
      cameraPosition,
      activeZoneMask,
      cameraFrustum,
      frustumCullingEnabled,
    ...
    this.bspGroup.children.forEach((child) => {
      if (child instanceof Mesh && (child as any).sectionIndex !== undefined) {
        ...
        if (child.visible) {
          visibleMeshCount++;
          if (!wasVisible)
            visibleMeshNames.push(`${child.name || `Section_${sectionIndex}`}`);
        } else {
          if (wasVisible)
            hiddenMeshNames.push(`${child.name || `Section_${sectionIndex}`}`);
        }
      }
    });
  ...
  protected updateLights(environment: L2Environment) {
    ...
    for (const light of this._animatedLights)
      light.update(environment, this.brightness);
  }

  protected isVisibilityCacheValid(
    environment: L2Environment,
    cameraPosition: Vector3,
    cameraFrustum: THREE.Frustum,
    frustumCullingEnabled: boolean,
    topLevelOnly: boolean,
    staticMeshCullDistanceSq: number,
    emitterCullDistanceSq: number,
  ) {
    const envVersion = environment.getEnvVersion();
    const timeStep = Math.floor(environment.getTimeOfDay() * 600);
    let unchanged =
      this.visibilityCacheInitialized &&
    ...
    for (
      let i = 0, offset = 0;
      unchanged && i < cameraFrustum.planes.length;
      i++
    ) {
  ...
  public updateVisibility(
    ...
    frustumCullingEnabled: boolean = true,
    topLevelOnly: boolean = false,
    staticMeshCullDistanceSq: number = Infinity,
    emitterCullDistanceSq: number = Infinity,
  ) {
    ...
    if (
      this.isVisibilityCacheValid(
        environment,
        cameraPosition,
        cameraFrustum,
        frustumCullingEnabled,
        topLevelOnly,
        staticMeshCullDistanceSq,
        emitterCullDistanceSq,
      )
    )
      return;
    ...
    const activeZoneMask = leafOnlyMode
      ? this.outdoorZoneMask
      : this.getActiveZoneMask(cameraPosition);
    ...
    if (this.bspGroup && this.bspSections && this.nodeToSection) {
      ...
      if (environment) {
        bspAmbientColor =
          environment.getAmbientPlaneBSPLightHalved(tmpColorByte);
        const sunColor =
          environment.getBaseColorPlaneBSPSunLightScaled(tmpColorByte2);
        bspAmbientColor.set(
          bspAmbientColor.r + sunColor.r,
          bspAmbientColor.g + sunColor.g,
          bspAmbientColor.b + sunColor.b,
          bspAmbientColor.a,
        );
      }
      ...
      this.bspGroup.children.forEach((child) => {
        if (
          ...
          (child as any).sectionIndex !== undefined
        ) {
          const sectionIndex = (child as any).sectionIndex;
          ...
          if (visible && topLevelOnly && frustumCullingEnabled) {
            ...
            tmpSphere
              .copy(child.geometry.boundingSphere)
          ...
          if (child.visible) {
            ...
            if (isOutdoor) outdoorCount++;
            else indoorCount++;

            const material = child.material;
            const hasLightmap = (m: any) =>
              m?.defines?.USE_LIGHTMAP !== undefined;

            const sectionZoneAmbient = isOutdoor
              ? null
              : this.getSectionZoneAmbient(sectionIndex);
            ...
            const applyAmbient = (m: any) => {
              if (m?.uniforms?.ambient?.value?.color) {
                ...
                if (isOutdoor && bspAmbientColor)
                  bspAmbientColor.toFloats(m.color);
    ...
    if (library && this.staticMeshGroup && this.staticMeshMap.size > 0) {
        ...
        if (actors) {
          for (const actorBase of actors) {
            ...
            const isFrustumVisible =
              !frustumCullingEnabled ||
              cameraFrustum.intersectsBox(tmpActorBox);
            const isZoneVisible =
              !frustumCullingEnabled ||
              !actor.zoneMask ||
              !!(actor.zoneMask & finalZoneMask);
            ...
            const isInRange =
              isRangeIgnored || distSq <= staticMeshCullDistanceSq;
      ...
      for (const { object, uuid } of this.staticMeshVisibilityEntries) {
        ...
        if ((object as any).isBatch) {
          ...
          const sortedTransparentMats = (object as any)
            .sortedTransparentMaterialIndexes as Set<number> | undefined;
          let elemVisibility = (object as any).elemVisibility as Uint8Array;
          if (
            !elemVisibility ||
            elemVisibility.length !== batchElements.length
          ) {
            elemVisibility = (object as any).elemVisibility = new Uint8Array(
          ...
          if (!elemDistances || elemDistances.length !== batchElements.length)
            elemDistances = (object as any).elemDistances = new Float64Array(
              batchElements.length,
            );
          let elemRelight = (object as any).elemRelight as Uint8Array;
          if (!elemRelight || elemRelight.length !== batchElements.length)
            elemRelight = (object as any).elemRelight = new Uint8Array(
              batchElements.length,
            );
            ...
            if (!elemVisible && !candidateActorUuids.has(elem.uuid)) {
              ...
              if (!inVisibleLeaves) {
                for (const li of elem.leaves) {
                  if (visibleLeaves.has(li)) {
                    inVisibleLeaves = true;
                    break;
                  }
                }
              }
              ...
              if (inVisibleLeaves) {
                ...
                const isZoneVisible =
                  !frustumCullingEnabled ||
                  !elem.zoneMask ||
                  !!(elem.zoneMask & finalZoneMask);
                const actorCenter = tmpActorBox.getCenter(tmpVec3);
                distSq = cameraPosition.distanceToSquared(actorCenter);
                const isInRange =
                  elem.isRangeIgnored || distSq <= staticMeshCullDistanceSq;
          ...
          const transparentSortPosition = (object as any)
            .transparentSortPosition as Vector3;
          const sortChanged =
            !!sortedTransparentMats?.size &&
            (!transparentSortPosition ||
              transparentSortPosition.distanceToSquared(cameraPosition) >=
          ...
          if (visibilityChanged || sortChanged) {
            ...
            let visibleGroups = (object as any)
              .visibleBatchGroups as BatchGroup_T[];
            if (!visibleGroups)
              visibleGroups = (object as any).visibleBatchGroups = [];
            ...
            if (sortedTransparentMats?.size) {
              if (transparentSortPosition)
                transparentSortPosition.copy(cameraPosition);
              else
                (object as any).transparentSortPosition =
          ...
          if (
            ...
            const isZoneVisible =
              !frustumCullingEnabled ||
              !zoneMask ||
              !!(zoneMask & finalZoneMask);
            const actorCenter = tmpActorBox.getCenter(tmpVec3);
            const distSq = cameraPosition.distanceToSquared(actorCenter);
            const isInRange =
              (object as any).actorRangeIgnored ||
              distSq <= staticMeshCullDistanceSq;
    ...
    if (library) {
      ...
      for (const actorBase of library.allEmitterActors) {
        ...
        const isZoneVisible =
          !frustumCullingEnabled || !zoneMask || !!(zoneMask & finalZoneMask);
        ...
        const isInRange =
          isRangeIgnored || dx * dx + dy * dy <= emitterCullDistanceSq;
        if (!isInRange) continue;

        let isFrustumVisible =
          !frustumCullingEnabled || cameraFrustum.containsPoint(origin);
    ...
    const terrainZoneVisible =
      !frustumCullingEnabled || (finalZoneMask & this.outdoorZoneMask) !== 0n;
    this.updateTerrainSectors(
      environment,
      cameraPosition,
      cameraFrustum,
  ...
  protected updateTerrainSectors(
    environment: L2Environment,
    cameraPosition: THREE.Vector3,
    cameraFrustum: THREE.Frustum,
    frustumCullingEnabled: boolean,
    zoneVisible: boolean = true,
  ) {
    ...
    if (!this.terrainRenderables) {
      ...
      this.zones.traverse((object) => {
        ...
        else if ((object as any).isTerrain && !(object as any).batchGeometry)
          standalone.push(object);
        else if ((object as any).isTerrainDecoration) {
          const decoration = object as TerrainDecoration;
          if (!decorations.has(decoration.terrainSegment))
            decorations.set(decoration.terrainSegment, []);
    ...
    for (const batch of this.terrainRenderables.batches) {
      ...
      sectors.forEach((sector) => {
        ...
        const isVisible =
          zoneVisible &&
          (!frustumCullingEnabled ||
            cameraFrustum.intersectsBox(sector.bounds));
        ...
        const decorations = this.terrainRenderables.decorations.get(
          sector.terrainSegmentUuid,
        );
        if (decorations)
          decorations.forEach((decoration) =>
            decoration.updateVisibility(cameraPosition, isVisible),
          );
      });
  ...
  public updateVisibleStaticMeshActors(
    environment: L2Environment,
    cameraPosition: THREE.Vector3,
    cameraFrustum: THREE.Frustum,
    frustumCullingEnabled: boolean = true,
    ...
    const { finalZoneMask, visibleLeaves } = this.traverseBSP(
      cameraPosition,
      activeZoneMask,
      cameraFrustum,
      frustumCullingEnabled,
    ...
    for (const leafIndex of visibleLeaves) {
      ...
      if (actors) {
        for (const actorBase of actors) {
          ...
          const isFrustumVisible =
            !frustumCullingEnabled || cameraFrustum.intersectsBox(actorBox);
          const isZoneVisible =
            !frustumCullingEnabled ||
            !actor.zoneMask ||
            !!(actor.zoneMask & finalZoneMask);
...
class BSPNodeData {
  ...
  public static fromInfo(info: any) {
    ...
    node.iRenderBound =
      info.iRenderBound !== undefined ? info.iRenderBound : undefined;

    if (info.spheres) {
      node.exclusiveSphereBound.center.set(
        info.spheres.exclusive[0],
        info.spheres.exclusive[1],
        info.spheres.exclusive[2],
      );
      node.exclusiveSphereBound.radius = info.spheres.exclusive[3];

      node.inclusiveSphereBound.center.set(
        info.spheres.inclusive[0],
        info.spheres.inclusive[1],
        info.spheres.inclusive[2],
      );
