...
const TRANSFORM_CHAIN_SLOTS = new Set([
  "shDiffuse",
  "shOpacity",
  "shSpecular",
  "shSpecularMask",
...
function applyParameters({
  ...
  if (parameters.isUsingMap) {
    ...
    if (parameters.transformType !== "none") {
      ...
      defines["OSCILLATE"] = 2;
      defines["ENVMAP"] = 3;
      defines[`USE_MAP_${defName}_TRANSFORM`] =
        parameters.transformType.toUpperCase();
      ...
      if (innerTransforms?.length > 0 && TRANSFORM_CHAIN_SLOTS.has(name)) {
        ...
        uniforms[name].value.innerTransforms =
          padTransformStages(innerTransforms);
      }
    }
  }
}
...
export default class MeshStaticMaterial extends ShaderMaterial {
  ...
  public constructor(info: MeshStaticMaterialParameters = {}) {
      ...
      UniformsUtils.merge([
        {
          ...
          lightMapIntensity: new Uniform(
            info.modulateStaticLighting2X === true ? 2 : 1,
          ),
    ...
    if (info.opacity || info.alphaTest !== undefined)
    ...
    this.isUpdatable =
      this.spriteEntries.length > 0 || this.proceduralMaps.length > 0;
    ...
    switch (info.blendingMode) {
      ...
      case "darken":
        ...
        break;
      default:
        console.warn("Unknown blending mode:", info.blendingMode);
        break;
    }
  }
  ...
  public enableDirectionalAmbient({
    vector,
    direction,
    brightness,
  }: IDirectionalAmbientLighting) {
  ...
  public setSway() {
    ...
    if (this.transparent && this.blending === NormalBlending)
  ...
  public update(time: number) {
    ...
    for (const map of this.proceduralMaps) map.update(time);
