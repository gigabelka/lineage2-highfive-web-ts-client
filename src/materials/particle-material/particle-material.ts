...
export function fixParticleTextureSampling(
  texture: any,
  disableMipmaps: boolean = false,
) {
  if (!texture) return;
  let changed = false;
  if (
...
class ParticleMaterial extends ShaderMaterial {
  ...
  constructor({
    ...
    name,
    usesSubdivision,
  }: ParticleMaterialInitSettings_T) {
    const uniforms = appendGlobalUniforms(
      UniformsUtils.merge([UniformsLib.common, UniformsLib.fog]),
    );
    ...
    const { isAdditive, ...blendingSettings } =
...
class AnimatedParticleMaterial extends ShaderMaterial {
  ...
  public update(time: number) {
    ...
    this.uniforms.map.value =
      this.sprites[activeFrameIndex].uniforms.map.texture;
...
export function getPartcileBlendingSettings(
  ...
  switch (blendingMode as string) {
    case "normal":
      return { blending: NoBlending }; // PTDS_Regular: ONE, ZERO
    case "alpha":
      return {
