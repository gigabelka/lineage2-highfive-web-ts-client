...
import decodeMaterial, {
  canonicalizeStaticMeshMaterials,
  decodeStaticMeshMaterial,
} from "./material-decoder";
import ZoneObject, {
  SectorObject,
  FogInfoObject,
} from "../../objects/zone-object";
...
import {
  batchTerrainSectors,
  createStaticMeshBatchJob,
  decodeStaticMeshInstance,
  makeSwayAttribute,
  stepStaticMeshBatchJob,
  StaticMeshBatchJob_T,
} from "./object-batching";
...
function getAttributeForTypedArray(
  IndexArrayConstructor: GD.IndexTypedArray,
): GD.IndexTypedArrayAttribute {
  switch (IndexArrayConstructor) {
    ...
    default:
      throw new Error(
        `Unsupported index array constructor ${IndexArrayConstructor.name}`,
      );
  }
}
...
function fetchGeometry(info: GD.IGeometryDecodeInfo) {
  ...
  const arrUvs =
    info.attributes.uvs instanceof Array
      ? info.attributes.uvs
      : [info.attributes.uvs];
  ...
  if (info.attributes.normals)
    geometry.setAttribute(
      "normal",
      new BufferAttribute(info.attributes.normals, 3),
    );
  if (info.attributes.positions)
    geometry.setAttribute(
      "position",
      new BufferAttribute(info.attributes.positions, 3),
    );
  if (info.attributes.colors)
    geometry.setAttribute(
      "color",
      new BufferAttribute(
        info.attributes.colors,
        3,
        info.attributes.colors instanceof Uint8Array ||
          info.attributes.colors instanceof Uint8ClampedArray,
      ),
    );
  if (info.attributes.colorsInstance)
    geometry.setAttribute(
      "colorInstance",
      new BufferAttribute(
        info.attributes.colorsInstance,
        3,
        info.attributes.colorsInstance instanceof Uint8Array ||
          info.attributes.colorsInstance instanceof Uint8ClampedArray,
      ),
    );
  if (info.attributes.skinIndex)
    geometry.setAttribute(
      "skinIndex",
      new BufferAttribute(info.attributes.skinIndex, 4),
    );
  if (info.attributes.skinWeight)
    geometry.setAttribute(
      "skinWeight",
      new BufferAttribute(info.attributes.skinWeight, 4),
    );
  if (info.attributes.nodeIndex)
    geometry.setAttribute(
      "nodeIndex",
      new BufferAttribute(info.attributes.nodeIndex, 1),
    );
  if (info.attributes.sway)
    geometry.setAttribute("sway", new BufferAttribute(info.attributes.sway, 4));
  if ((info.attributes as any).terrainIndex)
    geometry.setAttribute(
      "terrainIndex",
      new BufferAttribute((info.attributes as any).terrainIndex, 1),
    );

  if (info.indices) {
    const AttributeConstructor = getAttributeForTypedArray(
      info.indices.constructor as GD.IndexTypedArray,
    );
...
function applySimpleProperties<T extends THREE.Object3D>(
  library: GD.DecodeLibrary,
  object: T,
  info: GD.IBaseObjectDecodeInfo,
) {
  ...
  if (info.children)
    info.children.forEach((ch) => object.add(decodeObject3D(library, ch)));

  return object;
}

function decodeEmitterObject(
  library: GD.DecodeLibrary,
  info: GD.IBaseObjectDecodeInfo,
) {
  ...
  if (info.moveEvent && info.moveEvent !== "None")
    (object as any).moveEvent = info.moveEvent;
...
function decodeSimpleObject(
  library: GD.DecodeLibrary,
  Constructor: typeof Object3D | typeof Group,
  info: GD.IBaseObjectDecodeInfo,
) {
...
function decodeEdges(
  library: GD.DecodeLibrary,
  info: GD.IEdgesObjectDecodeInfo,
): THREE.Line {
  ...
  const mesh = new LineSegments(
    fetchGeometry(library.geometries[info.geometry] as GD.IGeometryDecodeInfo),
    material,
  );
...
function decodeStaticMeshData(
  library: GD.DecodeLibrary,
  info: GD.IStaticMeshObjectDecodeInfo,
) {
  let infoGeo = library.geometries[info.geometry];
  const infoMats = library.materials[info.materials];
  const sway = info.sway
    ? makeSwayAttribute(
        (infoGeo.attributes.positions as Float32Array).length / 3,
        info.sway,
      )
    : null;

  if (sway)
    infoGeo = { ...infoGeo, attributes: { ...infoGeo.attributes, sway } };

  const materials =
    decodeStaticMeshMaterial(
      library,
      infoMats,
      !!infoGeo.attributes.colors,
      false,
      !!sway,
...
function decodeStaticMeshWrapped(
  library: GD.DecodeLibrary,
  info: GD.IStaticMeshObjectDecodeInfo,
): THREE.Object3D {
...
function decodeStaticMeshActor(
  library: GD.DecodeLibrary,
  ...
  const object = info.mover
    ...
    : info.swaying
      ? new SwayingObject({ ...props, swaying: info.swaying })
      : info.rotating
        ? new RotatingObject({ ...props, rotating: info.rotating })
...
function decodeZoneObject(
  library: GD.DecodeLibrary,
  info: GD.IBaseZoneDecodeInfo,
) {
  const object = new ZoneObject();

  if (info.name) object.name = info.name;
  if (info.bounds?.isValid)
    object.setRenderBounds(info.bounds.min, info.bounds.max);
  ...
  if (info.type === "Sky") {
    ...
    if (info.position) {
      ...
      (object as any).skyOrigin = new Vector3().fromArray(
        (info as any).location,
      );
    }
  }
  if (info.children)
...
function decodeBSPSection(
  library: GD.DecodeLibrary,
  sectionInfo: GD.IBSPSectionDecodeInfo_T,
  sectionIndex: number,
): THREE.Mesh {
  ...
  // if (sectionInfo.sectionName.startsWith("Texture_Cm_o_wall010_"))
  ...
  (Array.isArray(materials) ? materials : [materials]).forEach((m) => {
    if (!m) return;
    if (sectionInfo.depthWrite !== undefined)
      m.depthWrite = sectionInfo.depthWrite;
    if (sectionInfo.depthTest !== undefined)
      m.depthTest = sectionInfo.depthTest;
...
function decodeLight(
  library: GD.DecodeLibrary,
  info: GD.ILightDecodeInfo | GD.ISunLightDecodeInfo,
): DynamicLight {
...
function decodeSectorCore(library: GD.DecodeLibrary) {
  ...
  library.bspZones.forEach((bspZone) =>
    sector.zones.add(decodeZoneObject(library, bspZone.zoneInfo)),
  );

  sector.setBSPInfo(library.bspZones, library.bspNodes, library.bspLeaves);
  sector.setLights(
    library.lightActors.map((info) => decodeLight(library, info)),
  );
  library.skyZoneInfos.forEach((info) =>
    sector.add(decodeObject3D(library, info)),
  );
...
function createSectorStaticMeshDecodeJob(
  library: GD.DecodeLibrary,
  sector: SectorObject,
  ...
  return createStaticMeshBatchJob(
    library,
    sector,
    staticMeshGroup,
    fetchGeometry,
...
function stepSectorStaticMeshDecodeJob(
  job: SectorStaticMeshDecodeJob_T,
): boolean {
...
function finishSectorStaticMeshes(
  library: GD.DecodeLibrary,
  sector: SectorObject,
) {
  ...
  library.celestials.forEach((celestialInfo) => {
    try {
      ...
      if (celestialInfo.sprites && celestialInfo.sprites.length > 0) {
        ...
        const material =
          celestialInfo.type === "Moon"
            ? decodeMaterial(library, materialInfo)
            : null;
        let spriteTextureInfo = materialInfo as
          | GD.ITextureDecodeInfo
          | GD.IShaderDecodeInfo;
        let texture = null;
        ...
        if (spriteTextureInfo && spriteTextureInfo.materialType === "shader") {
          ...
          if (shaderInfo.diffuse && library.materials[shaderInfo.diffuse]) {
            spriteTextureInfo = library.materials[
              shaderInfo.diffuse
        ...
        if (spriteTextureInfo) {
          const mapData = decodeTexture(
            library,
            spriteTextureInfo as GD.ITextureDecodeInfo,
          );
...
function decodeSectorStaticMeshes(
  library: GD.DecodeLibrary,
  sector: SectorObject,
) {
...
function decodeTerrainInfo(
  ...
  sectors.forEach((sector) =>
    sectorsByUuid.set(sector.terrainSegmentUuid, sector),
  );
  decorations.forEach((decoration) => {
    const terrain = sectorsByUuid.get(decoration.terrainSegment);
    if (!terrain)
      throw new Error(
        `Terrain segment '${decoration.terrainSegment}' not found for decoration '${decoration.name}'`,
      );
...
function decodeTerrainDecoration(
  library: GD.DecodeLibrary,
  info: GD.ITerrainDecorationDecodeInfo,
) {
  const mesh = info.mesh;
  let geometryInfo = library.geometries[mesh.geometry];
  const sway = mesh.sway
    ? makeSwayAttribute(
        (geometryInfo.attributes.positions as Float32Array).length / 3,
        mesh.sway,
      )
    : null;
  ...
  if (sway)
    geometryInfo = {
      ...geometryInfo,
      attributes: { ...geometryInfo.attributes, sway },
    };
  ...
  const materials =
    decodeStaticMeshMaterial(
      library,
      materialInfo,
      !!geometryInfo.attributes.colors,
      true,
      !!sway,
      true,
    ) || new MeshBasicMaterial({ color: 0xff00ff });
...
function decodeBones(
  library: GD.DecodeLibrary,
  infos: GD.IBoneDecodeInfo[],
): Bone[] {
...
function decodeAnimation(
  ...
  const tracks = info.map((info) => {
    ...
    switch (info.type) {
      case "Quaternion":
        KeyframeTrackConstructor = QuaternionKeyframeTrack;
        break;
      case "Vector":
        KeyframeTrackConstructor = VectorKeyframeTrack;
        break;
    }
...
function decodeSkinnedMesh(
  library: GD.DecodeLibrary,
  info: GD.ISkinnedMeshObjectDecodeInfo,
) {
  const geometry = fetchGeometry(library.geometries[info.geometry]);
  const infoMats = library.materials[info.materials];

  const materials =
    decodeMaterial(library, infoMats) ||
    new MeshBasicMaterial({ color: 0xff00ff });
...
function decodeMeshEmitter(
  library: GD.DecodeLibrary,
  info: GD.IMeshEmitterDecodeInfo,
) {
  const infoGeo = library.geometries[
    info.mesh.geometry
  ] as GD.IGeometryDecodeInfo;

  const geometry = fetchGeometry(infoGeo as GD.IGeometryDecodeInfo);
  const materials =
    decodeMaterial(library, {
      materialType: "particle",
      material: info.mesh.materials,
      opacity: info.opacity,
      blendingMode: info.blendingMode,
    } as GD.IParticleMaterialDecodeInfo) ||
    new MeshBasicMaterial({ color: 0xff00ff });
...
function decodeSpriteEmitter(
  library: GD.DecodeLibrary,
  info: GD.ISpriteEmitterDecodeInfo,
) {
...
function decodeFogInfo(
  ...
  const toRange = (arr: number[] | undefined) =>
    arr ? { A: arr[0], B: arr[1] } : { A: 0, B: 0 };
...
function decodeObject3D(
  library: GD.DecodeLibrary,
  info: GD.IBaseObjectOrInstanceDecodeInfo | GD.EmitterConfig_T,
): THREE.Object3D {
  switch (info.type) {
    case "Group":
    case "Level":
    case "TerrainInfo":
      return decodeTerrainInfo(library, info as GD.IBaseObjectDecodeInfo);
    case "Emitter":
      return decodeEmitterObject(library, info as GD.IBaseObjectDecodeInfo);
    case "StaticMeshActor":
      return decodeStaticMeshActor(
        library,
        info as GD.IStaticMeshActorDecodeInfo,
      );
    // case "Light": return decodeLight(library, info as GD.ILightDecodeInfo);
    case "TerrainSegment":
      return decodeTerrainSegment(
        library,
        info as GD.IStaticMeshObjectDecodeInfo,
      );
    case "TerrainDecoration":
      return decodeTerrainDecoration(
        library,
        info as GD.ITerrainDecorationDecodeInfo,
      );
    case "Model":
    case "StaticMesh":
      return decodeStaticMeshWrapped(
        library,
        info as GD.IStaticMeshObjectDecodeInfo,
      );
    case "Edges":
      return decodeEdges(library, info as GD.IEdgesObjectDecodeInfo);
    case "SkinnedMesh":
      return decodeSkinnedMesh(
        library,
        info as GD.ISkinnedMeshObjectDecodeInfo,
      );
    case "SpriteEmitter":
      return decodeSpriteEmitter(library, info as GD.ISpriteEmitterDecodeInfo);
    case "MeshEmitter":
      return decodeMeshEmitter(library, info as GD.IMeshEmitterDecodeInfo);
    case "BeamEmitter":
      return decodeBeamEmitter(library, info);
    case "L2FogInfo":
      return decodeFogInfo(library, info as GD.IBaseZoneDecodeInfo);
    case "Zone":
    case "Sky":
    case "SkyZoneInfo":
      return decodeZoneObject(library, info as any);
    default:
      throw new Error(`Unsupported object type: ${info.type}`);
  }
}
