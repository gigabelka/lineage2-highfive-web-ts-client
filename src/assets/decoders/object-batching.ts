...
type StaticMeshIndexCopy_T = {
  source: StaticMeshIndexArray_T;
  target: StaticMeshIndexArray_T;
  offset: number;
};
type StaticMeshBatchJob_T = {
  ...
  decodeObject3D: (
    library: GD.DecodeLibrary,
    info: GD.IBaseObjectOrInstanceDecodeInfo,
  ) => Object3D;
...
export function makeSwayAttribute(
  vertexCount: number,
  info: GD.IStaticMeshSwayDecodeInfo,
  phase: number = 0,
): Float32Array {
...
function createBatchObject(
  ...
  mergedObject.material = canonicalizeStaticMeshMaterials(
    mergedObject.material,
  );
  ...
  if (transparentMats.size > 0) {
    const opaque = mergedGeometry.groups.filter(
      (g) => !transparentMats.has(g.materialIndex),
    );

    mergedGeometry.clearGroups();
    for (const g of opaque)
...
export function decodeStaticMeshActorLight(
  _library: GD.DecodeLibrary,
  info?: GD.ILightInstanceDecodeInfo,
): MeshLight | null {
  if (!info) return null;
...
export function decodeStaticMeshInstance(
  ...
  const infoGeo = {
    ...
    attributes: {
      ...
      ...(sway ? { sway } : {}),
      ...Object.fromEntries(
        Object.keys(info.attributes).map((k: "colors") => [
          `${k}Instance`,
          (info.attributes as any)[k],
        ]),
      ),
    },
  };
...
function createStaticMeshBatchJob(
  ...
  const manifest: StaticMeshBatchManifest_T =
    (library as any).staticMeshBatches ?? buildStaticMeshBatchData(library);
...
function stepStaticMeshBatchJob(job: StaticMeshBatchJob_T): boolean {
  ...
  if (job.indexCopy) {
    ...
    const count = Math.min(
      ...
      Math.max(
        1,
        Math.floor(INDEX_COPY_CHUNK_BYTES / source.BYTES_PER_ELEMENT),
      ),
    );
    ...
    try {
      ...
      const materials =
        decodeStaticMeshMaterial(
          ...
          !!geometryInfo.attributes.colorsInstance,
          !!geometryInfo.attributes.sway,
        ) || (new MeshBasicMaterial({ color: 0xff00ff }) as Material);
      // The merged light matrix has always been identity (see mergeBatchGeometriesData)
      const lightInfo: MeshLight | null = batch.lights
        ? {
            matrix: new Matrix4(),
            scene: batch.lights.scene,
            environment: batch.lights.environment,
          }
        : null;
      ...
      if (source) {
        const target = new (source.constructor as any)(
          source.length,
        ) as StaticMeshIndexArray_T;
      ...
      for (const actor of batch.actors)
        (sector as any).staticMeshMap.set(actor.uuid, batchObject);
  ...
  for (const child of staticMeshGroup.children) {
    if ((child as any).isMovableObject || (child as any).isRotatingObject)
      continue;
...
export function batchStaticMeshActors(
  ...
  const job = createStaticMeshBatchJob(
    library,
    sector,
    staticMeshGroup,
    fetchGeometry,
...
function mergeTerrainGeometries(sectors: Terrain[]) {
  ...
  mergedGeometry.setAttribute(
    "color",
    new BufferAttribute(mergedColors, 3, true),
  );
  mergedGeometry.setAttribute(
    "terrainIndex",
    new BufferAttribute(mergedTerrainIndices, 1),
  );
...
export function batchTerrainSectors(
  ...
  try {
    const { mergedGeometry, batchSectorInfo, totalIndices } =
      mergeTerrainGeometries(sectors);
