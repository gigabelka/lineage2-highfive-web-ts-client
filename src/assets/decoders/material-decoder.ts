...
function getWeakCacheValue<T extends object>(
  cache: Map<string, WeakCacheEntry_T<T>>,
  key: string,
): T | undefined {
...
function setWeakCacheValue<T extends object>(
  cache: Map<string, WeakCacheEntry_T<T>>,
  key: string,
  value: T,
): void {
  cache.set(
    key,
    WeakRefConstructor ? new WeakRefConstructor(value) : { deref: () => value },
  );
}
...
const cacheStaticMaterials = new WeakMap<
  GD.IBaseMaterialDecodeInfo,
  Map<string, THREE.Material | THREE.Material[]>
>();
const cacheStaticMaterialsByName = new Map<
  string,
  WeakCacheEntry_T<Map<string, THREE.Material | THREE.Material[]>>
>();
const canonicalStaticMaterials = new Map<
  string,
  WeakCacheEntry_T<THREE.Material>
>();
const dynamicUniformNames = new Set([
  "ambientLightColor",
  "cameraBillboardRight",
  "cameraBillboardUp",
  "directionalLights",
...
function serializeMaterialValue(value: any, seen: WeakSet<object>): any {
  ...
  if (Array.isArray(value))
    return value.map((entry) => serializeMaterialValue(entry, seen));
  ...
  Object.keys(material.uniforms || {})
    ...
    .forEach((name) => {
      if (!dynamicUniformNames.has(name))
        uniforms[name] = serializeMaterialValue(
...
function canonicalizeStaticMeshMaterials(
...
function decodeStaticMeshMaterial(
  library: DecodeLibrary,
  info: GD.IBaseMaterialDecodeInfo,
  vertexColors: boolean,
  instanced: boolean,
  sway: boolean,
  terrainDecoration: boolean = false,
): THREE.Material | THREE.Material[] {
  if (!info) return null;

  const name = info.name;
  let variants = name
    ? getWeakCacheValue(cacheStaticMaterialsByName, name)
    : cacheStaticMaterials.get(info);
...
function fetchTexture(
  ...
  const name = info?.name;
  const namedTexture = name
    ? getWeakCacheValue(cacheTexturesByName, name)
    : undefined;

  if (namedTexture) return namedTexture;

  if (cacheTextures.has(info)) return cacheTextures.get(info);
  ...
  const data =
    (info as any).materialType !== "empty"
      ? _decodeTexture(library, info)
      : emptyMapData;
...
function fetchMapTexture(
  library: DecodeLibrary,
  info: GD.IBaseMaterialDecodeInfo,
): GD.MapData_T {
  if (!info || info.materialType === "empty")
    return fetchTexture(library, info as GD.ITextureDecodeInfo);
  return decodeParameter(library, info)?.uniforms?.map ?? null;
}

function fetchTransformedMap(
  library: DecodeLibrary,
  materialIndex: string | null,
): { map: GD.MapData_T | null; innerTransforms: any[] } {
  ...
  if (!info || info.materialType === "empty")
    return { map: fetchMapTexture(library, info), innerTransforms: [] };
  ...
  return {
    ...
    innerTransforms: [
      ...nestedTransforms,
      buildTransformStage(
        decoded.transformType as "pan" | "rotate" | "oscillate",
        decoded.uniforms.transform,
      ),
    ],
  };
}

function decodeFadeColorModifier(
  library: DecodeLibrary,
  info: GD.IFadeColorDecodeInfo,
): GD.IDecodedParameter {
...
function decodeTexPannerModifer(
  library: DecodeLibrary,
  info: GD.ITexPannerDecodeInfo,
  ...
  const resolved = isUsingMap
    ? fetchTransformedMap(library, materialIndex)
    : { map: null, innerTransforms: [] };
  ...
  return {
    ...
    uniforms: {
      ...
      transform: {
        ...
        rate: Array.isArray(info.transform.rate)
          ? new Vector2().fromArray(info.transform.rate)
          : info.transform.rate,
        map: materialIndex,
      },
...
function decodeTexRotatorModifer(
  library: DecodeLibrary,
  info: GD.ITexRotatorDecodeInfo,
  overrideMaterial?: string,
): GD.IDecodedParameter {
  const materialIndex =
    overrideMaterial !== undefined ? overrideMaterial : info.transform.map;
  ...
  return {
    ...
    uniforms: {
      ...
      transform: {
        matrix: new Matrix3().fromArray(info.transform.matrix),
        rotation: [
          info.transform.rotation[0],
          info.transform.rotation[1],
          info.transform.rotation[2],
        ],
        offsetU: info.transform.offsetU,
        offsetV: info.transform.offsetV,
        type:
          info.transform.type === "fixed"
            ? 0
            : info.transform.type === "rotating"
              ? 1
              : 2,
...
function decodeTexOscillatorModifer(
  library: DecodeLibrary,
  info: GD.ITexOscillatorDecodeInfo,
  overrideMaterial?: string,
): GD.IDecodedParameter {
  const materialIndex =
    overrideMaterial !== undefined ? overrideMaterial : info.transform.map;
  ...
  return {
    ...
    uniforms: {
      ...
      transform: {
        ...
        typeU:
          info.transform.typeU === "pan"
            ? 0
            : info.transform.typeU === "stretch"
              ? 1
              : info.transform.typeU === "stretchRepeat"
                ? 2
                : 3,
        typeV:
          info.transform.typeV === "pan"
            ? 0
            : info.transform.typeV === "stretch"
              ? 1
              : info.transform.typeV === "stretchRepeat"
                ? 2
                : 3,
        offsetU: info.transform.offsetU,
        offsetV: info.transform.offsetV,
...
function decodeTexEnvMapModifer(
  library: DecodeLibrary,
  info: GD.ITexEnvMapDecodeInfo,
): GD.IDecodedParameter {
  ...
  return {
    ...
    uniforms: {
      map: isUsingMap
        ? fetchMapTexture(library, library.materials[info.map])
        : null,
...
function decodeFinalBlendModifier(
  library: DecodeLibrary,
  info: GD.IFinalBlendDecodeInfo,
  overrideMaterial?: string,
): GD.IDecodedParameter {
  const materialIndex =
    overrideMaterial !== undefined ? overrideMaterial : info.material;
...
function decodeTexCoordSourceModifier(
  library: DecodeLibrary,
  info: GD.ITexCoordSourceDecodeInfo,
): GD.IDecodedParameter {
...
function decodeColorModifier(
  library: DecodeLibrary,
  info: GD.IColorModifierDecodeInfo,
  overrideMaterial?: string,
): GD.IDecodedParameter {
  const materialIndex =
    overrideMaterial !== undefined ? overrideMaterial : info.material;
...
function _decodeModifier(
  library: DecodeLibrary,
  info: GD.IBaseMaterialModifierDecodeInfo,
  overrideMaterial?: string,
): GD.IDecodedParameter {
  let param: GD.IDecodedParameter;
  switch (info.modifierType) {
    case "fadeColor":
      param = decodeFadeColorModifier(library, info as GD.IFadeColorDecodeInfo);
      break;
    case "panTexture":
      param = decodeTexPannerModifer(
        library,
        info as GD.ITexPannerDecodeInfo,
        overrideMaterial,
      );
      break;
    case "rotateTexture":
      param = decodeTexRotatorModifer(
        library,
        info as GD.ITexRotatorDecodeInfo,
        overrideMaterial,
      );
      break;
    case "oscillateTexture":
      param = decodeTexOscillatorModifer(
        library,
        info as GD.ITexOscillatorDecodeInfo,
        overrideMaterial,
      );
      break;
    case "envMapTexture":
      param = decodeTexEnvMapModifer(library, info as GD.ITexEnvMapDecodeInfo);
      break;
    case "colorModifier":
      param = decodeColorModifier(
        library,
        info as GD.IColorModifierDecodeInfo,
        overrideMaterial,
      );
      break;
    case "finalBlend":
      param = decodeFinalBlendModifier(
        library,
        info as GD.IFinalBlendDecodeInfo,
        overrideMaterial,
      );
      break;
    case "texCoordSource":
      param = decodeTexCoordSourceModifier(
        library,
        info as GD.ITexCoordSourceDecodeInfo,
      );
      break;
    default:
      throw new Error(`Unknown modifier type: ${info.modifierType}`);
  }
...
function decodeParameter(
  ...
  switch (info.materialType) {
    case "sprite":
      const decodedSprites = (info as GD.IAnimatedSpriteDecodeInfo).sprites.map(
        (info) => fetchTexture(library, info),
      );
      ...
      break;
    case "modifier":
      param = _decodeModifier(
        library,
        info as GD.IBaseMaterialModifierDecodeInfo,
      );
      break;
    case "texture":
    // "empty" == a texture we couldn't decode (0 mips / unmodelled layout); fetchTexture
    // returns the shared 1x1 transparent EmptyTexture, so the surface still builds.
    case "empty":
      param = {
    ...
    case "shader":
      ...
      param = decodeParameter(
        library,
        library.materials[(info as GD.IShaderDecodeInfo).diffuse],
      );
      break;
    case "combiner":
      ...
      param = decodeParameter(
        library,
        library.materials[(info as GD.ICombinerDecodeInfo).material1],
      );
      break;
    default:
      throw new Error(`Unsupported decoder parameter: ${info.materialType}`);
  }
...
function decodeCombiner(
  library: DecodeLibrary,
  info: GD.ICombinerDecodeInfo,
): MeshStaticMaterial {
...
function decodeShader(
  library: DecodeLibrary,
  info: GD.IShaderDecodeInfo,
): MeshStaticMaterial {
  // D3DTSS_COLOROP = D3DTOP_BLENDCURRENTALPHA (L2.dusk_and_dawn.trace call 9416187, TextureStageState2)
  const useSelfIllumination =
    !info.specular && !!info.selfIllumination && !!info.selfIlluminationMask;
  ...
  return new MeshStaticMaterial({
    ...
    specular: decodeParameter(
      library,
      library.materials[
        useSelfIllumination ? info.selfIllumination : info.specular
      ],
    ),
    specularMask: decodeParameter(
      library,
      library.materials[
        useSelfIllumination ? info.selfIlluminationMask : info.specularMask
      ],
    ),
...
function decodeModifier(
  ...
  if (info.modifierType === "colorModifier")
    materialIndex = (info as GD.IColorModifierDecodeInfo).material;
  else if (info.modifierType === "envMapTexture")
    materialIndex = (info as GD.ITexEnvMapDecodeInfo).map;
  else if (info.modifierType === "panTexture")
    materialIndex = (info as GD.ITexPannerDecodeInfo).transform.map;
  else if (info.modifierType === "rotateTexture")
    materialIndex = (info as GD.ITexRotatorDecodeInfo).transform.map;
  else if (info.modifierType === "oscillateTexture")
    materialIndex = (info as GD.ITexOscillatorDecodeInfo).transform.map;
  else if (info.modifierType === "finalBlend")
    materialIndex = (info as GD.IFinalBlendDecodeInfo).material;

  const baseMaterial = library.materials[
    materialIndex
  ...
  return new MeshStaticMaterial({
    diffuse: _decodeModifier(
      library,
      info,
      isShader ? shader.diffuse : materialIndex,
    ),
    opacity: isShader
      ? _decodeModifier(library, info, shader.opacity)
      : isColorMod && colorMod.alphaBlend
        ? _decodeModifier(library, info, materialIndex)
        : null,
    specular: isShader ? _decodeModifier(library, info, shader.specular) : null,
    specularMask: isShader
      ? _decodeModifier(library, info, shader.specularMask)
      : null,
    side:
      isColorMod && colorMod.doubleSide
        ? DoubleSide
        : isFinalBlend
          ? finalBlend.doubleSide
            ? DoubleSide
            : FrontSide
          : (info as GD.IBaseMaterialDecodeInfo).color
            ? DoubleSide
            : FrontSide,
    blendingMode: isFinalBlend
      ? finalBlend.blendingMode
      : isShader
        ? (shader.blendingMode ?? "normal")
        : "normal",
    transparent: isFinalBlend
      ? finalBlend.transparent
      : isColorMod
        ? colorMod.alphaBlend
        : isShader
          ? shader.transparent
          : false,
    depthWrite: isFinalBlend
      ? finalBlend.depthWrite
      : isShader
        ? shader.depthWrite
        : true,
    depthTest: isFinalBlend
      ? finalBlend.depthTest
      : isShader
        ? (shader.depthTest ?? true)
        : true,
    visible: isShader ? shader.visible : true,
    alphaTest: isFinalBlend
...
function decodeGroup(
  library: DecodeLibrary,
  info: GD.IMaterialGroupDecodeInfo,
): MeshStaticMaterial[] {
  return info.materials.map(
    (info) =>
      decodeMaterial(library, library.materials[info]) as MeshStaticMaterial,
  );
}

function decodeTerrainSegment(
  library: DecodeLibrary,
  info: GD.IMaterialTerrainSegmentDecodeInfo,
) {
  const terrainMaterial = library.materials[
    info.terrainMaterial
  ...
  return new MeshTerrainMaterial({
    ...
    layers: terrainMaterial.layers.map(({ map, alphaMap }) => {
      return {
        ...
        alphaMap: alphaMap
          ? decodeParameter(library, library.materials[alphaMap])
          : null,
      };
    }),
  });
...
function decodeLightmapped(
  library: DecodeLibrary,
  info: GD.ILightmappedDecodeInfo,
) {
  return (
    decodeMaterial(
      library,
      library.materials[info["material"]],
    ) as MeshStaticMaterial
  ).setLightmap(
    fetchTexture(
      library,
      library.materials[info["lightmap"]] as GD.ITextureDecodeInfo,
    ),
  );
}

function applyModAmbient(
  library: DecodeLibrary,
  material: MeshStaticMaterial,
  { color = [1, 1, 1], brightness }: GD.ILightAmbientMaterialModifier,
) {
...
function applyModDirectional(
  ...
  material: MeshStaticMaterial,
  {
    ...
    brightness,
    direction,
  }: GD.ILightDirectionalMaterialModifier,
) {
...
function applyModLighting(
  library: DecodeLibrary,
  material: MeshStaticMaterial,
  modifier: GD.IBaseLightingMaterialModifier,
) {
  switch (modifier.mode) {
    case "Ambient":
      applyModAmbient(
        library,
        material,
        modifier as GD.ILightAmbientMaterialModifier,
      );
      break;
    case "Directional":
      applyModDirectional(
        library,
        material,
        modifier as GD.ILightDirectionalMaterialModifier,
      );
      break;
  }
}
...
function applyModifiers(
  library: DecodeLibrary,
  material: MeshStaticMaterial,
  modifiers: GD.IMaterialModifier[],
) {
    ...
    switch (mod.type) {
      case "Lighting":
        applyModLighting(
          library,
          material,
          mod as GD.IBaseLightingMaterialModifier,
        );
        break;
    }
  });
...
function decodeInstancedMaterial(
  library: DecodeLibrary,
  info: GD.IMaterialInstancedDecodeInfo,
) {
  const materials = decodeMaterial(
    library,
    library.materials[info.baseMaterial],
  );
  const modifiers = info.modifiers.map(
    (uuid) => library.materialModifiers[uuid],
  );

  (materials instanceof Array ? materials : [materials])
    .filter((x) => x)
    .forEach((material) =>
      applyModifiers(library, material as MeshStaticMaterial, modifiers),
    );

  return materials;
}

function decodeSolidColor(
  library: DecodeLibrary,
  info: GD.ISolidMaterialDecodeInfo,
...
function decodeParticleMaterial(
  library: DecodeLibrary,
  info: GD.IParticleMaterialDecodeInfo,
): THREE.Material | THREE.Material[] {
  ...
  if (!baseMaterial)
    throw new Error(`Particle material '${info.material}' not found`);
  ...
  function decodeSprite(
    library: DecodeLibrary,
    info: GD.IAnimatedSpriteDecodeInfo,
  ): any {
    return {
      name: info.name,
      type: "sprite",
  ...
  function decodeMaterial(
    ...
    switch (info.materialType) {
      case "texture":
        return decodeTexture(library, info as GD.ITextureDecodeInfo);
      case "sprite":
        return decodeSprite(library, info as GD.IAnimatedSpriteDecodeInfo);
      case "empty":
        return decodeEmpty();
      ...
      case "shader": {
        ...
        return decodeMaterial(
          library,
          shader.diffuse ? library.materials[shader.diffuse] : null,
        );
      }
      default:
        throw new Error(`Unknown decodable type: ${info.materialType}`);
    }
  }
  ...
  switch (baseMaterial.materialType) {
    case "group":
      return decodeGroup(library, baseMaterial as GD.IMaterialGroupDecodeInfo);
    default:
      return decodeMaterial(library, baseMaterial);
  }
}
...
function decodeMaterial(
  ...
  const material = ((): THREE.Material | THREE.Material[] => {
    switch (info.materialType) {
      case "group":
        return decodeGroup(library, info as GD.IMaterialGroupDecodeInfo);
      case "shader":
        return decodeShader(library, info as GD.IShaderDecodeInfo);
      case "texture":
        return decodeTexture(library, info as GD.ITextureDecodeInfo);
      case "modifier":
        return decodeModifier(
          library,
          info as GD.IBaseMaterialModifierDecodeInfo,
        );
      // case "terrain": return decodeTerrain(library, info as GD.IMaterialTerrainDecodeInfo);
      case "lightmapped":
        return decodeLightmapped(library, info as GD.ILightmappedDecodeInfo);
      case "instance":
        return decodeInstancedMaterial(
          library,
          info as GD.IMaterialInstancedDecodeInfo,
        );
      case "terrainSegment":
        return decodeTerrainSegment(
          library,
          info as GD.IMaterialTerrainSegmentDecodeInfo,
        );
      case "solid":
        return decodeSolidColor(library, info as GD.ISolidMaterialDecodeInfo);
      case "particle":
        return decodeParticleMaterial(
          library,
          info as GD.IParticleMaterialDecodeInfo,
        );
      case "combiner":
        return decodeCombiner(library, info as GD.ICombinerDecodeInfo);
      case "empty":
        return decodeEmptyMaterial();
      default:
        throw new Error(`Unknown decodable type: ${info.materialType}`);
    }
  })();
