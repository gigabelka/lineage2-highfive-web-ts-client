...
function getFormat(type: GD.DataTextureFormats_T) {
  if (typeof type !== "string") return RGBAFormat;
  ...
  switch (type) {
    ...
    case "rgba":
      return RGBAFormat;
    default:
      throw new Error(`Unsupported texture format: ${type}`);
  }
}
...
function decodeCompressedDDS(buffer: ArrayBuffer): THREE.Texture {
  ...
  const format =
    dds.format === RGB_S3TC_DXT1_Format ? RGBA_S3TC_DXT1_Format : dds.format;
  const texture = new CompressedTexture(
    dds.mipmaps as ImageData[],
    dds.width,
    dds.height,
  ...
  texture.minFilter =
    dds.mipmapCount === 1 ? LinearFilter : LinearMipmapLinearFilter;
...
function decodeDDS(
  buffer: ArrayBuffer,
  preferCompressed: boolean = false,
): THREE.Texture {
  if (preferCompressed) {
    try {
      ...
      console.warn(
        "[decodeDDS] compressed upload failed, falling back to software decode:",
        e,
      );
    }
  }
  ...
  try {
    ...
    console.warn(
      "[decodeDDS] Software Decode Failed, falling back to DDSLoader",
      e,
    );
  }
  ...
  const texture = new CompressedTexture(
    mipmaps as ImageData[],
    width,
    height,
    _format as THREE.CompressedPixelFormat,
  );
...
function decodeRGBA(info: GD.IDataTextureDecodeInfo): DataTexture {
  ...
  if (info.buffer.byteLength < byteLength) {
    ...
    console.warn(
      `Texture '${info.name}' has ${info.buffer.byteLength} bytes, expected ${byteLength} - using placeholder`,
    );
...
function decodeTexture(
  ...
  switch (info.textureType) {
    case "dds":
      texture = decodeDDS(
        info.buffer,
        (library as any).preferCompressedTextures === true,
      );
      break;
    case "wet":
      texture = new WetWaterTexture(info as any);
      break;
    case "rgba":
      texture = decodeRGBA(info);
      break;
    case "g16":
      texture = decodeG16(info);
      break;
    case "float":
      texture = decodeFloat(info);
      break;
    default:
      throw new Error(`Unsupported texture format: ${info.textureType}`);
  }
