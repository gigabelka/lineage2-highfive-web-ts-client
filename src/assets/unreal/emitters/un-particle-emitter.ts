...
abstract class UParticleEmitter extends UObject {
  ...
  public setActor(actor: GA.UEmitter) {
    this.actor = actor;
    return this;
  }
    ...
    if (this._particles && this._particles.length > 0)
      if (this.sizeScale?.length === 1)
        ...
        return {
          ...
          rotationOffset: this.rotationOffset?.getQuaternionElements() || [
            0, 0, 0, 1,
          ],
          opacity: this.opacity,
          lifetime: this.lifetimeRange.loadSelf().getDecodeInfo(library),
          fadeIn: this.isFadingIn
            ? {
                time: this.fadeInEndTime,
                color: this.fadeInFactor
                  ?.loadSelf()
                  .getElements() as GD.Vector4Arr,
              }
            : null,
          fadeOut: this.isFadingOut
            ? {
                time: this.fadeOutStartTime,
                color: this.fadeOutFactor
                  ?.loadSelf()
                  .getElements() as GD.Vector4Arr,
              }
            : null,
          ...
          initial: {
            particlesPerSecond: this.isAutomaticInitialSpawning
              ? this.maxParticles / this.lifetimeRange.mid()
              : this.initialParticlesPerSecond,
            scale: this.startSizeRange.loadSelf().getDecodeInfo(library),
            angularVelocity: this.startSpinRange
              ?.loadSelf()
              .getDecodeInfo(library),
            velocity: this.startVelocityRange
              ?.loadSelf()
              .getDecodeInfo(library),
            position: this.startLocationRange
              ?.loadSelf()
              .getDecodeInfo(library),
            offset: this.startLocationOffset?.getElements() || [0, 0, 0],
          },
          colorMultiplierRange: this.colorMultiplierRange
            ?.loadSelf()
            .getDecodeInfo(library),
          revolutionCenterOffsetRange: this.revolutionCenterOffsetRange
            ?.loadSelf()
            .getDecodeInfo(library),
          revolutionsPerSecondRange: this.revolutionsPerSecondRange
            ?.loadSelf()
            .getDecodeInfo(library),
          initialTimeRange: this.initialTimeRange
            ?.loadSelf()
            .getDecodeInfo(library) || [0, 0],
          startMassRange: this.startMassRange
            ?.loadSelf()
            .getDecodeInfo(library) || [0, 0],
          sphereRadiusRange: this.sphereRadiusRange
            ?.loadSelf()
            .getDecodeInfo(library),
          startLocationPolarRange: this.startLocationPolarRange
            ?.loadSelf()
            .getDecodeInfo(library),
          addVelocityMultiplierRange: this.addVelocityMultiplierRange
            ?.loadSelf()
            .getDecodeInfo(library),
          velocityLossRange: this.velocityLossRange
            ?.loadSelf()
            .getDecodeInfo(library),
          forcedMaxParticles: this._forcedMaxParticles,
          particlesPerSecond: this.particlesPerSecond,
          angularVelocity: this.spinsPerSecondRange
            ?.loadSelf()
            .getDecodeInfo(library),
          blendingMode:
            blendingNames[this.drawStyle.valueOf() as EParticleDrawStyle_T],
          changesOverLifetime: {
            scale:
              this.isUsingSizeScale && (this.sizeScale?.length ?? 0) > 1
                ? {
                ...
                : null,
            color:
              this.isUsingColorScale && (this.colorScale?.length ?? 0) > 1
                ? {
                    values: this.colorScale.map((s) =>
                      s.getDecodeInfo(library),
                    ),
                    repeats: this.colorScaleRepeats,
                  }
                : null,
            velocity:
              this.isUsingVelocityScale && (this.velocityScale?.length ?? 0) > 1
                ? {
                    values: this.velocityScale.map((s) =>
                      s.getDecodeInfo(library),
                    ),
                ...
                : null,
            revolution:
              this.isUsingRevolutionScale &&
              (this.revolutionScale?.length ?? 0) > 1
                ? {
                    values: this.revolutionScale.map((s) =>
                      s.getDecodeInfo(library),
                    ),
  ...
  protected getSettingsSnapshot(
    ...
    for (const varName of REQUIRED_SETTINGS) {
      ...
      const value = enumNames
        ? (enumNames[raw?.valueOf() ?? 0] ?? enumNames[0])
        : toCloneSafeSetting(raw, library);
...
const REQUIRED_SETTINGS = [
  ...
  "isAutomaticInitialSpawning",
  "isDisabled",
  "isFadingIn",
  "isFadingOut",
  "isRespawningDeadParticles",
  "isScaleSizeRegular",
  "isSpawningTowardsNormal",
  "isSpinning",
  "isUniformScale",
  "isUsingCollision",
  "isUsingColorFromMesh",
  "isUsingColorScale",
  "isUsingRandomSubdiv",
  "isUsingRevolution",
  "isUsingRevolutionScale",
  "isUsingSizeScale",
  "isUsingVelocityScale",
  "isVelocityFromMesh",
  "maxAbsVelocity",
  "meshNormal",
  "meshScaleRange",
  "meshSpawning",
  "rotateVelocityLossRange",
  "rotationNormal",
  "rotationSource",
  "sizeScaleRepeats",
...
function toCloneSafeSetting(value: any, library: GD.DecodeLibrary): any {
  ...
  if (t === "object") {
    ...
    if (
      value instanceof FVector ||
      value instanceof UPlane ||
      value instanceof FRange ||
      value instanceof FRangeVector
    )
...
abstract class UParticleTimeScale extends UObject {
  ...
  public getDecodeInfo(library: GD.DecodeLibrary): [number, number] {
    return [this.relTime, this.relSize];
  }

  public toString() {
    return `ParticleTimeScale=(time=${this.relTime.toFixed(2)}, size=${this.relSize})`;
  }
}
...
abstract class UParticleSound extends UObject {
  ...
  public getDecodeInfo(
    builder: GD.DecodeLibraryBuilder,
  ): GD.IParticleSoundDecodeInfo | null {
    ...
    return {
      ...
      probability: this.probability?.loadSelf().getDecodeInfo(library) ?? [
        1, 1,
      ],
...
abstract class UParticleColorScale extends UObject {
  ...
  public getDecodeInfo(_library: GD.DecodeLibrary): [number, GD.Vector4Arr] {
    return [
      this.relTime,
      (this.color?.toArray() as GD.Vector4Arr) || [255, 255, 255, 255],
    ];
  }
}
...
export {
  UParticleEmitter,
  UParticleRevolutionScale,
  UParticleTimeScale,
  UParticleSound,
  UParticleVelocityScale,
  UParticle,
  UParticleColorScale,
  UParticleBeamEndPoint,
  UParticleBeamScale,
};
...
const blendingNames = {
  ...
  [EParticleDrawStyle_T.PTDS_AlphaModulate_MightNotFogCorrectly]:
    "alphaModulate",
...
const ENUM_SETTING_NAMES: Record<string, Record<number, string>> = {
  ...
  getVelocityDirectionFrom: {
    ...
    [EParticleVelocityDirection_T.PTVD_StartPositionAndOwner]:
      "startPositionAndOwner",
    [EParticleVelocityDirection_T.PTVD_OwnerAndStartPosition]:
