...
type StaticMeshDecodeResult_T = {
  object: GD.IStaticMeshObjectDecodeInfo;
...
abstract class UStaticMesh extends UPrimitive {
  ...
  public doLoad(pkg: C.APackage, exp: C.UExport) {
    ...
    if (verLicense < 17) {
      ...
      if (verArchive < 62) {
        ...
        this.collisionFaces = new FArrayLazy(FStaticMeshCollisionTriangle).load(
          pkg,
        );
        this.collisionNodes = new FArrayLazy(FStaticMeshCollisionNode).load(
          pkg,
        );
  ...
  public getDecodeInfo(
    ...
    for (let i = 0; i < collisionFaces; i++) {
      ...
      // winding is (v2,v1,v0): only that order reproduces the triangle's own stored plane, which
      // UStaticMesh::LineCheck's node traversal signs its front/back descent against
    ...
    // UStaticMesh's own collision BSP: 4 int32s per node (back, front, coplanar, triangle) plus a
    // float32 min/max box. `queryStaticNode` in collision-primitive.ts walks exactly this layout.
    ...
    const geometryInfo: GD.IGeometryDecodeInfo = {
      ...
      staticMeshCollision: {
        ...
        collisionModel: this.collisionModel
          ? this.collisionModel.loadSelf().getCollisionModelDecodeInfo()
          : null,
    ...
    // const materials = await Promise.all(this.materials.map((mat: UStaticMeshMaterial) => mat.getDecodeInfo(library)));
    const materials = this.materials.map((mat: GA.UStaticMeshMaterial) =>
  ...
  protected getDecodeTrisInfo(): {
    object: GD.IBaseObjectDecodeInfo;
    uuid: string;
    geometry: GD.IGeometryDecodeInfo;
  } {
    ...
    for (let i = 0, len = trisCount; i < len; i++) {
      const indOffset = i * 4;
      const vIndOffset = i * 3,
        vertOffset = vIndOffset * 3;
