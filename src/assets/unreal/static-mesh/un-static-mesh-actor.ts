...
type StaticMeshActorDecodeResult_T = {
  object: GD.IStaticMeshActorDecodeInfo;
  leafIndices: number[];
  zoneUuid: string;
  geometryUuid: string;
...
abstract class UStaticMeshActor extends UAActor {
  ...
  public getDecodeInfo(
  ...
  ): StaticMeshActorDecodeResult_T {
    const library = builder.library;

    if (!this.mesh) {
      console.warn(
        `StaticMeshActor '${this.objectName}' has no static mesh, skipping`,
      );
      return null;
    }
    ...
    const isMoverWithoutDynamicLight =
      this.physics === EPhysics_T.PHYS_MovingBrush && !this.isDynamicLightMover;

    if (!isStatic && !isMoverWithoutDynamicLight) {
      return this.getActorDecodeResult(
        library,
        meshInfo,
        null,
        predictedBox,
    ...
    const leaves: GA.FLeaf[] = baseModel
      ? baseModel.boxLeaves(predictedBox)
      : [];
    const instance = this.instance
      ? this.instance.getDecodeInfo(library)
      : null;
    ...
    if (!this.isSunAffected) {
      if (leaves.length > 0) {
        for (const leaf of leaves) {
          // seems that precalculated may be wrong for some objects and need to re-calc from zone, already had this regression, not sure why i gone back to using zone vector
    ...
    const ambVector = FColor.fromFloating(
      ambX,
      ambY,
      ambZ,
    ).toArray() as number[];
  ...
  protected getActorDecodeResult(
    library: GD.DecodeLibrary,
    meshInfo: GD.IStaticMeshObjectDecodeInfo,
    instanceColors: Float32Array | Uint8Array | null,
    predictedBox: GA.FBox,
    ambient: { glow: number; vector: number[]; isUnlit: boolean },
    lights?: GD.ILightInstanceDecodeInfo,
  ): StaticMeshActorDecodeResult_T {
    this.instance?.loadSelf().setActor(this);
    ...
    if (!geometryInfo) {
      console.warn(
        `Geometry info not found for meshInfo.geometry: ${meshInfo.geometry}, actor: ${this.objectName}`,
      );
      return null;
    }
    ...
    const rotating =
      this.physics === EPhysics_T.PHYS_Rotating &&
      this.isFixedRotationDir &&
      this.rotationRate &&
      (this.rotationRate.pitch !== 0 ||
        this.rotationRate.yaw !== 0 ||
        this.rotationRate.roll !== 0)
        ? ({
            rotator: [
              this.rotation.pitch,
              this.rotation.yaw,
            ...
            rate: [
              this.rotationRate.pitch,
              this.rotationRate.yaw,
              this.rotationRate.roll,
            ],
          } as GD.IRotatingDecodeInfo)
        : undefined;
    ...
    const actorInfo = {
      ...
      scale: this.scale?.multiplyScalar(this.drawScale).getElements() || [
        1, 1, 1,
      ],
    ...
    const inflatedBox = FBox.make(
      predictedBox.min.sub(margin),
      predictedBox.max.add(margin),
      1,
    );
    ...
    return {
      ...
      leafIndices,
      zoneUuid: zone.uuid,
      geometryUuid: meshInfo.geometry,
      zoneBounds,
    };
  }
}
