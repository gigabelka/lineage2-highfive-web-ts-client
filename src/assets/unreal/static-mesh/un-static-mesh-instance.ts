...
abstract class UStaticMeshInstance extends UObject {
  ...
  public setActor(actor: GA.UStaticMeshActor) {
    this.actor = actor;
    return this;
  }
  ...
  public getDecodeInfo(library: GD.DecodeLibrary): {
    ...
    for (const l of this.sceneLights)
      totalLength += l.vertexFlags.getElemCount();
    for (const l of this.environmentLights)
      totalLength += l.vertexFlags.getElemCount();
    ...
    for (const [lights, container] of [
      ...
      for (const light of lights) {
        const arr = light.vertexFlags.getTypedArray(),
          arrLen = arr.length;
  ...
  protected doLoad(pkg: C.APackage, exp: C.UExport): this {
    ...
    if (0x0b < verLicense)
      this.unkArrIndex = new Array(2).fill(1).map((_) => pkg.read("compat32"));
