  ...
  public getDecodeInfo(
    ...
    this.emitters.loadSelf().forEach((emitter) => {
      ...
      if (!isParticleEmitter(emitter)) {
        console.warn(
          `Emitter '${this.objectName}' skipping unsupported sub-emitter '${emitter.objectName}'`,
        );
        return;
      }

      if (
        this.subEmitterFilter &&
        !this.subEmitterFilter.includes(emitter.objectName)
      )
        return;
    ...
    if (baseModel) {
      leafIndices = baseModel.boxLeavesRecursive(
        0,
        worldOrigin,
        FVector.make(0, 0, 0),
      );
