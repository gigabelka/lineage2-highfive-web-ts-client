...
abstract class USkeletalMesh extends ULodMesh {
  ...
  public getDecodeInfo(
    ...
    const section = this;
    const { positions, uvs, bones, weights } = convertWedges(
      section.points,
      section.wedges,
      section.vertexInfluences,
    );
    const { indices, groups } = buildIndices(
      section.faces,
    ...
    if (this.animation) {
      ...
      for (let i = 0; i < boneCount; i++) {
        ...
        for (let j = 0, len = refBones.length; j < len; j++) {
          ...
          if (boneSkeleton.boneName !== boneAnim.boneName) continue;
...
function fixVector(v: FVector) {
  return new FVector(v.x, v.z, v.y);
}
function fixRotation(v: FQuaternion) {
  return new FQuaternion(v.x, v.z, v.y, v.w);
}
