...
/** rotation-shake kinds, UE2 Engine.EViewShakeType */
...
function requireNotifyProperty<T>(
  notify: UObject,
  name: string,
  value: T | undefined,
): T {
  if (value === undefined)
    throw new Error(
      `Animation notify '${notify.objectName}' has undefined mapped property '${name}'.`,
    );
...
/**
 * UE2 `UAnimNotify` hierarchy, the objects `FMeshAnimNotify.notifyObjectId` points at.
 * Only the subclasses that carry data the client acts on override `getDecodeInfo`; every
 * other notify decodes to a `native` marker holding its class name.
 */
abstract class UAnimNotify extends UObject {
  public getDecodeInfo(
    _builder: GD.DecodeLibraryBuilder,
  ): GD.IAnimationNotifyObjectDecodeInfo {
    return {
      type: "native",
      className: (this.constructor as any).friendlyName,
...
abstract class UAnimNotifySound extends UAnimNotify {
  ...
  public getDecodeInfo(
    builder: GD.DecodeLibraryBuilder,
  ): GD.IAnimationSoundNotifyDecodeInfo {
    return {
      type: "sound",
      ...
      defaultWalkSounds: pullSounds(
        builder,
        requireNotifyProperty(this, "defaultWalkSounds", this.defaultWalkSounds),
      ),
      defaultRunSounds: pullSounds(
        builder,
        requireNotifyProperty(this, "defaultRunSounds", this.defaultRunSounds),
      ),
      grassWalkSounds: pullSounds(
        builder,
        requireNotifyProperty(this, "grassWalkSounds", this.grassWalkSounds),
      ),
      grassRunSounds: pullSounds(
        builder,
        requireNotifyProperty(this, "grassRunSounds", this.grassRunSounds),
      ),
      waterWalkSounds: pullSounds(
        builder,
        requireNotifyProperty(this, "waterWalkSounds", this.waterWalkSounds),
      ),
      waterRunSounds: pullSounds(
        builder,
        requireNotifyProperty(this, "waterRunSounds", this.waterRunSounds),
      ),
      defaultActorWalkSounds: pullSounds(
        builder,
        requireNotifyProperty(
          this,
          "defaultActorWalkSounds",
          this.defaultActorWalkSounds,
        ),
      ),
      defaultActorRunSounds: pullSounds(
        builder,
        requireNotifyProperty(
          this,
          "defaultActorRunSounds",
          this.defaultActorRunSounds,
        ),
      ),
    };
  }
...
/* the swim profile itself lives in alaudio.int, the engine pulls it in later (see
   DecodeEngine.pullAnimationNotifyAssets) - decode leaves the sets null */
abstract class UAnimNotifySwimSound extends UAnimNotify {
  public getDecodeInfo(
    _builder: GD.DecodeLibraryBuilder,
  ): GD.IAnimationSwimSoundNotifyDecodeInfo {
    return {
      type: "swimSound",
...
abstract class UAnimNotifyEffect extends UAnimNotify {
  ...
  public getDecodeInfo(
    _builder: GD.DecodeLibraryBuilder,
  ): GD.IAnimationEffectNotifyDecodeInfo {
    const effectClass = requireNotifyProperty(
      this,
      "effectClass",
      this.effectClass,
    );
    ...
    return {
      ...
      objectName: this.objectName,
      effectClass: effectClass ? effectClass.objectName : null,
      bone: requireNotifyProperty(this, "bone", this.bone),
      offsetLocation: requireNotifyProperty(
        this,
        "offsetLocation",
        this.offsetLocation,
      ).getElements(),
      offsetRotation: requireNotifyProperty(
        this,
        "offsetRotation",
        this.offsetRotation,
      ).toArray() as GD.Vector3Arr,
      ...
      drawScale3D: requireNotifyProperty(
        this,
        "drawScale3D",
        this.drawScale3D,
      ).getElements(),
      trailCamera: requireNotifyProperty(
        this,
        "trailCamera",
        this.trailCamera,
      ),
      independentRotation: requireNotifyProperty(
        this,
        "independentRotation",
        this.independentRotation,
      ),
...
abstract class UAnimNotifyScreenFade extends UAnimNotify {
  ...
  public getDecodeInfo(
    _builder: GD.DecodeLibraryBuilder,
  ): GD.IAnimationScreenFadeNotifyDecodeInfo {
    return {
      type: "screenFade",
      className: "AnimNotify_ScreenFade",
      objectName: this.objectName,
      fadeOutDuration: requireNotifyProperty(
        this,
        "fadeOutDuration",
        this.fadeOutDuration,
      ),
      fadeOutColor: requireNotifyProperty(
        this,
        "fadeOutColor",
        this.fadeOutColor,
      ).toArray() as GD.Vector4Arr,
      blackOutDuration: requireNotifyProperty(
        this,
        "blackOutDuration",
        this.blackOutDuration,
      ),
      fadeInDuration: requireNotifyProperty(
        this,
        "fadeInDuration",
        this.fadeInDuration,
      ),
...
abstract class UAnimNotifyViewShake extends UAnimNotify {
  ...
  public getDecodeInfo(
    _builder: GD.DecodeLibraryBuilder,
  ): GD.IAnimationViewShakeNotifyDecodeInfo {
    const names: GD.IAnimationViewShakeNotifyDecodeInfo["shakeType"][] = [
      "damage",
      "vibration",
      "user",
      "up",
    ...
    const type = requireNotifyProperty(
      this,
      "shakeType",
      this.shakeType,
    ).valueOf();
    ...
    if (!shakeType)
      throw new Error(
        `Animation notify '${this.objectName}' has invalid shake type '${type}'.`,
      );
    ...
    return {
      ...
      objectName: this.objectName,
      shakeType,
      shakeIntensity: requireNotifyProperty(
        this,
        "shakeIntensity",
        this.shakeIntensity,
      ),
      shakeVector: requireNotifyProperty(
        this,
        "shakeVector",
        this.shakeVector,
      ).getElements(),
...
export {
  UAnimNotify,
  UAnimNotifyIdleSound,
  UAnimNotifyMatSubAction,
  UAnimNotifyScripted,
  UAnimNotifyScript,
  UAnimNotifySound,
  UAnimNotifySwimSound,
  UAnimNotifyDestroyEffect,
  UAnimNotifyEffect,
  UAnimNotifyAttackVoice,
  UAnimNotifyChanneling,
  UAnimNotifyAttackPreShot,
  UAnimNotifyAttackShot,
  UAnimNotifyAttackItem,
  UAnimNotifyScreenFade,
  UAnimNotifyViewShake,
  UAnimNotifyBoneScale,
  EViewShakeType_T,
};
