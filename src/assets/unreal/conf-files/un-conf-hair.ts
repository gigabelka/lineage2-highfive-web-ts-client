...
export type IDynamicHairCollisionPlaneDecodeInfo = {
  bone: string;
  distance: number;
};
export type IDynamicHairCollisionSphereDecodeInfo = {
  bone: string;
  offset: GD.Vector3Arr;
  radius: number;
};
export type IDynamicHairActionDecodeInfo = {
  name: string;
  initial: boolean;
  initialOffset: GD.Vector3Arr;
...
function takeNumber(
  ...
  if (!Number.isFinite(value))
    throw new Error(`'[${section}].${name}' is not a number.`);
...
function takeCount(
  ...
  if (!Number.isInteger(value) || value < 0)
    throw new Error(`'[${section}].${name}' is not a count.`);
...
function takeVector(
  ...
  const match =
    /^\(\s*X\s*=\s*([^,]+),\s*Y\s*=\s*([^,]+),\s*Z\s*=\s*([^\)]+)\s*\)$/i.exec(
      value,
    );
  ...
  if (!vector.every(Number.isFinite))
    throw new Error(`'[${section}].${name}' is not a vector.`);
...
function parseSection(
  ...
  for (const line of lines) {
    ...
    if (values.has(name))
      throw new Error(
        `Duplicate '[${section}].${trimmed.slice(0, index).trim()}'.`,
      );

    values.set(name, trimmed.slice(index + 1).trim());
  }

  const drawCollisionObject = takeBoolean(
    values,
    section,
    "bDrawCollisionObject",
  );
  ...
  for (let i = 0; i < planeCount; i++) {
    ...
    planes[i] = {
      bone: take(values, section, `CollisionPlaneBone${index}`),
      distance: takeNumber(values, section, `CollisionPlaneDist${index}`),
    };
  }
  ...
  for (let i = 0; i < sphereIndexCount; i++) {
    ...
    if (index >= sphereCount)
      throw new Error(
        `'[${section}].SphereIndex${i + 1}' references sphere '${index}' of '${sphereCount}'.`,
      );
  ...
  if (values.size > 0)
    throw new Error(
      `'[${section}]' has unconsumed key '${values.keys().next().value}'.`,
    );
...
/* system/Hair.int: per-hair-mesh spring parameters, keyed by the hair mesh's own
   section or by `<hairMesh>.<bodyMesh>` when the behaviour depends on the body it sits on. */
export class UConfigHair extends BaseConfigFile {
  ...
  public load(): this {
      ...
      if (match) {
        if (section) {
          ...
          if (this.sections.has(key))
            throw new Error(`Duplicate hair section '[${section}]'.`);
    ...
    if (this.sections.size === 0)
      throw new Error(`'${this.path}' has no hair sections.`);
  ...
  public getDecodeInfo(
    ...
    if (this.sections.size === 0)
      throw new Error(`'${this.path}' was not loaded.`);

    const composite = bodyMesh ? `${hairMesh}.${bodyMesh}` : null;
    const info = composite
      ? this.sections.get(composite.toLowerCase()) ||
        this.sections.get(hairMesh.toLowerCase())
      : this.sections.get(hairMesh.toLowerCase());

    if (!info)
      throw new Error(
        `Hair config has no '[${composite}]' or '[${hairMesh}]' section.`,
      );
