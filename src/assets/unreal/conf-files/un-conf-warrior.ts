...
export class UConfigWarrior extends BaseConfigFile {
  ...
  public async load(): Promise<this> {
    ...
    for (const section of sections) {
      ...
      for (const field of Object.keys(ANIM_KEYS) as (keyof WarriorAnimations_T)[]) {
        const match = new RegExp(`^${ANIM_KEYS[field]}\\[0\\]=(.+)$`, "m").exec(
          section,
        );
      ...
      if (animations.wait)
        this.classAnimations.set(className.toLowerCase(), animations);
    }

    if (this.classAnimations.size === 0)
      throw new Error(`'${this.path}' declared no playable classes.`);
  ...
  public getAnimations(className: string): WarriorAnimations_T {
    ...
    if (!animations)
      throw new Error(`'${this.path}' has no '${className}' class.`);

    return animations;
  }

  public getClassNames(): string[] {
    return [...this.classAnimations.keys()];
  }
}
