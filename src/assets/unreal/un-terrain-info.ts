...
import {
  ...
  dxt3ToRgba,
  dxt5ToRgba,
} from "@client/assets/decoders/dxt-decode";

type TerrainInfoDecodeResult_T = {
  object: GD.IBaseObjectDecodeInfo & { children: GD.IBaseObjectDecodeInfo[] };
  material: GD.IMaterialTerrainDecodeInfo;
  zoneUuid: string;
};
...
function getTextureRgba(texture: GA.UTexture): Uint8Array {
  ...
  const data = texture.mipmaps
    .getElem(0)
    .dataArray.getTypedArray() as Uint8Array;
  let rgba: Uint8Array;

  switch (texture.format) {
    case ETextureFormat.TEXF_DXT1:
      rgba = dxt1ToRgba(texture.width, texture.height, data);
      break;
    case ETextureFormat.TEXF_DXT3:
      rgba = dxt3ToRgba(texture.width, texture.height, data);
      break;
    case ETextureFormat.TEXF_DXT5:
      rgba = dxt5ToRgba(texture.width, texture.height, data);
      break;
    default:
      return null;
  }
...
abstract class ATerrainInfo extends AInfo {
  ...
  public getFromBitmap(
    bitmap: FPrimitiveArray<"int32">,
    x: number,
    y: number,
  ): boolean {
  ...
  public getQuadVisibilityBitmap(x: number, y: number): boolean {
    return this.getFromBitmap(this.quadVisibilityBitmap, x, y);
  }
  public getQuadVisibilityBitmapOrig(x: number, y: number): boolean {
    return this.getFromBitmap(this.quadVisibilityBitmapOrig, x, y);
  }
  public getEdgeTurnBitmap(x: number, y: number): boolean {
    return this.getFromBitmap(this.edgeTurnBitmap, x, y);
  }
  public getEdgeTurnBitmapOrig(x: number, y: number): boolean {
    return this.getFromBitmap(this.edgeTurnBitmapOrig, x, y);
  }
  public isInvertedTerrain() {
  ...
  public getLayerAlpha(
    ...
    y: number,
    layer: number,
    alphaMap: GA.UTexture,
  ) {
    const texture = alphaMap
      ? alphaMap
      : layer === -1
        ? this.terrainMap
        : this.layers[layer].alphaMap;
    ...
    switch (texture.format) {
      case ETextureFormat.TEXF_L8:
        return data.getElem(offset);
      case ETextureFormat.TEXF_P8:
        return texture.palette.loadSelf().colors.getElem(data.getElem(offset))
          .r;
      case ETextureFormat.TEXF_RGB8:
      case ETextureFormat.TEXF_RGBA8:
        return data.getElem(offset * 4 + 3);
      case ETextureFormat.TEXF_DXT1:
      case ETextureFormat.TEXF_DXT3:
      case ETextureFormat.TEXF_DXT5:
        return getTextureRgba(texture)[offset * 4];
      default:
        throw new Error("invalid");
    }
  }

  public getTextureColor(
    x: number,
    y: number,
    texture: GA.UTexture,
  ): GD.Vector3Arr {
    ...
    switch (texture.format) {
      ...
      case ETextureFormat.TEXF_RGBA8:
        return [
          data.getElem(offset + 2) / 255,
          data.getElem(offset + 1) / 255,
          data.getElem(offset) / 255,
        ];
      case ETextureFormat.TEXF_DXT1:
      case ETextureFormat.TEXF_DXT3:
      case ETextureFormat.TEXF_DXT5: {
  ...
  public getRenderCombination(
    layers: number[],
    method: ETerrainRenderMethod_T,
  ): number {
    // TODO: this can be rewriten without labels
    nextCombo: for (
      let i = 0, len = this.renderCombinations.length;
      i < len;
      i++
    ) {
  ...
  protected calcLayerTexCoords() {
    ...
    for (let i = 0, lcount = this.layers.length; i < lcount; i++) {
      ...
      if (
        layer.alphaMap &&
        layer.alphaMap.loadSelf().format === ETextureFormat.TEXF_P8
      ) {
      ...
      const addOrigin = FVector.make(
        layer.panW * layer.scaleW,
        layer.panH * layer.scaleH,
        0,
      ).transformVectorBy(this.toWorld);
      ...
      switch (layer.mapAxis) {
        ...
        case TextureMapAxis_T.TEXMAPAXIS_XZ:
          ...
          [texCoords.origin.y, texCoords.origin.z] = [
            texCoords.origin.z,
            texCoords.origin.y,
          ];
          [texCoords.origin.x, texCoords.origin.z] = [
            texCoords.origin.z,
            texCoords.origin.x,
          ];
          [texCoords.xAxis.x, texCoords.xAxis.z] = [
            texCoords.xAxis.z,
            texCoords.xAxis.x,
          ];
          [texCoords.yAxis.x, texCoords.yAxis.y] = [
            texCoords.yAxis.y,
            texCoords.yAxis.x,
          ];
          [texCoords.zAxis.y, texCoords.zAxis.z] = [
            texCoords.zAxis.z,
            texCoords.zAxis.y,
          ];
          break;
        case TextureMapAxis_T.TEXMAPAXIS_YZ:
          ...
          [texCoords.zAxis.x, texCoords.zAxis.z] = [
            texCoords.zAxis.z,
            texCoords.zAxis.x,
          ];
          break;
  ...
  public getGlobalVertex(x: number, y: number) {
    return x + y * this.heightmapX;
  }
  protected heightmapToWorld(H: FVector) {
    return H.transformPointBy(this.toWorld);
  }
  protected getHeightmap(x: number, y: number) {
    x = Math.min(x, this.heightmapX - 1);
    y = Math.min(y, this.heightmapY - 1);

    // heightmap texture failed to provide mip data (e.g. unmodelled 0x100 layout) -> flat
    if ((this.terrainMap?.mipmaps?.getElemCount() ?? 0) <= 0) return 0;
    ...
    switch (this.terrainMap.format) {
      ...
      case ETextureFormat.TEXF_P8:
        return (
          256 * this.terrainMap.mipmaps[0].dataArray.getElem(x + y * width)
        );
      case ETextureFormat.TEXF_RGB8:
  ...
  protected updateVertices(
    ...
    for (let i = 0; i < vCount; i++) faceNormals[i] = new FTerrainNormalPair();

    //

    for (let y = startY; y < endY; y++) {
      for (let x = startX; x < endX; x++) {
        vertices[this.getGlobalVertex(x, y)] = this.heightmapToWorld(
          FVector.make(x, y, this.getHeightmap(x, y)),
        );
        ...
        if (x > 0 && y > 0) {
          ...
          if (this.getEdgeTurnBitmapOrig(x - 1, y - 1)) {
            // 124, 423
            faceNormal.normal1 = FPlane.fromPoints(
              vertices[this.getGlobalVertex(x - 1, y - 1)],
              vertices[this.getGlobalVertex(x, y - 1)],
              vertices[this.getGlobalVertex(x - 1, y)],
            )
              .vector()
              .normalized();
            faceNormal.normal2 = FPlane.fromPoints(
              vertices[this.getGlobalVertex(x - 1, y)],
              vertices[this.getGlobalVertex(x, y - 1)],
              vertices[this.getGlobalVertex(x, y)],
            )
              .vector()
              .normalized();
          } else {
  ...
  public getDecodeInfo(
    ...
    for (let k = 0; k < layerCount; k++) {
      ...
      if (
        layer.map?.loadSelf().mipmaps?.getElemCount() === 0 &&
        layer.alphaMap?.loadSelf().mipmaps?.getElemCount() === 0
      ) {
        layers[k] = { map: null, alphaMap: null };
        continue;
      }
    ...
    const children: GD.IBaseObjectDecodeInfo[] = sectors.map((sector) =>
      builder.pullTerrainSector(sector, this, heightmapData),
    );
    ...
    this.decoLayers?.forEach((layer) => {
      ...
      const decorations = layer
        .loadSelf()
        .getDecodeInfo(builder, this, sectors, this.decoLayerOffset || 0);
      children.push(...decorations);
    });

    const decodeInfo: GD.IBaseObjectDecodeInfo & {
      children: GD.IBaseObjectDecodeInfo[];
    } = {
      uuid: this.uuid,
      type: "TerrainInfo",
