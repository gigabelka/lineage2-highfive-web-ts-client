...
abstract class UTexture extends UMaterial {
  ...
  public isTransparent() {
    return this.isAlphaTexture || this.isMasked;
  }
  ...
  public doLoad(pkg: C.APackage, exp: C.UExport) {
    ...
    if (
      mipLoadError ||
      (this.mipmaps.length === 0 && this.readTail - this.readHead > 0)
    ) {
      console.warn(
  ...
  protected getTexturePixelFormat(): ETexturePixelFormat {
    switch (this.format.valueOf()) {
      ...
      case ETextureFormat.TEXF_G16:
        return ETexturePixelFormat.TPF_G16;
      default:
        throw new Error(`Unknown UE2 pixel format: ${this.format}`);
    }
  }
  ...
  private loadPrependedMipArray(pkg: C.APackage, startPos: number): boolean {
    ...
    const off = locatePrependedMipArray(
      dv,
      startPos,
      readTail,
      this.width | 0,
  ...
  protected decodeTexture(
    library: GD.DecodeLibrary,
  ): GD.ITextureDecodeInfo | GD.IBaseMaterialDecodeInfo {
    ...
    const lastMipmap = loadMipmaps
      ? this.mipmaps[totalMipCount - 1]
      : firstMipmap;
    const insertMipmap = loadMipmaps
      ? lastMipmap.sizeW !== 1 || lastMipmap.sizeH !== 1
      : false;
    const levelsToInsert = loadMipmaps
      ? Math.log2(Math.max(lastMipmap.sizeW, lastMipmap.sizeH))
      : 0;
    ...
    for (
      let i = levelsToInsert - 1, w = lastMipmap.sizeW, h = lastMipmap.sizeH;
      i >= 0;
      i--
    ) {
      w = Math.max(1, w / 2);
      ...
      const dataLength =
        (((Math.max(4, w) / 4) * Math.max(4, h)) / 4) * blockSize;
    ...
    if (insertMipmap) {
      ...
      for (
        let j = levelsToInsert - 1, w = lastMipmap.sizeW, h = lastMipmap.sizeH;
        j >= 0;
        j--
      ) {
        w = Math.max(1, w / 2);
    ...
    const width = firstMipmap.sizeW,
      height = firstMipmap.sizeH;
    ...
    if (typeof this.totalFrameNum === "number" && this.totalFrameNum > 1) {
      ...
      for (let i = 0, len = this.totalFrameNum; i < len && tex; i++) {
        sprites.push(
          tex
            .loadSelf()
            .decodeTexture(builder.library) as GD.ITextureDecodeInfo,
        );
        tex = tex.animNext;
      }
