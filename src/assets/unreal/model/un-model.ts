...
import FArray, {
  FObjectArray,
  FPrimitiveArray,
} from "@l2js/core/src/unreal/un-array";
...
abstract class UModel extends UPrimitive {
  ...
  public setLevelInfo(levelInfo: GA.ULevelInfo) {
  ...
  public getIsRootOutside() {
    return this.isRootOutside;
  }
  ...
  protected doLoad(pkg: C.APackage, exp: C.UExport): this {
    ...
    this.isSky = pkg.path
      .toLowerCase()
      .replace(/\\/g, "/")
      .endsWith("assets/maps/skylevel.unr");

    return this;
  }

  public getZoneActor(iZone: number) {
    return this.zones[iZone].zoneActor ?? this.levelInfo;
  }

  public boxLeavesRecursive(
    iNode: number,
    origin: GA.FVector,
    extent: GA.FVector,
    outLeaves?: number[],
    ...
    if (min * 2 > max) {
      ...
      while (index >= 0) {
        ...
        if (distance < pushOut) {
          if (node.iBack !== -1) iNodes[++index] = node.iBack;
          else if (node.iLeaf[0] !== -1) outLeaves.push(node.iLeaf[0]);
        }

        if (-distance < pushOut) {
          if (node.iFront !== -1) iNodes[++index] = node.iFront;
          else if (node.iLeaf[1] !== -1) outLeaves.push(node.iLeaf[1]);
        }
      }
    } else {
      while (index >= 0) {
  ...
  public boxLeaves(box: GA.FBox): GA.FLeaf[] {
    ...
    return this.boxLeavesRecursive(0, origin, extent).map(
      (i) => this.leaves[i],
    );
  }
  ...
  public isCsg(node: FBSPNode): boolean {
    return (
      node.numVertices > 0 &&
      !(node.flags & (BspNodeFlags_T.NF_IsNew | BspNodeFlags_T.NF_NotCsg))
    );
  }

  public getZoneDecodeInfo(
    library: GD.DecodeLibrary,
    uLevelInfo: GA.ULevelInfo,
  ): ModelZoneDecodeResult_T {
    const result: ModelZoneDecodeResult_T = {
      bspLeaves: [],
      bspZones: [],
      bspZoneIndexMap: {},
    };
    ...
    this.leaves.forEach((leaf: FLeaf) =>
      result.bspLeaves.push(leaf.getDecodeInfo()),
    );
  ...
  public getDecodeInfo(
    ...
    const result: ModelDecodeResult_T = Object.assign(
      this.getZoneDecodeInfo(library, uLevelInfo),
      {
        ...
        bspRenderBounds: [],
        bspSections: [],
        bspSectionIndexMap: new Map(),
        geometries: [],
        materials: [],
      },
    );

    this.multiLightmaps.map((lm: FMultiLightmapTexture) =>
      builder.pullStaticLightmap(lm.textures[0].staticLightmap),
    );
    ...
    for (let i = 0; i < result.bspLeaves.length; i++) result.leafActors[i] = [];

    for (let i = 0; i < this.bspNodes.length; i++) result.nodeToSection[i] = -1;

    const _box = FBox.make();

    for (let i = 0, len = this.bounds.getElemCount(); i < len; i++)
      result.bspRenderBounds[i] = this.bounds
        .getElem(i, _box)
        .getDecodeInfo() ?? { isValid: false, min: [0, 0, 0], max: [0, 0, 0] };

    const sectionMap = new Map<
      PriorityGroups_T,
      Map<string, ObjectsForSection_T>
    >();

    for (
      let nodeIndex = 0, ncount = this.bspNodes.length;
      nodeIndex < ncount;
      nodeIndex++
    ) {
      ...
      if (node.iCollisionBound >= 0) {
        ...
        const initialVector = new Float32Array(
          new Int32Array(
            hullIndexList.slice(hullPlanesCount + 1, hullPlanesCount + 1 + 6),
          ).buffer,
        );
      ...
      const lightmapIndex: FLightmapIndex =
        node.iLightmapIndex === undefined
          ? null
          : this.lightmaps[node.iLightmapIndex];
      const lightmap = lightmapIndex
        ? (this.multiLightmaps[lightmapIndex.iLightmapTexture].textures[0]
            .staticLightmap as GA.FStaticLightmapTexture)
        : null;
      const priority: PriorityGroups_T =
        /*false &&*/ surf.flags & PolyFlags_T.PF_AddLast
          ? "transparent"
          : "opaque";

      // Get material UUID
      const materialUuid = builder.pullMaterial(surf.material);
      const lightmapTextureIndex = lightmapIndex
        ? lightmapIndex.iLightmapTexture
        : -1;
      ...
      // UE2 groups sections by: Material + PolyFlags + iLightMapTexture
      // PolyFlags used: PF_Unlit | PF_Selected | PF_TwoSided (from UnModel.cpp line 1000)
      const sectionPolyFlags =
        surf.flags &
        (PF_Unlit | PolyFlags_T.PF_Selected | PolyFlags_T.PF_TwoSided);
      ...
      const isOutdoor =
        zoneIndex === 0 || (zoneActor && (zoneActor as any).isSunAffected); // zone 0 is LevelInfo (Outdoor)
      ...
      const light: LightmapInfo = !lightmap
        ? null
        : {
            uuid: lightmap.uuid,
    ...
    const _textureBase = FVector.make(),
      _position = FVector.make();
    const _textureX: FVector = FVector.make(),
      _textureY: FVector = FVector.make();
    const _tangentZ: FVector = FVector.make();

    // Create sections from sectionMap (UE2-style: material + lightmap only, NOT split by zone)
    const createSection = (
      priority: PriorityGroups_T,
      sectionKey: string,
      sectionData: ObjectsForSection_T,
    ): number => {
      ...
      const uvs = new Float32Array(totalVertices * 2),
        uvs2 = new Float32Array(totalVertices * 2);
      ...
      for (const { node, surf, light, nodeIndex } of nodes) {
        const textureBase: FVector = this.points.getElem(
          surf.pBase,
          _textureBase,
        );
        const textureX: FVector = this.vectors.getElem(
          surf.vTextureU,
          _textureX,
        );
        const textureY: FVector = this.vectors.getElem(
          surf.vTextureV,
          _textureY,
        );
        ...
        for (
          let vertexIndex = 0, vcount = node.numVertices;
          vertexIndex < vcount;
          vertexIndex++
        ) {
          const vert: FVert = this.vertices.getElem(
            node.iVertPool + vertexIndex,
          );
          const position: FVector = this.points.getElem(
            vert.pVertex,
            _position,
          );
          ...
          const vOffset = dstVertices * 3,
            uOffset = dstVertices * 2;
          ...
          if (light) {
            ...
            const u = posLightmapped.x / light.size.width,
              v = posLightmapped.y / light.size.height;
            const { width, height } = light.resolution;
            const lmU = light.offset.x / width + u * (light.size.width / width);
            const lmV =
              light.offset.y / height + v * (light.size.height / height);
            uvs2[uOffset + 0] = lmU;
            uvs2[uOffset + 1] = lmV;
          }
      ...
      const sectionInfo: GD.IBSPSectionDecodeInfo_T & { isOutdoor: boolean } = {
        ...
        side:
          this.isSky || sectionData.polyFlags & PolyFlags_T.PF_TwoSided
            ? 2
            : undefined, // 2 = DoubleSide
        blendingMode: this.isSky
          ? !(sectionData.polyFlags & PolyFlags_T.PF_TwoSided)
            ? "brighten"
            : "normal"
          : sectionData.polyFlags & PolyFlags_T.PF_Additive
            ? "brighten"
            : undefined,
      };
...
type ModelZoneDecodeResult_T = {
  bspLeaves: GD.IBSPLeafDecodeInfo_T[];
  bspZones: GD.IBSPZoneDecodeInfo_T[];
  bspZoneIndexMap: Record<string, number>;
};
type ModelDecodeResult_T = ModelZoneDecodeResult_T & {
  bspNodes: GD.IBSPNodeDecodeInfo_T[];
  bspColliders: GD.IBoxDecodeInfo[];
  leafActors: GD.IBaseObjectOrInstanceDecodeInfo[][];
  nodeToSection: number[];
  nodeZoneMasks: bigint[];
  bspRenderBounds: GD.IBoxDecodeInfo[];
  bspSections: GD.IBSPSectionDecodeInfo_T[];
  bspSectionIndexMap: Map<string, number>;
  geometries: [string, GD.IGeometryDecodeInfo][];
