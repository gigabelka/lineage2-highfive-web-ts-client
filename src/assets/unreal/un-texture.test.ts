...
function buildExport(
  mips: { w: number; h: number; payload: number }[],
  prependLen: number,
  startPos = 4096,
) {
  const bytes: number[] = [];
...
describe("locatePrependedMipArray", () => {
  it("finds the mip array after a variable-length prepended block (single mip)", () => {
    const { dv, startPos, readTail, offset } = buildExport(
      [{ w: 32, h: 32, payload: 512 }],
      205,
    );

    expect(locatePrependedMipArray(dv, startPos, readTail, 32, 32)).toBe(
      offset,
    );
  });
  ...
  it("finds the mip array with a full mip chain and multi-byte compact indices", () => {
    ...
    expect(locatePrependedMipArray(dv, startPos, readTail, 256, 256)).toBe(
      offset,
    );
  });

  it("returns -1 when mip 0 does not match the declared texture size", () => {
    const { dv, startPos, readTail } = buildExport(
      [{ w: 32, h: 32, payload: 512 }],
      205,
    );
  ...
  it("returns -1 when the mip array does not run exactly to the export end", () => {
    ...
    expect(locatePrependedMipArray(dv, startPos, readTail - 1, 32, 32)).toBe(
      -1,
    );
  });
