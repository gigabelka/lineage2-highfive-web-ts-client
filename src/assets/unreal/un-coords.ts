...
class FCoords extends UObject {
  ...
  static fromRotator({ pitch, yaw, roll }: GA.FRotator) {
    ...
    tmpCoords.yAxis.y =
      DAT_101e29dc + (((roll + 0x4000) >> 0x2) & 0x3fff) * 0x4;
    ...
    tmpCoords.xAxis.x =
      DAT_101e29dc + (((pitch + 0x4000) >> 0x2) & 0x3fff) * 0x4;
    tmpCoords.origin.x = 0.0;
    tmpCoords.origin.y = 0.0;
    tmpCoords.origin.z = 0.0;
    tmpCoords.yAxis.x = 0.0;
...
function multiplyAxis(coords: FCoords, inVector: GA.FVector) {
  ...
  outVector.x =
    fVar3 * coords.xAxis.z + fVar2 * coords.xAxis.y + fVar1 * coords.xAxis.x;
...
function multiplyOrigin(coord: FCoords, inVector: GA.FVector) {
  ...
  outVector.x =
    fVar9 * coord.xAxis.z + fVar8 * coord.xAxis.y + fVar7 * coord.xAxis.x;
...
function divVector(coord: FCoords, other: FVector): FCoords {
  ...
  return FCoords.make(origin, coord.xAxis, coord.yAxis, coord.zAxis);
}
