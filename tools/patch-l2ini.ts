#!/usr/bin/env node
/*
 * patch-l2ini.ts — сменить ServerAddr в оригинальном l2.ini клиента C4.
 *
 ...
 *
 * Usage:
 *   npm run patch-l2ini -- --ip 192.168.0.33              # -> плайнтекст
 *   npm run patch-l2ini -- --ip 192.168.0.33 --encrypt    # -> Lineage2Ver413
 *   npm run patch-l2ini -- --check                         # только показать [URL]
 *   npm run patch-l2ini -- --ip X --file path/to/l2.ini
 *
 * Внешних зависимостей нет — только встроенные zlib/fs + BigInt (запуск через tsx).
 */
...
function parseArgs(argv: string[]): Args {
    const args: Args = { ip: null, file: null, check: false, encrypt: false };
...
function bigIntToBlock(x: bigint): Buffer {
 ...
 */
function decryptVer41x(body: Buffer): Buffer {
 ...
 */
function encryptVer413(plainBuf: Buffer): Buffer {
    const mod = BigInt("0x" + RSA_MODULUS_HEX);
...
function getPlaintext(buf: Buffer): { text: string; wasEncrypted: boolean; version?: string } {
...
function patchServerAddr(text: string, ip: string): { text: string; old: string; changed: boolean } {
...
function main(): void {
    ...
    if (args.help) {
        console.log("npm run patch-l2ini -- --ip <addr> [--encrypt] [--file <l2.ini>] [--check]");
        return;
    }
